...
TL;DR: 本文提出了一种细胞分布归一化（C-Norm）方法，并结合YOLOv12与DINOv3模块，提升了TCT图像中宫颈癌细胞检测的准确性和泛化能力，在真实临床条件下实现了优于主流算法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型在实际临床条件下对TCT图像的检测效果差，主要受限于细胞分布不均和高质量标注数据稀缺的问题。

Method: 提出C-Norm方法，通过解耦异常与正常细胞并重新合成图像，实现细胞群均匀分布；结合YOLOv12与DINOv3构建混合架构，提升检测能力和特征表达。

Result: 实验表明该方法显著优于主流检测算法，在TCT图像识别任务中达到先进水平。

Conclusion: 所提方法有效缓解了细胞分布偏差带来的泛化性能下降问题，为宫颈癌早筛提供了更可靠的AI解决方案。
...
TL;DR: Boogu-Image-0.1是一个开源的多模态理解与生成模型系列，在文本到图像生成、快速推理、指令编辑和中英双语文本渲染方面表现出色，仅用2.0862亿独特图像和约40万美元训练成本，性能媲美甚至超越其他开源模型，并接近闭源系统。


<details>
  <summary>Details</summary>
Motivation: 现有的闭源多模态系统（如Nano-Banana-Pro和GPT-Image-2）虽然性能强，但其内部技术未公开。本文旨在通过透明的方法，在有限计算资源下实现高性能的统一多模态模型，推动开源社区发展。

Method: 通过改进模型理解能力、提升数据质量、优化训练流程，并结合基于代理的推理时扩展策略，构建了包含Base、Turbo、Edit和Edit-Turbo四种变体的Boogu-Image-0.1模型家族。

Result: Boogu-Image-0.1在多个标准基准上表现优于或匹配现有开源模型，接近顶尖闭源系统水平，且仅使用2.0862亿独特图像，基础模型训练成本约为40万美元。

Conclusion: 在有限计算预算下，通过针对性改进模型架构、数据与训练方法，结合推理时扩展机制，可显著提升多模态生成与编辑性能。作者已开源模型权重、代码和训练方案，以促进统一多模态研究的发展。
...
TL;DR: 本文提出了一种基于Transformer的掩码自编码器（Masked Autoencoder）方法，用于无监督学习钢材表面缺陷的表示，并通过聚类实现缺陷分组，取得了91.3%的匈牙利匹配准确率。


<details>
  <summary>Details</summary>
Motivation: 由于钢材表面缺陷的标注数据稀缺且获取成本高，而未标注图像丰富，因此需要一种无需标签的自监督方法来学习有效的特征表示，以支持缺陷的无监督分组。

Method: 采用基于Transformer的掩码自编码器进行预训练，随机遮蔽75%的图像块，利用轻量级解码器从剩余25%可见块中重建被遮蔽区域；同时，编码器与一个仅用于训练信号的辅助缺陷定位目标联合训练；随后使用UMAP降维和凝聚聚类对预训练编码器提取的特征进行聚类分析。

Result: 解码器在重建任务上达到0.92的结构相似性得分和0.47的均方误差；在无监督聚类中，与六种已知缺陷类别的匈牙利匹配准确率达到91.3%。

Conclusion: 该方法能有效利用大量无标签数据学习钢材表面缺陷的有意义表示，实现了高性能的无监督缺陷分组，具有在实际工业质检中应用的潜力。
...
TL;DR: 本文提出了一种名为MGFace的遮罩门控人脸识别管道，通过预测查询人脸的遮罩状态并选择性地进行全局或局部相似性计算，显著提升了遮挡情况下的人脸识别准确率，同时大幅降低了计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理面部遮挡（如口罩）时识别精度下降明显，且依赖高成本的细粒度匹配机制，难以适用于大规模检索场景。

Method: MGFace根据查询人脸是否被遮挡进行分类：对未遮挡人脸采用全局嵌入匹配，对遮挡人脸则激活基于上半脸区域的局部重排序机制，实现高效且准确的识别。

Result: 在LFW-Mask数据集上，使用FaceNet和ArcFace骨干网络分别达到80%以上和90%以上的识别准确率，相比EMD重排序方法性能更优，查询速度提升约20倍。

Conclusion: MGFace能有效提升遮挡人脸的识别性能，同时保持低计算开销，适合实际大规模应用。
...
TL;DR: 本文系统研究了自监督学习中预训练-微调（PFT）与联合训练（JT）两种范式的效果，发现在不同任务、标签数据比例和领域复杂性下，JT在数据效率和低标签场景中表现更优，而PFT在特定领域更可靠。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习多采用两阶段的预训练-微调方法，但自监督与监督目标之间的交互机制尚不清楚，本文旨在探究联合优化是否是更优选择。

Method: 对比预训练-微调（PFT）与联合训练（JT）两种训练范式，在八种代表性自监督方法和多个视觉任务（包括自然图像、医学、灾害响应和遥感数据）上进行实验，评估不同标签数据比例下的性能。

Result: 结果显示JT在低标签设置下更具鲁棒性，且提升数据和训练效率；PFT在专业化领域更可靠；同时分析了表示质量、鲁棒性和跨域泛化能力。

Conclusion: 联合训练在多数通用和低资源场景中优于传统两阶段方法，但任务特性和领域复杂性是选择训练策略的关键因素，本文为混合式半监督学习提供了实证基准和实践指导。
...
TL;DR: 本文提出了一种名为BitMind Forensics（BMF）的新型深度伪造检测方法，通过在开放对抗竞赛Bittensor SN34中持续更新训练数据分布，解决了现有静态检测器在现实场景中性能大幅下降的问题。实验表明，BMF在多个公开数据集上显著优于现有开源和商业检测器，具有更强的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测器在学术基准上表现良好，但在真实世界内容上性能急剧下降（AUC下降45-50%），主要因为其训练方式静态，无法适应不断演进的生成技术。因此需要一种能够持续更新、动态应对新型伪造手段的检测机制。

Method: BMF采用在Bittensor SN34平台上运行的开放对抗竞赛机制进行训练，该机制持续引入新的生成模型所创建的伪造样本，从而动态更新训练数据分布。模型包含图像、通用视频和人像视频三种检查点，并在多个数据集上进行了广泛评估。

Result: BMF在Sumsub数据集上达到0.936 AUC，在其四条件操纵测试中达到0.872 pooled AUC，并在压缩和降尺度等扰动下保持稳健；在Deepfake-Eval-2024上图像检测AUC为0.915，视频为0.822，均超过最佳商业检测器；在AI-image panel上达0.991 AUC，在GenVidBench上达0.918 AUC，并在DFDC和Celeb-DF v2上显著超越传统FF++训练模型；时间序列研究表明模型随时间持续改进。

Conclusion: BMF通过持续学习机制有效应对了深度伪造技术快速演进带来的挑战，在多种现实场景和基准测试中展现出卓越的检测性能和鲁棒性，标志着从静态检测向动态自适应检测的重要转变。
...
TL;DR: 提出了一种结合主动学习和双损失优化的人机协同知识获取框架，显著减少了腹腔镜视频中物体定位与分割所需的标注工作量。


<details>
  <summary>Details</summary>
Motivation: 精确的腹腔镜视频时空标注耗时且依赖专家知识，限制了手术工具分割模型的发展与应用。

Method: 采用基础模型生成时间一致的类别激活图（CAMs），结合视频级工具存在标签的弱监督损失和主动学习获得的人工修正图像级掩码损失，通过迭代生成伪掩码并引导专家修正，实现高效标注。

Result: 相比完全手动标注，该框架在训练结束时将标注工作量减少了50%。

Conclusion: 该人机协同框架通过最小化专家输入实现了高效的手术视频标注，支持向更大、更多样化的数据集及真实临床场景扩展，为手术工具分割模型的可扩展性提供了实用解决方案。
...
TL;DR: 本文提出JITOMA，一种用于3D场景图的即时按需记忆激活框架，通过任务热图和大语言模型动态激活相关节点，减少冗余感知和延迟，提升边缘平台上的实时性能。


<details>
  <summary>Details</summary>
Motivation: 传统3D场景图构建方法存在感知饱和和高延迟问题，难以满足边缘设备对实时性和低延迟的需求。

Method: 提出JITOMA框架，前端使用任务热图过滤观测数据，维护低成本的休眠锚点；后端利用大语言模型解析机器人意图，动态激活与任务相关的局部子图进行密集处理。

Result: 实验表明，JITOMA显著减小了活跃图规模和描述生成延迟，在长视野任务切换下保持稳定的处理时间。

Conclusion: JITOMA有效解决了3D场景图在边缘平台上的感知冗余与实时性矛盾，实现了高效、动态的场景理解。
...
TL;DR: AffectFlow-DINO是一个用于第11届ABAW挑战赛的多任务学习系统，结合条件修正流头建模野外面部行为的模糊性，通过蒙特卡洛采样实现不确定性感知的一对多预测，在多任务性能上显著超越基线。


<details>
  <summary>Details</summary>
Motivation: 面部情感在真实场景中具有高度模糊性和多样性，传统确定性模型难以捕捉这种不确定性，因此需要一个能建模一到多映射关系的框架以提升多任务情感识别性能。

Method: 提出AffectFlow-DINO，基于冻结的DINOv3 ViT-S/16视觉骨干网络，引入条件修正流（rectified-flow）头部，联合估计连续效价-唤醒度、分类八种面部表情并检测十二种动作单元，并采用蒙特卡洛采样生成多样化预测结果；结合后处理阈值校准与骨干微调策略优化性能。

Result: 修正流解码显著提升确定性预测，尤其在效价-唤醒估计上CCC-V提升+0.058；阈值校准使罕见类如恐惧的检测率从3.8%提升至33.1%；最终模型P_MTL达1.177，远超官方基线0.45。

Conclusion: AffectFlow-DINO通过引入生成式修正流建模面部情感的不确定性，实现了更鲁棒和准确的多任务情感识别，验证了概率化建模在复杂面部行为分析中的有效性。
...
TL;DR: 提出了一种鲁棒的、偏振感知的可微路径追踪方法，通过结合路径重放和局部缓存来估计无偏梯度，有效解决了传统方法在处理偏振光时因算子秩不足导致的数值不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于物理的可微渲染方法大多忽略偏振信息，而偏振光包含对场景几何和材质属性有约束的有价值线索；将反向模式微分扩展到偏振光模拟领域面临由常见偏振算子（如线性偏光片、漫反射）秩不足引起的数值不稳定问题。

Method: 基于Mueller-Stokes演算构建前向偏振光模拟，并结合路径重放与局部缓存机制实现反向梯度计算，从而支持对复杂场景中材质和光照参数的高效稳定优化。

Result: 实现了在复杂场景下的稳定、无偏梯度估计，显著提升了包含偏振信息的逆向渲染任务（如3D重建、反射率估计、光照估计）的精度与稳定性。

Conclusion: 所提出的方法克服了传统可微渲染在偏振光处理中的关键数值难题，拓展了基于物理的逆向渲染的应用范围。
...
TL;DR: 本文提出了一种无需训练的聚焦语义记忆系统FOLIO，用于在线流式视频理解，通过动态焦点机制在段级更新记忆，平衡细节保留与内存开销，在多个基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在流式视频理解中，视频帧持续到达且查询可能随时发生，传统方法难以有效管理不断增长的历史信息并满足未来未知查询的需求，尤其是对特定实体和动作的时间变化的理解。

Method: FOLIO结合短期视觉缓冲区和长期语义记忆，后者围绕观察到的实体组织，并链接至视觉证据缓存；利用动态焦点状态在段级别更新记忆，并在查询时采用轻量级混合检索，结合结构化记忆上的直接匹配与语义查询扩展。

Result: FOLIO在OVO-Bench上以Qwen3-VL-8B达到82.0/69.1的感知/回溯准确率，在StreamingBench上取得74.5的整体准确率，显著降低了流式记忆维护成本。

Conclusion: FOLIO通过聚焦关键内容并紧凑存储上下文，有效解决了流式视频理解中的记忆管理难题，实现了高效、准确的实时查询响应。
...
TL;DR: 提出了一种基于Fréchet距离损失（FD-loss）的微调方法，用于提升扩散生成模型在合成复杂异质性肿瘤医学图像中的性能，显著改善下游分割任务的表现。


<details>
  <summary>Details</summary>
Motivation: 标准的去噪目标函数（逐像素误差最小化）会平滑肿瘤等不规则结构的高方差特征，导致生成图像无法准确捕捉复杂形态，限制了其在临床分割任务中的应用。因此需要一种能更好保留肿瘤形态特征的生成方法。

Method: 引入Fréchet Distance损失（FD-loss），在预训练编码器空间中对齐真实与生成图像的一阶和二阶特征统计量，并将其用于微调扩散生成模型。该方法结合自然图像和医学图像的编码器，在多种肝癌和脑癌数据集（CT与MRI）上进行验证。

Result: 使用FD-loss正则化的合成数据训练的下游分割网络，肿瘤DSC指标相比未正则化方法平均提升超过5%，且定性分析显示肿瘤合成更真实、分割幻觉更少。

Conclusion: FD-loss是一种有效的医学图像生成模型正则化策略，能够显著提升生成图像在复杂肿瘤结构上的保真度，进而优化临床工作流中的下游任务性能。
...
TL;DR: 该研究质疑了视频大语言模型在基准测试中准确率等同于视觉理解能力的假设，提出了“视觉依赖差距（VDG）”作为评估指标，发现多数模型的性能提升并不依赖于真正的视觉输入，尤其是时间顺序信息贡献极小，表明当前视频LLM的基准表现可能并未反映真实的视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 检验视频大语言模型在基准测试中表现出的高准确率是否真正依赖于视觉输入，还是仅靠语言先验即可完成任务。

Method: 引入视觉依赖差距（VDG）指标，通过在原始视频与黑屏条件下比较模型表现，并使用MVBench上的McNemar检验分析准确性与视觉依赖的关系；设计诊断阶梯实验（黑屏、单帧、打乱帧序、原始视频）以分析不同视觉线索的贡献，并对帧率和编码（H.264）进行控制实验。

Result: 准确性与视觉依赖可分离：模型在原始视频上表现不同（p=0.0003），但在黑屏下无差异（p=0.53）；属性感知任务具有强视觉依赖，而时序推理接近语言基线；帧多样性带来主要视觉增益，时间顺序几乎无贡献；0.5至24FPS的消融实验排除稀疏采样的影响；H.264编码导致问题级答案双向翻转；四个API模型的VDG值介于0.025–0.315之间。

Conclusion: 提出VDG作为标准审计工具，揭示当前视频LLM基准表现不能可靠反映视觉理解能力，呼吁更严谨的评估方式以区分语言记忆与真实视觉接地能力。
...
TL;DR: 本文提出了一种基于专家示范的检索时过程推理方法，用于生成高质量的程序化材质。通过将专家工作流程表示为包含构造步骤、参数和设计意图的过程轨迹，并利用预训练的大语言模型合成与用户意图对齐的过程轨迹，再将其转化为可执行的Blender材质图，实现了优于现有方法的生成与编辑性能。


<details>
  <summary>Details</summary>
Motivation: 现有的程序化材质生成方法主要关注于复制节点图，而忽视了专家在创建材质时的实际构建过程。为了提升生成质量，需要捕捉并利用专家的设计流程和决策逻辑。

Method: 将专家的工作流程表示为文本形式的过程轨迹，结合从教程视频中提取的LLM兼容的轨迹数据，使用基于大语言模型的ProcessSynthesizer生成符合用户意图的过程轨迹，并通过Compiler模型将其转换为可在Blender中执行的材质图。

Result: 在五名具有平均7.5年经验的Blender艺术家参与的专家研究中，该方法生成的材质工作流需要更少的修改，且更接近专业设计策略；在150名参与者的大规模用户研究中，该方法在生成和编辑性能上均优于先前的程序化系统。

Conclusion: 通过将过程推理引入程序化材质生成，能够有效提升生成结果的质量和实用性，验证了利用专家示范中的过程知识对于改进自动化系统的重要性。
...
TL;DR: 提出了一种名为SARFA的新框架，通过放射组学特征对齐和直接偏好优化，在医学图像分割中改进了对模糊目标的分割性能。


<details>
  <summary>Details</summary>
Motivation: SAM在处理模糊目标时表现不佳，而医学图像中如肿瘤等目标的边界常不明确，需要更鲁棒的分割方法。

Method: 利用概率提示生成多个可能的掩码，并通过基于Fréchet放射组学距离（FRD）和直接偏好优化（DPO）的目标函数进行优化，使分割结果在放射组学特征上与真实区域对齐。

Result: 在CT和MRI数据集上，SARFA优于现有的模糊分割方法，验证了放射组学特征对齐和DPO式排序作为训练目标的有效性。

Conclusion: SARFA通过结合放射组学特征和候选掩码排序，提升了医学图像中模糊目标的分割准确性，具有临床应用潜力。
...
TL;DR: 本文提出了TC-UAP，首个能够抵御基于参考和微调的视频个性化定制的通用对抗性保护方法，通过在多视频滑动窗口上优化具有时间一致性的扰动，实现对未见视频和多种时间攻击的鲁棒保护。


<details>
  <summary>Details</summary>
Motivation: 现有图像保护方法无法有效应对视频在时间压缩、时序编辑和动态变化下的隐私与身份保护需求，缺乏对视频级个性化定制的有效防御手段。

Method: 提出TC-UAP，通过在多个视频的滑动窗口上联合优化多帧通用对抗扰动，结合内在时间建模和外在代理时间攻击损失，确保扰动的时间一致性与跨视频泛化能力。

Result: 实验表明TC-UAP在参考式和微调式视频定制场景下均显著优于现有方法，能有效保护身份信息，并对多种未见过的时间攻击保持鲁棒性。

Conclusion: TC-UAP为视频内容提供了首个统一且鲁棒的抗个性化保护方案，推动了多媒体安全与隐私保护的发展。
...
TL;DR: 本文提出了一种无标记增强现实（AR）工作流程，用于将头颈部肿瘤手术中切除标本的阳性边缘信息映射到患者切除床，显著提高了定位精度。


<details>
  <summary>Details</summary>
Motivation: 在头颈肿瘤手术中，病理医生基于变形后的切除标本判断阳性边缘，而外科医生需根据口头描述在患者体内定位对应位置，缺乏视觉引导，容易产生误差。因此需要一种更精确、直观的映射方法。

Method: 该方法结合了轮廓约束形变、深度扫描残差配准、基于表面的头戴显示器融合以及目标投影技术。通过 bead-suture 对应关系估计标本形变，且患者到显示设备的融合无需外部标记物。

Result: 实验结果显示，颊部和头皮的形变目标误差分别为 7.63±3.74 mm 和 3.72±1.02 mm；残差配准距离为 2.43±2.15 mm 和 2.19±1.06 mm；无标记融合误差为 2.15±0.87 mm；端到端定位误差从口头指导的 21.40±3.84 mm 显著降低至 AR 指导的 6.19±1.79 mm（p < 0.001），在线融合耗时仅 5.23±0.34 秒。

Conclusion: 该无标记 AR 导航系统能有效提升阳性边缘的定位精度，支持更精准的肿瘤再切除，具有临床应用潜力。
...
TL;DR: 提出了一种用于情感犹豫视频识别的音频-文本系统，融合了语音、文本和手工特征，并通过跨注意力和多实例学习进行建模，集成模型在公开数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 针对ABAW竞赛中的Ambivalence/Hesitancy识别挑战，探索不依赖视觉信息的多模态方法，专注于音频与文本中表达的矛盾与犹豫信号。

Method: 将视频切分为5秒重叠窗口，结合语音描述符、RoBERTa情感嵌入和不确定性相关手工特征；使用时间交叉注意力融合音频与文本，手工特征输入至门控MIL池化以调整窗口权重，最后集成五个独立模型的预测结果。

Result: 在公共开发集上达到0.875的平均精度和0.72的宏F1分数。

Conclusion: 所提非视觉方法能有效捕捉犹豫情绪，验证了音频与语言特征在识别态度矛盾中的重要性，具备可复现性（代码开源）。
...
TL;DR: 开放词汇检测器的置信度在物体被严重遮挡时仍保持高位，且与实际可见性无关，导致其无法准确反映特定目标的可见状态，影响多种下游应用。


<details>
  <summary>Details</summary>
Motivation: 研究开放词汇检测器在物体遮挡情况下的置信度是否真实反映目标可见性，揭示当前方法在感知遮挡时的根本缺陷。

Method: 通过几何分割oracle提供无需检测器的真实可见性标签，系统评估三种主流检测器（Grounding DINO、OWLv2、SAM3）在不同遮挡程度、类别和场景下的置信度变化。

Result: 检测器置信度在遮挡加剧时几乎不变，且与真实可见性无关；在仅1/8场景可见时仍报告目标存在约90%的时间，并常响应同类别干扰物；该现象跨模型、类别和环境一致。

Conclusion: 当前开放词汇检测器的置信度不能作为目标可见性的可靠信号，建议使用基于目标定位的更可靠信号用于门控和评估，并发布受控基准以促进改进。
...
TL;DR: 提出DiffGI，一种端到端的3D到2D映射框架，使用连续的2D截断符号距离函数（TSDF）和可微分Marching Squares算法，实现高保真、低资源消耗的3D生成，尤其适用于服装等复杂表面。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型依赖隐式体表示或离散几何图像方法，难以处理薄壳和非流形几何（如服装），且存在阶梯伪影、信息丢失及重建不可微问题。

Method: 提出Differentiable Geometry Image（DiffGI），用连续2D TSDF替代二值占据图以实现亚像素精度边界编码，并设计基于解析线性插值的可微分Marching Squares算法，使3D表面损失梯度可反传至2D潜在空间；结合几何感知法向渲染损失训练DiffGI-VAE，压缩3D表面至32×32潜在空间，并在其上构建基于流匹配目标的Transformer扩散模型进行条件生成。

Result: 在服装和物体数据集上的实验表明，该方法相比以往几何图像和体素方法，在重建保真度和边界精度上更优，且计算资源需求显著降低。

Conclusion: DiffGI通过连续表面表示与完全可微管道，有效解决了传统方法在分辨率依赖、边界失真和非可微重建方面的局限，为复杂3D形状（尤其是非流形结构）提供了高效紧凑的生成建模方案。
...
TL;DR: 提出了一种基于条件扩散模型的方法RoughNet，利用光学卫星影像实现高分辨率海冰表面粗糙度的估计，克服了传统方法空间覆盖和可扩展性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有海冰粗糙度估计方法依赖昂贵的机载调查或稀疏的实地测量，限制了空间覆盖范围和业务可扩展性。

Method: 提出RoughNet模型，通过条件扩散框架将10米分辨率的Sentinel-2多光谱影像映射为1米分辨率的局部归一化地表高程残差场，并使用机载LiDAR数据进行训练与验证。

Result: 在未见过的第三北极区域上验证，最佳模型实现域外均方根误差9厘米，且保留了粗糙度场的统计和频谱特性。

Conclusion: 生成式扩散模型能仅从光学影像中恢复物理上有意义的地表结构，为数据稀疏环境下的高分辨率海冰制图提供了可扩展的解决方案。
...
TL;DR: 本文提出FM²框架，解决跨机构医学影像联合建模中的模态异质性问题，支持从零训练与多模态对齐，在保护隐私的同时实现优异的跨模态泛化性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像基础模型需要跨机构数据聚合，但隐私法规限制了数据集中化；现有方法在医学领域迁移效果差或缺乏多模态统一能力，且未充分应对模态异质性挑战。

Method: 提出FM²框架，采用从零训练的主干网络以保持医学域保真度，引入双混合专家模块（类别级和域级MoE）及异质模态对齐正则化器（HMA），并结合GPT-4o生成的本地保留字幕实现跨模态语义桥接（CEL）。

Result: 在MIMH基准和真实世界医学VQA数据集上，FM²在分类、字幕增强学习和VQA任务中均优于现有联邦学习基线方法，并展现出强大的跨模态泛化能力。

Conclusion: FM²为联邦医学影像建模提供了一种统一且可扩展的解决方案，有效应对模态异质性问题，同时保证理论收敛性与实际性能提升。
...
TL;DR: 提出了一种名为AnomExpert的原型驱动框架，用于仅使用病例级监督的产前超声异常诊断，通过可学习的平面原型和疾病感知稀疏选择机制，在多中心数据集上表现出优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 为了在无需平面级标注的情况下，准确进行产前超声先天异常诊断，提升现有弱监督方法对解剖平面识别和疾病相关平面选择的建模能力。

Method: 提出AnomExpert框架，引入可学习的平面原型来组织无序图像，并通过疾病感知稀疏选择机制为每种异常选择关键解剖平面，仅依赖病例级标签进行训练。

Result: 在包含3,654个病例的多中心数据集上，AnomExpert优于九种典型的多实例学习方法，使用ViT-small主干网络达到86.9%的准确率和84.2%的F1分数，同时保持参数高效性。

Conclusion: 建模解剖平面识别与疾病特异性平面选择有助于提升弱监督下的多平面产前超声异常分类性能，AnomExpert为临床诊断提供了一种高效且准确的解决方案。
...
TL;DR: 本文提出了一种名为MultiAnimate的框架，用于实现多个角色在共享环境中的同步动画生成，解决了现有方法仅限于单参考图像的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的角色动画生成方法主要基于单张参考图像，难以应对多角色交互等复杂场景，限制了其应用范围。因此，需要一种能够同时处理多角色动画并保持身份一致性和空间关系的方法。

Method: 该框架通过三个关键机制实现多角色动画：1）引入身份特定的参考网络，从多张参考图像中提取外观信息；2）设计身份感知的姿态编码器，利用注意力机制准确区分和处理多个姿态序列；3）提出交互引导模块，利用角色特定的掩码信息增强角色间的交互效果。

Result: 大量实验和消融分析表明，MultiAnimate在多角色动画任务中表现出色，尤其在复杂动作序列和角色交互场景下优于现有方法。

Conclusion: MultiAnimate为多角色动画提供了一个有效的解决方案，能够在保持身份一致性和空间关系的同时，实现高质量的多角色同步动画生成。
...
TL;DR: 本文提出了一种名为ScanFocus的新型粗到精框架，用于时空视频定位（STVG），通过解耦全局时空扫描与局部边界聚焦来提升定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理长视频时因计算成本高而采用低速率时序下采样和隐式运动建模，导致高频边界线索被抑制且忽略帧间依赖关系，难以实现精确边界定位。

Method: 提出ScanFocus框架：首先使用统一的视觉-语言融合编码器和轻量级可变形语义-运动融合模块进行全局扫描并生成粗略候选；然后在精细化阶段引入语义引导的时间聚合器（SGTA），在粗略边界附近密集采样，显式建模短期时间交互以恢复细节并实现精准时间戳回归。

Result: 在三个广泛使用的基准上进行了大量实验，结果表明所提方法在性能上优于先前方法。

Conclusion: ScanFocus通过粗到精的设计有效平衡了全局上下文建模与精确边界定位，在STVG任务中实现了更优的性能表现。
...
TL;DR: 提出了一种统一的无标签CLIP引导区域评分框架，用于细粒度分类，通过比较不同区域生成方法和评分策略，发现软负边距评分效果最佳，且随机裁剪在伪标签评分中 consistently 优于SAM掩码。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的细粒度分类方法在缺乏真实标签时难以有效选择判别性证据，尚不清楚不同局部区域和评分策略的影响。

Method: 提出了一个统一的无标签区域评分框架，评估了基于余弦相似度、边距和熵的评分策略，结合SAM生成的掩码与随机裁剪，并引入基于全局图像嵌入和局部区域嵌入的两种无标签伪标签变体。

Result: 在五个细粒度数据集上实验表明，软负边距评分性能最强，伪标签评分接近真实标签表现；尽管SAM生成语义合理的掩码，但随机裁剪在所有数据集上均优于SAM；此外，SAM受益于聚合所有区域嵌入，而随机裁剪在较小top-k子集下表现更好。

Conclusion: 随机裁剪比SAM更优的原因在于其保留了周围信息，在伪标签噪声存在时提供更稳定的语义上下文，该研究为基于CLIP的细粒度分类提供了新见解。
...
TL;DR: 本文提出了一种基于单张图像的未知航天器三维形状与位姿联合估计新框架，通过学习密集的2D-3D对应关系实现6自由度姿态估计，在SPE3R数据集上表现出优异性能和强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在自主交会与近场操作中，对未知目标的6自由度位姿估计具有挑战性，因其需同时重建目标形状模型，现有方法难以兼顾精度与泛化性。

Method: 提出双阶段框架：首先从单幅图像重建目标3D形状，然后利用冻结的DINOv3视觉Transformer提取图像特征，结合动态图卷积网络编码点云几何特征，并通过双流Transformer匹配器优化描述符，生成软对应关系送入PnP求解器恢复位姿。

Result: 在SPE3R数据集上测试，平均指向误差为0.157度，仅使用单张图像和重建几何即可实现可靠位姿估计，且对未见过的航天器表现出强泛化能力。

Conclusion: 所提方法在单样本条件下实现了高精度、强泛化的未知航天器形状与位姿联合估计，推动了非合作目标空间操作的技术发展。
...
TL;DR: 提出了一种受共生关系启发的知识蒸馏方法（SIKD），通过空间和语义两个层面的共生信息保留旧知识，提升增量目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法多采用分离特征空间的策略，忽视了物体间共现与遮挡带来的空间和语义依赖，导致表示失真、类别混淆和灾难性遗忘。

Method: 提出SIKD，包含空间共生蒸馏（SpSD）和语义共生蒸馏（SeSD）：SpSD在高重叠区域进行槽对齐监督以保留可泛化的旧类特征；SeSD通过置信度加权原型和软排序对齐维持类间语义结构。

Result: 在多个标准数据集上进行了广泛实验，结果表明SIKD在保持旧类知识和学习新类方面显著优于现有方法。

Conclusion: 通过显式建模对象间的空间与语义共生关系，SIKD有效缓解了灾难性遗忘，为增量目标检测提供了新的思路。
...
TL;DR: 本文提出了一种名为GeoAnchor的交错文本-潜在推理框架，用于提升多模态大语言模型在从2D图像理解3D空间关系方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态方法主要依赖符号化文本标记或单一潜在表示，在表达连续几何信息和适应多样化空间任务方面存在局限性，难以准确理解复杂的3D空间关系。

Method: 提出GeoAnchor框架，将3D空间信息分解为位置潜变量（对象定位）、方向潜变量（关系朝向）和几何潜变量（场景结构）三个互补部分，并在结构化空间中重组以实现局部证据构建与全局上下文捕捉；采用交错的文本-潜在推理方式和协作训练策略，逐步引导模型从局部空间感知到全面的3D理解。

Result: 在多种复杂3D推理任务上的实验表明，GeoAnchor性能优于现有最先进方法，展现出更强的有效性和泛化能力。

Conclusion: GeoAnchor通过引入多类型互补的潜在表示和结构化推理机制，显著提升了多模态大语言模型对2D图像中3D空间关系的理解能力，为未来3D感知研究提供了可解释且灵活的新思路。
...
TL;DR: 提出了一种新的数学框架，用于分析和生成复杂的树状3D对象（如植物和树木），能够同时建模其几何形状和分支结构的变化。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅考虑树状物体的骨架结构或粗略的几何近似，缺乏对三维几何与分支结构联合变形的精确建模。

Method: 推广了适用于类球面的SRNF表示法至树状3D对象，并构建了一个新的黎曼树形空间及相应的度量，用以量化表面弯曲、拉伸和结构变化；在此基础上计算对应关系和测地路径。

Result: 实现了树状3D对象间的点对点和分支级对应、测地线对齐、统计汇总（均值和主变模式）以及基于概率分布的新样本生成。

Conclusion: 该框架在真实与合成植物数据上显著优于现有最先进方法，为树状3D形状的统计分析与生成提供了有效工具。
...
TL;DR: 本文提出了一种针对场景文本识别（STR）的改进型二维旋转位置编码方法2D-RoPE-STR，适配文本区域的各向异性特性并扩展至编解码器交叉注意力中，提升了对弯曲、旋转和透视变形文本的识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在场景文本识别中受限于一维位置编码，无法有效建模文本图像的二维空间结构，且现有2D-RoPE方法假设图像内容为各向同性，不适用于长宽比显著的文本区域，也未考虑编解码器架构中的跨注意力机制。

Method: 提出2D-RoPE-STR，包含两项关键设计：(1) 根据文本区域宽高比动态分配行/列维度的各向异性位置编码；(2) 将旋转变换扩展到编码器-解码器的交叉注意力中，使解码器能基于二维布局关注编码器特征。该方法无需额外参数或网络结构修改。

Result: 在六个标准数据集（IIIT5K、SVT、ICDAR 2013/2015、CUTE80、SVTP）上验证了方法有效性，尤其在弯曲、旋转和透视变形等非规则文本布局上性能提升显著。通过控制变量实验、净胜分析和注意力可视化等诊断手段，验证了相对二维位置信息的作用机制。

Conclusion: 2D-RoPE-STR通过适配各向异性文本布局并将2D旋转位置编码引入跨注意力机制，有效增强了模型对复杂空间结构文本的理解能力，且无需增加参数或改变网络结构，具有良好的实用性和可扩展性。
...
TL;DR: LPM是首个在工业规模部署的基于扩散的生成式视频修复模型，适用于复杂的真实场景退化，已在快手生产环境应用，显著提升观看体验并降低带宽成本。


<details>
  <summary>Details</summary>
Motivation: 针对用户生成内容中多样且复杂的视频退化问题，现有方法难以实现高质量、时序一致且可扩展的修复，因此需要一个统一、高效且可大规模部署的解决方案。

Method: 提出Large Processing Model（LPM），采用增强的扩散架构、渐进式训练策略和时序金字塔推理机制，结合大规模数据工程与基础模型训练，实现对任意长度视频的高保真实时修复。

Result: LPM在快手部署后，处理的视频占总播放时长的约45%，显著提升用户体验指标；在相同比特率下感知质量更高，或在相同质量下相对自研编码器降低20%比特率，每年节省数亿级带宽成本，并支持Kling等产品集成。

Conclusion: LPM证明了生成式视频修复技术在大规模实际应用中的可行性、高效性与经济价值，为UGC平台的视频处理提供了新的技术范式。
...
TL;DR: 本文提出了一种基于分层体素增强框架的高质量3D场景生成方法HIVE-3D，通过粗到细的分层超分辨率策略实现高分辨率3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有单图像生成3D对象的方法受限于表示分辨率，难以适用于复杂的3D场景生成。

Method: 提出HIVE-3D，首先生成粗糙初始场景，利用图像分割和基于注意力的检索对齐2D与3D组件，构建分层组件树，并设计体素超分辨率模型进行逐级细化。

Result: 实验表明该方法显著优于先前方法，在3D场景生成质量与分辨率上达到先进水平。

Conclusion: HIVE-3D通过分层体素增强实现了高质量、高分辨率的3D场景生成，推动了单图像3D场景重建的发展。
...
TL;DR: 提出了一种情感感知的音乐驱动360°视频生成管道Bring Music The Horizon，能够根据音乐的情感轨迹生成沉浸式、反映音乐情绪变化的全景可视化视频。


<details>
  <summary>Details</summary>
Motivation: 现有音乐可视化方法多依赖歌词或生成非沉浸式的平面视频，难以传达音乐的情感动态和提供沉浸体验，因此需要一种能结合音乐情感与全景视觉的新方法。

Method: 首先预测每四小节的效价-唤醒值以估计音乐情感轨迹，然后通过EmotiCrafter将情绪转化为视觉引导，并利用SEGA框架进行细粒度语义控制生成关键帧，最后使用图像到视频模型合成连续的360°视频。

Result: 成功生成了反映音乐情感发展和时间结构的360°可视化视频，在多种音乐类型上验证了方法的有效性，并在项目页面提供了与From-Sound-To-Sight基线模型的定性对比。

Conclusion: 该管道能够有效实现情感感知的沉浸式音乐可视化，为听众提供了更丰富的情绪表达和交互体验，展示了音乐与全景视频融合的潜力。
...
TL;DR: 本文提出GPOcc和GPOcc++，通过将视觉几何先验转化为占空感知的稀疏高斯表示，实现高效且具表达性的三维场景建模，在室内外多视角及时序占用预测任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 从视觉观测中恢复完整的三维占据表示在遮挡和未观测区域仍具挑战，现有几何先验方法侧重表面而难以建模体积极自由空间，需建立更有效的桥梁。

Method: 提出GPOcc，将视觉几何先验转换为占空感知的稀疏高斯表示；进一步构建GPOcc++框架，统一处理多视角与时间序列观测，并扩展至室外场景。

Result: 在多个室内外基准上实验表明，该方法在多视角和时序设置下均取得优异性能，具备良好效率与泛化能力。

Conclusion: GPOcc系列方法有效融合视觉几何先验与体积占据建模，为3D场景理解提供了高效、统一且可扩展的解决方案。
...
TL;DR: 本文提出CASA-SDF，一种用于室内场景高保真3D重建的神经隐式表示框架，通过融合语义与光度不确定性进行空间自适应监督，并利用曲率感知密度变换增强细小结构表达，有效平衡了平面平滑性与细节保留。


<details>
  <summary>Details</summary>
Motivation: 室内场景具有显著的几何异质性，大面积无纹理平面区域需要强正则化以抑制高频伪影，而细小结构则需要更锐利、自适应的表示来缓解MLP的频谱偏差和过平滑问题。现有方法因使用空间上一致的先验监督和全局SDF到密度的转换，难以兼顾二者。

Method: 提出CASA-SDF框架，包含两个核心模块：1）混合空间自适应不确定性退火（SAUA），融合语义与光度不确定性构建像素级课程学习策略，动态调整单目先验监督的可靠性；2）曲率感知局部自适应密度变换（CALADT），通过曲率代理逐步调节SDF到密度映射的锐度，增强对细小结构的表达能力。

Result: 在标准室内数据集上的实验表明，CASA-SDF在保持平面稳定性的同时，显著提升了表面完整性与高频结构的细节恢复能力。

Conclusion: CASA-SDF通过联合优化监督策略与表示能力的空间自适应机制，有效解决了室内场景中几何异质性带来的重建挑战，实现了平面平滑性与细节保真度的良好平衡。
...
TL;DR: 本文提出了一种用于3D异常检测的新型模型M2P-AD，通过记忆到原型模块和边界感知策略有效抑制正常区域和物体边界的误报，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法在正常区域产生过多异常响应，并在物体边界附近出现假阳性，影响检测精度和稳定性，需加以解决。

Method: 提出Memory-to-Prototype (M2P) 模块学习正常特征的代表性原型，并结合边界提取（BE）模块与边界感知分数优化（BSR）策略，利用边界信息校准异常分数。

Result: 在Real3D-AD、Anomaly-ShapeNet和MulSen-AD数据集上达到最先进性能，定性结果表明正常区域的异常分数减少，边界附近的假阳性被有效抑制。

Conclusion: 所提方法能实现更可靠、稳定的3D异常检测，适用于实际工业场景。
...
TL;DR: 本文提出了一种名为Groc-PO的多阶段偏好优化框架，通过构建包含对象、上下文和推理三个阶段的偏好数据集，增强多模态大模型在各阶段的忠实性监督，有效缓解跨阶段错误传播与幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型（MLLMs）存在视觉幻觉、内容捏造和不忠推理等问题，而传统的基于最终答案的偏好优化方法（如DPO）难以有效纠正早期接地阶段的错误，导致错误传播。因此需要一种更细粒度的、针对多阶段接地过程的优化机制。

Method: 提出了Groc-PO框架，并构建了包含物体接地、上下文接地和接地推理三阶段的GCPD偏好数据集，在每个阶段引入显式的偏好监督，实现对多模态推理过程中不同阶段的精准信用分配与错误抑制。

Result: 实验表明，相比标准DPO及其他强基线方法，Groc-PO在减少幻觉、提升忠实推理和整体可靠性方面表现更优，能有效缓解跨阶段错误传播。

Conclusion: 通过在多个接地阶段引入显式偏好监督，Groc-PO能够显著提升MLLMs的可信推理能力，验证了细粒度、接地感知的训练策略对于构建可靠多模态系统的重要性。
...
TL;DR: 本文提出了一种无需注意力机制且轻量级的视觉令牌压缩框架，通过信息熵评估令牌重要性并利用变换一致性信号保证多样性，在保持高性能的同时显著提升视觉语言模型在边缘设备上的推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉令牌压缩方法依赖注意力图或高计算成本的相似性比较，难以兼容现代加速框架且缺乏可扩展性，限制了视觉语言模型在资源受限设备上的部署。

Method: 提出一种无需注意力的轻量级令牌压缩框架：1）基于信息熵准则量化令牌的信息量以选择重要令牌；2）引入变换诱导的一致性信号，通过排序和步幅采样实现多样化令牌保留。

Result: 在多个视觉语言模型基准上实验表明，该方法在高度压缩下仍保持竞争力的准确性，显著提升了推理效率。

Conclusion: 所提框架作为一种即插即用模块，有效平衡了视觉语言模型的精度与效率，适合在资源受限的边缘设备上部署。
...
TL;DR: 提出DP-BOA，一种基于在线Dirichlet过程高斯混合模型的后验预测决策框架，显式建模类别诞生，显著提升在标准OCD基准上的新类发现性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常隐式处理类别生成，缺乏对创建新类别的统计证据支持，本文旨在通过显式比较分配到已有类别与生成新类别的后验预测证据来改进决策机制。

Method: 采用带Normal-Inverse-Wishart先验的在线Dirichlet过程高斯混合模型，在训练阶段用标注数据校准共享NIW先验并初始化已知类别后验；测试时对每个样本比较其归属于现有类别或触发新类别的证据，并在线更新类别统计。

Result: 在标准OCD基准上，DP-BOA持续优于强基线方法，尤其在新类发现方面表现突出，同时保持有竞争力的已知类准确率。

Conclusion: DP-BOA通过显式建模类别生成的统计证据，提供了更合理的在线类别发现决策机制，有效捕捉各向异性类别结构，并随证据积累自适应调整决策置信度。
...
TL;DR: 提出了一种无需病灶分割的端到端时序多模态模型，用于预测主动监测期间前列腺癌的病理进展，能有效减少不必要的重复活检。


<details>
  <summary>Details</summary>
Motivation: 现有主动监测方案依赖定期重复活检，多数并无进展发现，且现有风险分层工具受限于单时间点成像或需明确病灶分割，难以捕捉纵向变化。

Method: 利用预训练的3D MRI基础模型编码每次连续扫描，并引入时间注意力门机制重新校准多时间点特征，放大与进展相关的局部影像变化，再与临床变量融合进行多模态预测。

Result: 在纵向队列中验证，该方法持续优于其他基线模型，表现媲美放射科医生评估，具有高阴性预测值和更高的阳性预测值。

Conclusion: 该模型有望安全减少主动监测过程中的不必要活检。
...
TL;DR: 本文介绍了一种用于移动中人脸识别的边境控制场景数据集，包含近红外车辆通行视频和基于智能手机的预注册数据，揭示了现有模型在真实条件下的性能局限性。


<details>
  <summary>Details</summary>
Motivation: 由于跨境流动性持续增长，现有的边境控制基础设施面临越来越大的压力，需要在检查站内直接对车内旅客进行生物特征认证。然而，目前缺乏代表性的数据集来支持这一领域的研究和发展。

Method: 提出了一个新的数据集，该数据集包括近红外（NIR）车辆穿越视频和通过智能手机收集的预先注册数据，以模拟实际的边境检查情况。使用最先进的模型进行了基线评估。

Result: 基准测试显示，在这些现实条件下，当前最先进的模型表现出明显的性能限制。

Conclusion: 研究表明，为了推进该领域的发展，需要开发专门的方法来应对移动中人脸识别所面临的挑战。
...
TL;DR: 本文提出VGIF-Score，一种高度自动化且可解释的视频生成模型指令遵循评估框架，包含客观完成度与主观满意度双分支，并在包含223条复杂指令的VGIF-Bench上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型的评估方法多依赖短而浅语义的提示词，缺乏对长、组合性指令的充分评估，且依赖人工或手工视觉流程，诊断能力弱。

Method: 提出VGIF-Score框架：1）客观完成分支将指令解析为时空有向无环图（ST-DAG），进行依赖感知的问答与短路诊断；2）主观满意度分支使用指令条件化的AutoRubric评估 cinematography、视觉纯净度、运动平滑性和物理一致性。

Result: 在223条复杂提示和约4.3K细粒度评估项的VGIF-Bench上，对14个VGM进行超3K视频实验，表明VGIF-Score具备高可靠性、可解释性和诊断价值。

Conclusion: VGIF-Score能有效评估视频生成模型在长、结构复杂指令下的遵循能力，提供统一且可分解的评分，推动更智能的评估体系发展。
...
TL;DR: 本文提出ThinkBLOX，一种基于视觉语言模型的渐进式3D室内场景生成框架，通过构建大规模数据集和分层强化学习策略，在物理合理性、语义一致性和交互编辑性方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法在交互式编辑时需全局重优化，且现有单次生成模型难以保证物理与语义合理性，缺乏有效的增量更新能力。

Method: 提出ThinkBLOX框架，将布局生成建模为状态条件下的逐步推理-动作过程；构建包含22万+样本的ThinkBLOX-Data-200K数据集，并采用监督微调与新型分层奖励解耦的强化学习算法Tier-Decoupled GDPO进行训练。

Result: 实验表明ThinkBLOX在物理合理性、语义对齐和可编辑性上显著优于最新的单次和迭代基线方法，并支持全局与局部生成及重排等多样化应用。

Conclusion: ThinkBLOX通过渐进式推理与分层优化机制，有效提升了3D场景生成的质量与交互能力，为未来智能空间设计提供了新范式。
...
TL;DR: 本文提出了一种名为Nexus的扩散方法，用于整体生成高质量三角网格，通过解耦顶点和拓扑生成，在顶点生成中采用八叉树结构的稀疏体素扩散模型，并引入时空区间编码复杂拓扑结构，实现了优于现有自回归和两阶段方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的主流网格生成方法依赖于序列化和自回归过程，存在推理效率低和误差累积敏感的问题，因此需要一种能够克服这些限制的新型生成方法。

Method: 将网格顶点视为组织成八叉树的稀疏体素，使用扩散模型以由粗到细的方式生成顶点；对于拓扑建模，提出时空区间（Spacetime Interval）来扩展时空距离，将任意边和面拓扑编码为连续的每顶点嵌入，并用扩散模型生成这些嵌入。

Result: 在Objaverse、Toys4K数据集及野外图像上的实验表明，该方法优于当前最先进的自回归和两阶段基线方法，能够有效避免序列化建模的固有局限性。

Conclusion: Nexus通过解耦顶点与拓扑的扩散生成，实现了高效、高质量的网格生成，获得了3D从业者盲测中的强烈偏好，展示了其在实际应用中的潜力。
...
TL;DR: 提出了一种名为GHR-VLM的视觉接地混合推理框架，用于零样本公交视频分析，结合边缘-云设计以提高效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 显式视觉接地可以将长时间监控流转换为紧凑的、以乘客为中心的时空证据，从而提升VLM推理能力。

Method: 采用轻量级边缘监测器持续跟踪车门状态并分割乘客片段，后端VLM通过两阶段从粗到细的时空证据优化来识别上车乘客和支付行为。

Result: 在486分钟真实公交监控视频上的评估表明，该方法能有效支持乘客级支付行为分析，但在视频质量较差时仍面临挑战。

Conclusion: GHR-VLM通过仅在接地的乘客片段和联系图上调用VLM，减少了云端推理开销，无需支付专用训练数据，为零样本公交视频理解提供了可行方案。
...
TL;DR: 本文提出了UniPhys框架，用于将原始3D资产自动转换为具有统一物理语义的仿真就绪资产，并构建了大规模数据集UniPhys-40K和评估基准UniPhys-Bench。同时提出UniPhysGen模型，联合推理关节语义和内在物理属性，实验证明其在多种任务上达到SOTA性能，且生成的资产可直接用于机器人仿真环境。


<details>
  <summary>Details</summary>
Motivation: 现有3D资产缺乏统一的物理语义（如关节结构和物理属性），限制了在具身AI和机器人仿真中的真实交互，且当前方法独立处理这些语义或依赖标准化结构，难以泛化到异构资产。

Method: 提出UniPhys框架和UniPhysGen模型，联合推理物体的关节语义与内在物理属性，引入几何鲁棒的关节接地机制以减少在不同部件分解下的几何捷径偏差。基于该框架构建UniPhys-40K数据集和UniPhys-Bench评估基准。

Result: 实验表明UniPhysGen在关节接地和物理属性估计任务上达到最先进的性能，生成的资产可直接部署于机器人仿真环境，支持真实物理交互。

Conclusion: UniPhys实现了从异构3D资产中自动构建具有统一物理语义的仿真就绪模型，推动了具身AI和机器人模拟中对真实交互的支持。
...
TL;DR: OvisOCR2是一个0.8B参数的端到端文档解析模型，能够将文档页面图像直接转换为按自然阅读顺序排列的Markdown表示，涵盖文本、公式、表格和视觉区域。


<details>
  <summary>Details</summary>
Motivation: 为了提升文档解析的效率与准确性，克服传统流水线方法的局限性，探索端到端模型在复杂文档理解中的潜力。

Method: 构建了一个结合过滤后的真实文档注释与合成页面的数据引擎，使用相同的HTML源生成渲染图像和Markdown目标；训练过程包括监督微调、基于4B分支模型的强化学习（多组件奖励设计）、策略蒸馏至0.8B模型以及模型融合。

Result: 在OmniDocBench v1.6上取得96.58的SOTA综合得分，在PureDocBench上达到75.06的最高Avg3分数，并在内部更具挑战性的长尾场景基准上表现最优。

Conclusion: OvisOCR2展示了端到端模型在文档解析任务中的领先地位和强大泛化能力，推动了该领域的发展方向。
...
TL;DR: 本文提出了一种名为Human4K的大规模高分辨率多视角人体重建数据集，包含超过六百万张4K图像和精确的SMPL-X标注，用于提升复杂动作和自遮挡情况下的三维人体重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体重建模型在真实复杂场景中表现不佳，主要由于缺乏高分辨率图像、高精度标注和多样化全身动作的结合数据集，难以应对深度模糊和自遮挡问题。

Method: 构建了一个八视角4K相机系统，并与Vicon动捕设备同步，采集11名受试者在复杂、高度关节化和强自遮挡动作下的数据；通过Motion-Retargeting and Refinement Module（MRRM）处理序列，确保全身及末端肢体的精确对齐，并生成mocap级SMPL-X标注。

Result: 在标准基准上实验表明，使用Human4K训练显著提升了整体重建效果，尤其在手部、脚部以及深度模糊的肢体结构上改善明显。

Conclusion: Human4K为高保真全身体重建提供了高质量数据支持，有效解决了现有数据集在分辨率、精度和动作多样性上的不足，推动了复杂场景下人体重建的发展。
...
TL;DR: 本文提出了一种新的评估视觉模型颜色表示的方法，使用基于人类调查数据的86个模糊感知类别，衡量模型在类别边界、紧凑性和超越几何结构的渐变对齐方面的表现。研究发现不同Vision Transformer模型在类别级别结果相似，但在渐变对齐上差异显著，其中Masked Autoencoders（MAE）表现最佳，并在自然图像中更好地捕捉表面颜色的整体表示。


<details>
  <summary>Details</summary>
Motivation: 现有颜色表示评估方法多依赖于几何空间（如CIELAB）或离散标签，无法反映人类对颜色的渐进式组织方式。本文旨在通过更贴近人类感知的模糊分类模型，全面评估视觉模型的颜色 grounding 能力。

Method: 提出一个基于86个拟合自人类调查数据的模糊感知类别的评估框架，用于分析图像编码器在类别边界、类别紧凑性以及超越颜色几何结构的渐变对齐三方面的能力，并在11种Vision Transformer编码器上进行层间分析。

Result: 各类Vision Transformer在类别级结果上表现相近，但在渐变对齐上差异明显；Masked Autoencoders（MAE）在此指标上显著优于其他模型，且置信区间无重叠；层分析显示MAE在输出层保持了该结构；在自然图像中，MAE更全局地表示表面颜色，而语言监督模型则更关注前景物体的颜色。

Conclusion: 人类式的颜色感知包含多个独立维度，不应简化为单一评分；MAE在捕捉超越几何结构的渐进颜色感知方面表现最优，表明其颜色表示更接近人类感知。
...
TL;DR: 本研究提出了一种基于LeWorldModel的云感知地球观测序列预测方法，用于解决地表可见性预测问题，在EarthNet2021数据集上显著优于持续性基准。


<details>
  <summary>Details</summary>
Motivation: 地球观测处理链的瓶颈在于图像获取时地表是否可见，因此需要对可观测性进行准确预测以提高效率。

Method: 采用LeWorldModel这一联合嵌入预测架构的世界模型，将原始多光谱影像和气象驱动数据转换为包含五个影像通道和八个协变量的HDF5序列，并从头训练模型。

Result: 模型在下一时刻可用性预测中平衡准确率介于0.769至0.887之间，首次可用时间点预测准确率为0.602至0.806，均显著优于持久性基准；相比LightGBM在多个任务上表现更优，但在简单二分类任务和OOD场景下稍弱。

Conclusion: LeWorldModel能有效预测地球观测中的地表可见性，在复杂时序建模任务中表现优异，具备应用于实际观测调度的潜力。
...
TL;DR: 本文提出了一种名为REAL的智能体框架，用于开放世界中的移动操作任务，通过模拟到真实一致的环境API和人类在环交互机制，在主动探索、视觉干扰、关节操作和交互消歧等复杂任务中实现了卓越性能，并在真实双臂移动机器人上验证了其零样本迁移能力和现实差距的弥合效果。


<details>
  <summary>Details</summary>
Motivation: 现实世界中具身智能体的部署面临主动探索、视觉定位和交互式意图消歧等挑战，而现有方法多依赖于理想的模拟器状态或假设指令完整，难以应对实际场景。因此需要一个更贴近真实部署条件的框架来解决这些问题。

Method: 提出了REAL框架，构建了无需感知先验知识的模拟到真实一致的环境API，引入模拟用户实现人机交互；设计多样化的任务组合以支持数据收集、监督微调和在线强化学习，系统性优化智能体性能，并推出涵盖241项任务的REAL-Bench基准进行综合评估。

Result: 实验结果显示，训练后的智能体在交互任务中达到56.9%的成功率，优于主流商业闭源视觉语言模型；在真实双臂移动机器人上的60个任务中实现78.3%的端到端成功率，表现出强大的零样本迁移能力。

Conclusion: REAL框架通过统一的仿真到现实设计，有效缩小了现实差距，能够在长视野移动操作任务中实现高效、鲁棒的零样本迁移，为现实世界具身智能体的部署提供了可行路径。
...
TL;DR: 本文提出了一种基于SMPL-X先验的文本驱动3D人体服装编辑方法T3HG-Editor，通过可编辑高斯获取、服装一致性编辑和带溢出修剪的高斯更新三个阶段，实现了高保真且一致的3D服装编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯编辑方法在应用于人体服装编辑时存在失真和服装不一致问题，而SMPL-X模型提供了丰富的人体先验信息，为解决该问题提供了新思路。

Method: 提出T3HG-Editor，包含三个阶段：沿SMPL-X法线生成近表面高斯并用2D掩码定位可编辑高斯；跨视角聚合对应同一SMPL-X顶点的token以保持服装一致性；利用SMPL-X上的符号距离函数构建人体距离场，结合2D语义掩码修剪溢出高斯。

Result: 在多个主体和多种服装类型上的实验表明，该方法在编辑质量和服装一致性方面优于现有最先进方法。

Conclusion: T3HG-Editor有效利用SMPL-X的几何与关节先验，解决了文本驱动3D人体服装编辑中的失真与不一致问题，显著提升了编辑质量。
...
TL;DR: 本文提出了一种名为FreeLit的无需配对数据的室内场景重光照框架，通过物理引导的光照先验和去噪扩散模型实现对光源位置、颜色和强度的显式控制，并引入了光照不变反射率稳定策略以提升低光场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于数据驱动的方法依赖成对多光照数据，成本高且难以扩展；逆向渲染方法虽减少数据依赖但内在估计在复杂条件下鲁棒性不足。因此需要一种无需配对数据且具备强鲁棒性的可控重光照方法。

Method: 提出FreeLit框架，利用物理引导的光照先验从固有图像属性生成结构化光照图和伪重光照图像，指导扩散模型合成；引入重光照引导的内在稳定性策略，通过结构感知蒸馏和一致性约束保持反射率的光照不变性；设计面向可控性的评估指标，量化用户指定光照颜色与强度的匹配程度。

Result: 实验表明FreeLit在无需配对监督的情况下实现了稳定、符合物理规律且可控制的室内重光照效果，在低光场景下表现出更强的鲁棒性，并在新提出的可控性指标上表现优异。

Conclusion: FreeLit通过结合物理先验与扩散模型，在不依赖配对数据的前提下实现了精细的光源级控制，提升了室内重光照在复杂条件尤其是低光场景下的稳定性与可控性，为实际应用提供了更可行的解决方案。
...
TL;DR: SFF-CLIP提出了一种无需额外区域标注的细粒度微调方法，通过运行时的区域-短语对齐机制提升CLIP的细粒度特征表示能力，同时保持其全局视觉语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP微调方法依赖额外区域标注，限制语义多样性且耗费数据处理成本，同时可能损害CLIP原有的全局表示能力。

Method: 设计了一种运行时区域-短语对齐方案：从输入句子中提取概念短语，并利用文本特定热图将其与提取的基于区域的特征对齐，仅使用图像-文本对进行微调。

Result: 实验表明，SFF-CLIP在细粒度密集特征表示任务上显著提升性能，同时在图像级任务上保持了CLIP原有性能。

Conclusion: SFF-CLIP有效增强了CLIP的细粒度理解能力，无需额外标注，兼顾局部细节与全局语义，为CLIP微调提供了高效可行的解决方案。
...
TL;DR: POLY-SIM 2026挑战旨在解决多模态说话人识别中模态缺失和多语言带来的鲁棒性与泛化难题，并提供标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 现实场景中常出现音频或视频信息缺失以及多语言说话人的情况，传统多模态系统在这些条件下性能下降，缺乏统一的评估标准。

Method: 提出POLY-SIM 2026挑战赛，建立标准化的多模态说话人识别任务设置，涵盖不完整模态输入和多语言场景，促进鲁棒系统的开发与比较。

Result: 为研究社区提供了一个新的基准平台，用于评估在模态缺失和多语言条件下的说话人识别系统性能。

Conclusion: 通过该挑战赛可推动更具鲁棒性和泛化能力的多模态说话人识别技术的发展。
...
TL;DR: WAVE-Stereo提出了一种新的立体匹配方法，通过融合相关体和特征扭曲的互补线索，在多个真实世界基准上实现了高精度和实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将相关体和特征扭曲分别建模，未能充分利用两者之间的互补性，导致在无纹理区域匹配性能下降。

Method: 提出GeoWarp Correspondence Encoder (GWCE) 并行编码匹配搜索、残差对齐和视差先验，并设计Periodic Global Context Propagation (PGCP) 周期性传播全局空间信息以缓解无纹理区域的匹配退化。

Result: 在Middlebury、ETH3D、KITTI 2012、KITTI 2015和Booster五个真实世界基准上取得优异表现，KITTI 2015上D1-all为3.18%，Booster上Bad-2.0为4.42%，推理时间仅66ms。

Conclusion: WAVE-Stereo通过统一建模两种匹配线索并引入全局上下文传播机制，在不依赖外部基础模型的情况下实现了精度与效率的良好平衡。
...
TL;DR: 本文提出了VSI-Super-Wild，一个用于评估真实世界多样化场景中长时间空间超感知的大规模基准，揭示了现有模型在跨时间连贯追踪世界状态方面的根本缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的空间超感知基准多依赖合成的、短片段拼接的视频，且局限于家庭场景，无法充分反映现实世界的时间连续性和场景多样性。受人类认知结构启发，需构建更贴近真实世界的评测基准以推动模型发展。

Method: 构建了一个包含442个真实世界长视频（最长超4小时）、覆盖8类场景的大规模数据集VSI-Super-Wild，并设计了6,980个人工验证的问题-答案对，系统性地评估模型对代理、物体和环境三要素的世界状态理解能力。

Result: 实验表明，尽管模型在静态图像理解上有进展，但在需要长时间连贯追踪世界状态的任务上表现不佳；性能随世界状态复杂度和时间跨度增加而下降，并总结出四种失败模式：空间坍缩、语义捷径、更新不足和实例混淆。

Conclusion: 当前多模态模型缺乏将代理、物体与环境绑定为统一空间世界模型的机制，这一根本缺陷指明了空间超感知未来的发展方向。
...
TL;DR: 本文提出了一种基于变分密度后验的辐射高斯点阵方法，实现了稀疏视角CT重建中精确传播的逐体素不确定性估计，并通过系统校准实验验证了其在多种场景下的有效性与优越性。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏视角CT重建方法（如辐射高斯点阵）虽快，但仅输出确定性结果，缺乏对重建可信度的评估；本文旨在引入可量化、可校准的不确定性估计，以提升临床或诊断中的可靠性判断。

Method: 利用透射X射线成像中投影与体素化对高斯密度的线性特性，构建具有变分密度后验的辐射高斯模型，推导出在体积空间和投影空间中闭式精确传播预测方差的方法（σ²(x) = Σgᵢ(x)²sᵢ² 和 Var[Iₚ] = Σwᵢ,ₚ²sᵢ²），并进行温度缩放下的系统校准（Spearman/AUSE/ECE）。

Result: 在官方基准15个场景中的14个上，该方法的逐体素不确定性与真实重建误差的排序相关性表现优异；其中9个在单次温度缩放后即满足幅值校准目标；相比之下，当前工作的扰动集成启发法在校准协议下表现极差（排序相关性低至-0.08）。研究还识别出三种采样状态（各向同性、病态、各向异性），并表明不确定性引导在各向异性状态下具有明显优势。

Conclusion: 本文首次实现了高斯点阵CT中的系统性不确定性校准，证明了其在视图选择和潜在剂量自适应停止策略中的价值，为可信医学成像提供了新工具。
...
TL;DR: 本文提出了一种名为DNA的双阶段被动归因框架，用于图像生成模型的家族内变体溯源，无需额外训练即可实现高精度归因，并构建了包含24个生成模型的基准数据集DNA-30K进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有主动方法依赖水印或模型修改，影响视觉质量和部署灵活性；被动方法依赖大规模监督训练或单一重建信号，难以应对未知源和区分高度相似的家族内变体。

Method: 提出Dual-stage Native Attribution (DNA)，采用粗到细的两阶段框架：第一阶段使用Autoencoder Double-Reconstruction (AEDR) 进行开放集家族级筛选；第二阶段通过Native Prediction Consistency (NPC) 在多个噪声水平下比较家族内变体的原生预测误差，结合语义条件与校准得分实现闭集模型级归因。

Result: 在新构建的DNA-30K基准上，DNA实现了89.11%的端到端归因准确率（随机猜测低于1%），相比最强基线提升33.81%，即使仅用AEDR作为粗粒度阶段仍表现优越。

Conclusion: DNA通过利用生成模型中分层的归因信号，实现了无需训练、高效且精确的图像来源归因，尤其擅长区分高度相似的家族内生成模型变体。
...
TL;DR: 提出了一种紧凑的卷积网络用于46类DHCD天城文识别，参数量仅为1.11M，准确率达到99.73%，为当前最高且模型大小缩小15.6倍。实验表明性能已接近饱和，小模型在无知识蒸馏情况下即可匹敌大模型，且在跨数据集和鲁棒性测试中表现更优。


<details>
  <summary>Details</summary>
Motivation: 旨在设计一个轻量级但高性能的卷积网络用于天城文字符识别，以突破现有模型在效率与准确率之间的权衡，并探索识别任务的性能上限。

Method: 构建了一个仅含111万参数的紧凑卷积网络，在DHCD数据集上进行训练；通过McNemar检验评估不同模型间的统计显著性差异，并测试包括教师模型在内的多种配置；同时在CMATERdb数据集上评估零样本和微调性能，以及模型在输入污染下的鲁棒性。

Result: 在DHCD数据集上达到99.73%的准确率，错误数稳定在11例的内在下限，所有模型（包括大型集成模型）均无法显著超越；小模型与大模型（1732万参数）相比无统计学差异（p=0.345）；零样本迁移至CMATERdb数字识别达76.6%，微调后达97.8%；在污染数据上的平均准确率为75.7%，远高于大模型的38.7%。

Conclusion: 所提出的紧凑卷积网络在天城文识别任务中达到了当前最优性能，且模型规模显著缩小；实验揭示该任务可能已逼近其内在性能极限，小模型无需知识蒸馏即可媲美大模型，并展现出更强的泛化能力和鲁棒性，挑战了‘更大即更好’的传统认知。
...
TL;DR: M$^\text{4}$World是一个多视角、多模态生成式驾驶世界模型，支持细粒度对象操控和稳定的分钟级视频与LiDAR流生成，通过多阶段训练和条件控制实现高可控性与长时序一致性，适用于可扩展的自动驾驶仿真。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界生成方法在对象级可控性和长时序稳定性方面存在不足，难以满足自动驾驶仿真中对精细控制和长时间连贯模拟的需求。

Method: 提出M$^\text{4}$World，采用多阶段训练框架实现仅需四步去噪的在线因果生成；设计灵活的条件接口以支持对单个对象空间布局和外观的显式控制；引入少片段后训练和视觉参考条件生成模型，并构建基于视觉语言模型（VLM）的自动化评估流水线评估场景可控性。

Result: 实现了分钟级稳定流式生成，精确的对象操控能力，以及跨视角一致性和高生成质量；在长尾场景增强和场景编辑等下游任务中表现良好。

Conclusion: M$^\text{4}$World在生成质量、可控性和长时稳定性之间取得了平衡，展现出在可控、可扩展的自动驾驶仿真中的巨大潜力。
...
TL;DR: 该研究发现，尽管深度视频模型在估计左心室射血分数（EF）时能实现解剖学上的空间可解释性，但它们在时间维度上无法准确定位关键的心动周期帧（如ES/ED帧），表明现有归因方法可能掩盖模型的时间盲区，提示需改进训练和评估方法以增强时间感知能力。


<details>
  <summary>Details</summary>
Motivation: 验证深度学习模型在心脏超声视频分析中的归因是否在空间和时间上都具有可解释性和可信度，特别是是否真正关注了决定EF的关键帧（ES/ED帧）。

Method: 基于EchoNet-Dynamic数据集微调两种EF回归器（VideoMAE变压器和R(2+1)D CNN），并使用与架构匹配的归因方法（如Chefer、Grad-CAM）从三个维度进行审计：与左心室掩码的交集-相关性（IoR）、删除AUC、以及对ES/ED帧的时间定位指数，并通过tubelet遮蔽实验探究模型行为。

Result: 两种模型在空间上具有可解释性（IoR显著高于随机水平），但在时间上无法区分关键帧（ES/ED），时间定位能力与随机无异；遮蔽实验证实模型本身并未优先依赖关键帧，说明时间盲区是模型行为而非归因误差。

Conclusion: 空间可信不等于时间可信，当前归因方法可能误导对模型可靠性的判断，需发展更具时间意识的训练与评估策略，警惕仅依赖事后归因验证诊断模型的风险。
...
TL;DR: 本文提出了EgoProceVQA任务和EgoProceGen数据生成平台，用于评估和提升多模态大语言模型在自我中心视频中的程序性理解能力，并设计了EgoProceAgent框架以实现无监督的技能探索，显著提升了关键步骤级推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心视频理解方法较少关注程序性理解，尤其缺乏对关键步骤级别的复杂推理评估，限制了可穿戴设备上 procedural AI 助手的发展。

Method: 提出EgoProceVQA任务，包含六种关键步骤导向的问题类型；构建EgoProceGen数据生成平台并建立含3600个问题的基准；设计通用工具库与标准化子技能库，开发EgoProceAgent实现自探索的代理框架。

Result: 现有MLLMs在EgoProceVQA上表现不佳，显示程序性理解仍有较大提升空间；EgoProceAgent在多个任务上达到开源模型中的最先进性能。

Conclusion: EgoProceVQA基准、EgoProceGen平台与EgoProceAgent框架共同为自我中心程序性理解提供了统一基础，推动面向实际应用的 procedural AI 助手发展。
...
TL;DR: 该研究评估了多种模型在胸部X光片数据集上结合临床指征和影像的性能，发现临床指征与报告衍生目标有强关联，且后置报告文本会导致显著的标签循环问题。


<details>
  <summary>Details</summary>
Motivation: 解决胸部X光片分析中不同阶段输入（如临床指征、影像）融合的问题，并避免因使用后续生成的报告内容作为标签而导致的数据泄露和模型偏差。

Method: 使用15,000个ReXGradient-160K研究样本，比较仅影像、仅临床指征、固定顺序多模态、随机交换、DeepSets和SectionGuard-MI等模型；采用冻结的DenseNet-121和Bio+ClinicalBERT编码器，通过五次随机种子训练及2000次患者簇自举法估计不确定性。

Result: 普通两图像加临床指征融合模型AUROC为0.780，SectionGuard-MI达到0.783 AUROC和0.260 AUPRC；DeepSets前瞻性AUROC最高（0.787），随机交换融合AUPRC最高（0.265）且校准更好；完整报告文本单独预测时AUROC达0.979，提示存在严重报告-标签循环问题。

Conclusion: 临床指征对预测具有重要价值，排列感知的融合方法表现良好，但使用后续生成的报告内容会导致严重的数据泄露，应在前瞻性建模中避免。
...
TL;DR: 本文提出了一种基于RGB-事件相机的渐进式视频去雨框架RainDancer，采用“先分解后交互”的范式，在模态内部分离雨和背景成分后再进行跨模态融合，有效提升了去雨效果与感知鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于RGB视频的去雨方法在动态场景中存在歧义，难以区分雨滴、纹理、运动等相似模式；而事件相机虽提供高时序分辨率的运动信息，但直接融合易受噪声和背景响应干扰。因此需要一种能有效分离雨与背景并合理融合双模态信息的方法。

Method: 提出RainDancer框架：在RGB分支中逐步分解帧特征为雨和背景表示；在事件分支中使用面向降雨的脉冲神经网络捕捉与雨相关的稀疏突发事件动态；在成分级对语义对齐的表示进行融合，并引入事件域监督来规范稀疏事件重建、结构一致性和梯度方向。

Result: 在合成和真实RGB-事件视频去雨数据集上实验表明，该方法在定量指标、视觉质量和下游感知任务鲁棒性方面均优于现有方法。

Conclusion: RainDancer通过先分解后交互的跨模态融合策略，有效解决了动态雨天场景中的去雨难题，显著提升了恢复图像质量与感知可靠性。
...
TL;DR: 提出AspectCLIP框架，通过基于文本相似性的属性聚类和簇内一致性正则化，解决图文对比学习中的信息不对称问题，提升表示空间的结构性和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有图文对比学习方法使用全局一致性正则化，忽视了图像与文本间的信息不对称性：同一图像可对应描述不同方面的文本，导致语义失真。

Method: 将训练样本按文本相似性划分为属性簇，在每个簇内应用完整的循环一致性损失，跨簇正则化则限制为原型级比较，实现面向特定方面的正则化。

Result: 实验表明，AspectCLIP在多个下游任务上持续优于传统方法，并学习到更结构化的表示空间。

Conclusion: 通过尊重图文一对多结构的方面感知正则化，能有效缓解语义扭曲，提升跨模态表示学习质量。
...
TL;DR: 提出了一种解耦辐射表示方法，通过2D surfels和紧凑纹理图集实现高效、稀疏的3D高斯点阵渲染，在保持高质量视觉效果的同时显著提升速度，实现实时4K渲染。


<details>
  <summary>Details</summary>
Motivation: 现有基于哈希网格的3D高斯点阵渲染虽能捕捉精细颜色细节，但片段渲染计算成本高，需提高效率与稀疏性。

Method: 采用解耦辐射表示，用2D surfels建模低频几何和视角相关外观特征，用视图无关的空间哈希网格表示高频纹理并烘焙到紧凑纹理图集，结合增强稀疏性的优化策略，剔除不重要的surfels。

Result: 相比之前的方法，重建更快速且更稀疏，最高可达3DGS五倍的速度提升，并在消费级硬件上以60 FPS实现4K实时渲染，同时保持最先进的视觉保真度。

Conclusion: 该方法通过解耦表示和稀疏优化，在保证视觉质量的前提下大幅提升了渲染效率和稀疏性，推动了3D高斯点阵技术向实时应用的发展。
...
TL;DR: 提出了一种全自动多模态深度学习框架，结合3D增强CT和临床信息，准确分类胰腺癌的可切除性。


<details>
  <summary>Details</summary>
Motivation: 由于专家在评估胰腺肿瘤与周围血管关系时存在较大差异，需要一种更准确、一致的方法来判断PDAC的可切除性。

Method: 采用Swin-UNETR骨干网络进行胰腺、肿瘤和血管结构的分割，并提取17个常规临床变量形成紧凑的临床嵌入，与图像特征融合后通过轻量级分类头进行三类可切除性分类。训练过程中使用动态多任务目标函数，根据肿瘤分割性能自适应调整分割与分类任务的权重。

Result: 该模型能够生成既具有解剖学感知又具判别性的特征表示，在NCCN三类可切除性分类任务中表现出色。

Conclusion: 所提出的框架有望提高胰腺癌可切除性判断的一致性和准确性，具有临床应用潜力。
...
TL;DR: 提出了一种基于切片级数据合成范式的大规模结构化推理数据集，用于提升多模态大语言模型在3D医学图像理解中的性能，无需昂贵的3D预训练即可实现强空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的2D多模态大语言模型难以有效扩展到3D医学影像理解，主要受限于标注成本高、数据集不透明以及缺乏显式的临床推理过程。

Method: 受放射科医生真实诊断流程启发，提出一种新的切片级数据合成范式，将全局临床先验转化为细粒度的逐切片观察，并构建可解释的思维链（Chain-of-Thought, CoT），在训练中引入序列空间追踪、多切片空间感知和排除性鉴别等临床原则。使用该合成数据对标准2D预训练MLLM进行指令微调。

Result: 在多个3D医学影像基准测试中，该方法显著优于2D基线模型，展现出强大的空间推理能力，并接近计算成本更高的原生3D架构性能。

Conclusion: 该数据驱动策略无需昂贵的3D专用预训练，即可实现深度的容积理解与高度可解释的临床逻辑，有效弥合了2D模型与3D任务之间的性能差距。
...
TL;DR: 本文提出了一种无需训练的智能体框架AgentHOI，利用基础模型的多模态推理能力实现开放世界中的人-物交互检测，通过上下文感知的多轮推理和多维度交互定位机制，在无需HOID数据训练的情况下超越现有监督和弱监督方法。


<details>
  <summary>Details</summary>
Motivation: 传统人-物交互检测依赖预定义类别和特定数据集监督，难以泛化到开放世界和组合场景；现有基于提示的方法未能充分挖掘多模态大模型的推理能力。

Method: 提出AgentHOI框架，模块化协调多个视觉基础模型进行开放式语义推理与空间定位；引入上下文感知的多轮推理机制以逐步优化交互假设，并采用多维度交互定位机制结合语义、空间和外观线索生成实例描述以提升定位精度。

Result: 在真实场景实验中，AgentHOI在无需任何HOID训练数据的情况下，性能优于现有的全监督和弱监督方法，尤其在复杂场景下的交互发现与定位方面表现更优。

Conclusion: AgentHOI成功将通用多模态推理能力迁移到人-物交互检测任务中，实现了训练免费、高泛化性的开放世界HOI检测，为未来开放环境感知提供了新思路。
...
TL;DR: 本文提出了一种结构感知的音乐驱动舞蹈生成框架，通过将舞蹈建模为原子运动序列，提升了生成舞蹈的结构性、节奏对齐性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有舞蹈生成方法通常将动作建模为连续信号，忽略了舞蹈的组成性，导致生成结果结构混乱且难以控制。

Method: 首先对大规模舞蹈数据进行分割和聚类，构建原子运动词汇；利用大语言模型对聚类结果进行语义重标注；设计两阶段生成框架：第一阶段根据音乐预测原子运动的类型、时长和时机，形成符号化编排；第二阶段通过过渡感知生成器合成流畅动作。

Result: 实验表明，该方法在结构连贯性、节奏对齐和感知自然度方面优于现有方法，并支持基于结构表示的可解释编辑。

Conclusion: 通过引入语义可解释的原子运动表示和两阶段生成流程，本方法实现了更可控、结构更清晰的音乐驱动舞蹈生成。
...
TL;DR: 本文提出了一种名为OCP-CT的器官条件模式对齐框架，用于CT视觉-语言预训练，通过细粒度的模式级对齐提升零样本异常诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在CT图像与放射学报告的对齐中采用全局对比或粗略的器官级对齐，难以捕捉同一器官不同影像表现的异质性，因此需要更精细的对齐单元。

Method: 提出OCP-CT框架，保持全局对比分支的同时引入器官模式接口：使用稀疏Mixture-of-Experts（MoE）根据潜在放射学模式路由图像和文本token，可学习槽位查询路由后的token生成连续模式token，并通过配对token对比结合报告导出的临床相似性构建的结构化软目标实现对齐。

Result: 在CT-RATE和RAD-ChestCT两个公开基准上，OCP-CT实现了84.5%和69.9%的平均AUROC，用于零样本异常诊断，相比之前最优结果分别提升了6.7和0.8个百分点。

Conclusion: OCP-CT通过引入器官条件下的放射学模式作为更细粒度的对齐单元，有效提升了CT视觉-语言预训练的表征能力，在零样本异常诊断任务中取得了显著性能提升。
...
TL;DR: 本文提出了一种多专家系统，通过迭代微调的检查点作为领域专家，并使用轻量级页面图像分类器按书写风格路由页面，有效提升了历史满文OCR在不同书写风格下的识别精度。


<details>
  <summary>Details</summary>
Motivation: 由于历史满文存在多种视觉差异大的书写风格（如楷书、行书、宫中奏折体），且标注数据有限，传统OCR方法难以兼顾各风格的识别性能，因此需要一种能自适应不同域的系统。

Method: 采用多专家系统框架，复用迭代微调过程中的多个检查点作为不同领域的专家，并设计一个轻量级页面级图像分类器作为路由器，将输入页面分配给最合适的专家处理；当现有检查点无法覆盖某领域时，训练新的专家。

Result: 在三个固定测试集上，该路由系统对每种风格的识别错误率（CER）分别达到0.30%（楷书）、1.57%（奏折体）、4.83%（行书），路由器页面级领域准确率达99.3%，与领域标签预言机精度相当。值得注意的是，三个被选中的专家中有两个并未在其最终服务的领域上专门训练过。

Conclusion: 该多专家路由系统能高效利用已有模型资源，在低资源条件下实现对多种满文书写风格的高精度识别，具备良好的可扩展性与实际应用价值。
...
TL;DR: The 2nd International StepUP Competition advanced footstep biometrics using the large StepUP-P150 dataset, focusing on user generalization, robustness to footwear and speed variations, and left-right footstep fusion, achieving a best 8.00% EER.


<details>
  <summary>Details</summary>
Motivation: To promote research in pressure-based footstep biometrics under realistic and challenging conditions, particularly addressing generalization, domain shift, and information fusion.

Method: Utilized the StepUP-P150 dataset and a hidden test set; evaluated participants on three challenges: unseen user verification with limited data, cross-domain robustness (footwear/speed), and left-right footstep fusion using stride-level analysis.

Result: 26 teams participated; best result was 8.00% equal error rate by ArogyaPandit Research Team using spatiotemporal CNN and ensemble scoring; top methods leveraged temporal patterns and inference-time normalization.

Conclusion: While significant progress was made in footstep biometrics, recognizing users in unseen personal footwear remains challenging, especially with similar distractors, highlighting the need for better generalization and calibration techniques.
...
TL;DR: 本文提出TCA-Net，通过阈值化跨流注意力机制实现低光照图像增强中亮度与色度的可靠融合，结合结构感知初始化和去耦引导模块，在噪声抑制、色彩保真和效率之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于HVI的方法虽解耦亮度与色度以缓解颜色纠缠，但固定比例的稀疏注意力机制无法适应不同层的置信度变化，导致信息丢失或噪声保留，影响融合质量。

Method: 提出TCA-Net，核心是阈值化跨注意力（TCA），用固定置信度阈值替代Top-K选择，实现输入和层自适应的高置信度交互保留；引入相位引导傅里叶交互模块提供结构感知的亮度初始化，并设计去耦双流引导模块抑制色度泄漏；采用尺度感知一致性正则化提升训练鲁棒性。

Result: 在LOL-v1、LOL-v2、Sony-Total-Dark和LSRW-Huawei数据集上实验表明，TCA-Net在恢复精度、色彩保真度方面表现优异，且模型参数更少，效率更高。

Conclusion: TCA-Net通过自适应阈值化的跨流注意力机制，有效提升了低光图像增强中亮度与色度融合的可靠性，在多个标准数据集上实现了更优的综合性能。
...
TL;DR: 本文提出了一种基于潜在扩散模型的统一天气编辑框架Cyclone，利用循环一致性约束和图像-文本模型的知识，在无需配对数据的情况下生成多种天气条件下的逼真图像，提升了自动驾驶感知任务的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的恶劣天气合成或去天气方法依赖于成对数据或特定物理模型，难以泛化且生成效果不真实，因此需要一种更灵活、无需配对数据且能适应多场景的天气编辑方法。

Method: 提出Cyclone框架，结合潜在扩散模型、循环一致性约束以及图像-文本先验知识，实现无配对数据下的多类型天气编辑，并可蒸馏为视频扩散模型以实现时序一致的天气变换。

Result: 实验表明，Cyclone生成的图像更加逼真且保持结构完整性，在多个自动驾驶感知任务中带来性能提升，并支持视频级别的天气编辑。

Conclusion: Cyclone是一种无需配对数据、高度通用的天气编辑框架，能够有效增强自动驾驶系统在多样化天气条件下的感知鲁棒性。
...
TL;DR: SIVA-RL是一种基于样本结果的视觉对齐框架，通过局部干预和奖励变化动态分配监督信号，在多模态推理任务中显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉干预方法依赖干预类型而非实际效果进行监督，导致相同操作在不同样本上产生异质结果时失效，无法确保模型真正基于视觉证据推理。

Method: 提出SIVA-RL框架：使用token对齐、距离约束的图像内PatchSwap生成局部干预；利用冻结的审计策略评估原始与干预图像对的奖励差异；根据实际奖励下降程度作为软路由权重，分别驱动敏感性对齐和不变性对齐，实现样本级监督。

Result: 在九个涵盖数学、逻辑和视觉依赖任务的多模态推理基准上，SIVA-RL在3B和7B模型上均优于基线；在视觉依赖推理上提升8.79个百分点，四种配置下整体相对提升最高达14.9%。

Conclusion: SIVA-RL通过解耦干预构建与监督分配，实现了更精准的视觉对齐，有效提升了多模态推理中模型对视觉证据的依赖性和推理准确性。
...
TL;DR: 本研究提出了一种基于PET/MRI多参数定量分析的两步聚类方法，用于表征致心律失常性左心室心肌病的心肌异质性，并生成自动化的健康报告，结果表明该方法能准确识别医生观察到的异常区域。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏金标准诊断标准，致心律失常性左心室心肌病难以诊断，需要一种系统化的方法来整合多模态影像数据以提高诊断准确性。

Method: 对99名患者的T1、T2图、LGE和18F-FDG-PET图像进行独立z-score标准化并合并为单个体积，使用超体素分割和谱聚类获得32个跨患者超体素组，为每个簇和模态分配“异常”评分，并生成自动化文本和靶心图健康报告。

Result: 在重复嵌套交叉验证中，该方法生成的报告与心脏影像专家评估的一致性较高（BA = 0.76 ± 0.04），在167个数字 phantom 验证中表现更优（BA ≥ 0.8），且识别出的异常簇与视觉观察高度一致。

Conclusion: 该方法可系统化处理多模态PET/MRI数据，有助于识别不同程度的心肌纤维化或炎症，提升对该疾病表型和进展的理解。
...
TL;DR: 本文提出了一种基于心理学“峰终定律”的视频美学评估轻量级框架Peak-End-Net，通过引入图像美学先验和美学节奏编码器，结合动态门控融合机制，在少量可训练参数下实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 视频美学评估（VAA）研究受限于大规模数据集的缺乏以及审美判断的主观性，本文从心理学视角重新审视该问题，旨在利用人类感知规律提升模型性能。

Method: 提出Peak-End-Net，利用预训练的图像美学评估模块生成帧级美学先验，识别显著时刻；设计美学节奏编码器建模时间演变过程；采用动态门控融合机制增强分布偏移下的鲁棒性；整体基于冻结的ViT构建，参数高效。

Result: 在VADB和DIVIDE-3K两个基准上进行了实验，结果表明该方法在域内和跨域设置下均达到最先进水平。

Conclusion: 基于心理学原理（如峰终定律）建模有助于提升视频美学评估性能，所提出的Peak-End-Net兼具高性能、高解释性和参数效率，为VAA提供了新思路。
...
TL;DR: 该研究利用63个EMIT-derived Carbon Mapper甲烷羽流记录，揭示了当前发布的标量参数（如IME、羽流长度和排放率）无法唯一确定羽流边界，存在多种合理但不同的掩膜结果。通过遗传算法集合和PlumeQuant工具进行分析，发现高置信度核心仅覆盖13%的可能羽流范围，尤其在弱信号、低重叠的羽流中不确定性更大。研究提出了一种无需参考掩膜即可生成的CM-like掩膜方法，在固定参数下实现了对标量量的高精度复现，并提供了产品一致性诊断，用于识别需专家审查的模糊或不可靠羽流。


<details>
  <summary>Details</summary>
Motivation: 现有的甲烷羽流产品虽提供关键物理量，但其标量输出可能对应多个不同的空间掩膜，导致对羽流边界的解释不唯一，影响数据可靠性和应用准确性，亟需评估其内在一致性与不确定性。

Method: 采用遗传算法（GA）集合，在给定发布IMF和羽流长度条件下生成多组符合条件的掩膜，揭示解的等效性；开发PlumeQuant工具，基于分发的产品组件重新计算IME、羽流长度、排放率及五项不确定性，并评估四种掩膜表示：发布参考掩膜、CM-like掩膜、GA集合和专家编辑掩膜。CM-like掩膜在无访问参考信息的情况下逐羽流生成，参数在44个独立场景上一次性固定。

Result: GA集合显示高置信核心仅覆盖13%的可能羽流区域，表明边界高度不确定，尤其在弱信号羽流中更显著；CM-like掩膜复现发布IME中位偏差+0.72%，排放率+0.16%（平均绝对偏差6.98%），与参考掩膜中位IoU达0.843，不确定性比例中位数为1.01；保留样本的平均绝对误差为7.6%（IME）、9.5%（长度）、6.1%（速率）。

Conclusion: 当前发布的甲烷羽流标量产品不能唯一约束羽流边界，存在显著的空间解释歧义。PlumeQuant提供的产品级一致性诊断可有效识别弱、偏移或模糊的羽流，建议结合专家审查以提高可靠性。
...
TL;DR: 本文提出了一种名为CF-Net的深度多模态网络，用于检测视频中的矛盾与犹豫情绪，该网络通过冲突融合模块显式建模跨模态不一致性，并结合多种训练策略，在BAH数据集上取得了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 由于矛盾与犹豫情绪在视频中表现为模糊且非典型的跨模态不一致，传统方法难以有效识别，因此需要设计能够捕捉细微模态间差异的模型。

Method: CF-Net采用冻结的SigLIP2、HuBERT和DistilBERT作为视觉、音频和文本流的骨干网络，对每说话者特征进行归一化以减少身份泄露，并通过ConflictFusion模块融合多模态信息；训练时使用确定性加权焦点损失、流形混合增强、模态丢弃以及辅助的确定性回归头来利用模糊性标注。

Result: CF-Net在BAH验证集上达到0.7155的Macro F1分数，在私有挑战测试集上达到0.7364（AP为0.7492）的成绩。

Conclusion: CF-Net通过显式建模跨模态不一致性和引入针对模糊样本的训练策略，有效提升了视频中矛盾与犹豫情绪的检测性能，验证了对细粒度情感理解任务中模态冲突建模的重要性。
...
TL;DR: 提出VisionScreen模型，将语言建模中的Screening机制扩展到视觉识别领域，通过在二维空间域中独立评估图像块的相关性，有效过滤无关背景或冗余信息，提升图像分类性能。


<details>
  <summary>Details</summary>
Motivation: ViT中的自注意力机制使用softmax归一化权重，难以独立评估图像块之间的相关性，且无法显式排除背景或冗余块，影响特征聚合效果。

Method: 引入Screening机制，基于查询-键相似度在二维空间网格上进行绝对相关性估计，并设定阈值过滤低相关性图像块，实现内容和空间上相关块的选择性聚合。

Result: 在图像分类基准上的实验表明，VisionScreen优于传统ViT模型，验证了该方法的有效性。

Conclusion: Screening机制可有效应用于视觉识别任务，提供了一种替代softmax注意力的特征聚合方式，具有更强的相关性筛选能力。
...
TL;DR: 本文研究了针对ABAW11多任务情感行为分析的自适应特征融合方法，使用冻结的预训练视觉骨干网络提取帧级特征，并比较多种融合与预测策略，最终采用任务特定的方法在验证集上取得了良好表现。


<details>
  <summary>Details</summary>
Motivation: 不同情感分析任务（如效价-唤醒度、表情类别和面部动作单元）虽相关，但对特征和处理方式的需求不同，需探索更灵活的任务自适应融合策略。

Method: 基于DINOv2 ViT-L和DINOv3 ConvNeXt-base两个预训练模型提取互补帧级特征，冻结后比较多种预测头、时间建模方法、融合机制及校准策略，选择任务特定的最优组合。

Result: 在ABAW11验证集上达到EXPR宏F1为0.4222，AU宏F1为0.5402，VA平均CCC为0.6717，总分为1.6341。

Conclusion: 任务自适应的冻结特征融合是一种简单且有效的多任务情感行为分析策略。
...
TL;DR: 本文探讨了构建可交互游戏世界的关键维度，包括玩家动作控制、游戏状态动态、状态-观测持久性以及实时交互生成，并提出了一个用于《黑神话：悟空》的可扩展数据引擎，以推动基于状态感知的游戏世界建模研究。


<details>
  <summary>Details</summary>
Motivation: 实现真正可交互的游戏世界需要长期一致性、规则化的行为结果和实时生成能力，而现有生成模型在这些方面仍存在不足，因此需要系统性分析当前方法的能力与局限。

Method: 以传统游戏引擎的循环结构（动作-状态-观测）为框架，从四个维度对现有交互式游戏世界建模方法进行分类和比较，并发布了一个包含90多小时带标注 gameplay 数据的数据引擎。

Result: 对现有方法进行了系统性梳理，划分出代表性方法家族并分析其优劣；同时发布了高质量、多模态的《黑神话：悟空》游戏数据集，支持状态感知的生成模型研究。

Conclusion: 通过结构化的分析视角和高质量数据资源的提供，本文为构建下一代基于生成模型的交互式游戏世界提供了理论基础和技术路径。
...
TL;DR: VideoRAE是一种新型表示自编码器，利用冻结视频基础模型的多尺度特征，通过轻量级投影器生成紧凑且适合生成的视频潜在表示，在连续和离散生成任务中均实现优异重建效果和更快收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有3D-VAE依赖像素级重建，限制了语义和时空结构的学习；尽管视频基础模型（VFMs）具备强理解能力，但其冻结表示是否可用于生成友好型视频潜在空间尚不明确。

Method: 提出VideoRAE，使用冻结视频基础模型的多尺度分层特征，结合轻量1D自注意力投影器进行压缩；支持连续潜在变量（用于Diffusion Transformers）和离散token（用于自回归模型），并通过多码本高维量化实现；解码时采用局部与全局表示对齐目标，无需KL正则化。

Result: 在UCF-101上，VideoRAE结合AR和DiT生成器分别达到40和93的SOTA class-to-video gFVD分数，收敛速度比基线快约5倍；在2B规模文本到视频实验中，替代LTX-VAE后仍保持可比性能且收敛更快。

Conclusion: 验证了冻结视频基础模型的表示可作为通用、紧凑且生成友好的视频潜在空间，为视频生成提供了新方向。
...
TL;DR: FixItFlow 是一个利用大语言模型从历史事件数据中自动生成故障排除指南的系统，旨在提升云服务故障响应效率并减轻工程师文档负担。


<details>
  <summary>Details</summary>
Motivation: 手动创建故障排除指南费时费力，导致覆盖率不足和文档过时，难以满足云服务频繁 incident 的快速诊断需求。

Method: FixItFlow 利用大语言模型从历史 incident 数据中提取工程师操作的诊断模式，合成包含已验证命令的结构化指南，并通过严格验证防止生成虚假内容。

Result: 在26名工程师的评估中，生成的指南获得61.5%的清晰度正面评价，并使有对应指南的 incident 缓解时间减少2.3倍。

Conclusion: 自动化生成故障排除指南可有效提升 incident 响应速度，同时显著降低工程团队的文档维护负担。
...
TL;DR: Safe-Psych是一个用于评估大语言模型在临床精神病学中处理诊断不确定性能力的序列基准，揭示了现有模型在信息不完整时难以准确判断是否需要澄清或 abstain 的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的医疗基准通常假设信息完整，无法反映临床信息逐步演化的真实场景，因此需要一个能评估模型在信息不全时行为安全性的新基准。

Method: 构建包含1000多个真实精神病临床记录的Safe-Psych基准，将病历分段模拟信息逐步披露，并在每个阶段标注精神科医生建议的操作（诊断、澄清或 abstain），评估多个先进LLM在全信息和序列设置下的表现。

Result: 实验显示，即使最先进的模型在信息不全时也普遍存在过早诊断的问题，超过60%的情况下未能适当 abstain；安全提示虽可减少过早诊断但导致过度 abstention；模型很少主动请求澄清。

Conclusion: 当前LLM在识别临床证据不足并决定是否需要进一步信息方面存在显著缺陷，Safe-Psych的发布旨在推动更安全的医疗AI研究。
...
TL;DR: 本文探讨了在消费级硬件上检测LLM生成的专利文本所面临的挑战，发现现有零样本检测器存在高误报率，提出基于语言复杂性特征的逻辑回归模型可显著改善检测效果。


<details>
  <summary>Details</summary>
Motivation: 由于EPO新规要求申请人对LLM辅助内容负责，且专利权利要求本身具有低困惑度、低突发性的特点，与LLM输出相似，导致难以准确识别AI生成文本，亟需适用于现实审查环境（如仅配备8GB VRAM消费级GPU）的有效检测方法。

Method: 在500份真实EPO H04类电信专利与500份LLM生成专利文本上，评估三种开源零样本检测器的表现，并测试不同提示策略、模型规模及架构（包括Qwen2.5-3B-Instruct、LoRA适配的Pythia-2.8B、Falcon-7B和GPT-J-6B）下的泛化能力；进一步构建七特征语言复杂性逻辑回归模型，在不使用似然信息的前提下实现高效检测。

Result: 所有零样本检测器在权利要求层面均表现出超过60%的误报率（Binoculars 78.3%，Fast-DetectGPT 61.3%，DetectGPT 80.5%），该问题在多种模型与技术条件下持续存在；基于语言复杂性的逻辑回归模型达到74.0%准确率与28.1%误报率，相较仅依赖困惑度的基线提升13个百分点。

Conclusion: 当前主流零样本检测方法在现实约束下对专利文本的AI生成内容检测效果有限，高误报率源于结构性缺陷而非模型容量不足；提出的语言复杂性特征模型在相同硬件预算下表现更优，为实际审查场景提供了可行替代方案。
...
TL;DR: 提出了一种基于数据驱动的简单方法CSSEL-P2P，用于实现无需架构修改的高效同步语音翻译，在保持较低延迟的同时显著提升流式翻译质量。


<details>
  <summary>Details</summary>
Motivation: 解决解码器-only大语言模型在同步语音翻译中因上下文有限和跨语言重排序导致的挑战，避免现有方法因显式读写策略在对话语音中因分段边界模糊而表现不稳定的问题。

Method: 采用固定长度块进行累积流式解码，结合基于回溯的已提交前缀机制，并使用教师标注的带有限等待的前缀到前缀（P2P）目标进行微调。

Result: 在自建的对话语音评估中，CSSEL-P2P相比基线CSSEL在相近延迟（+0.15秒平均滞后）下，流式翻译质量提升了+1.54 COMETKiwi。

Conclusion: 通过P2P监督可在不改变模型架构的情况下有效实现高质量的同步语音翻译，验证了数据驱动方法在SimulST中的潜力。
...
TL;DR: 本文首次系统性地应用了人格向量（persona vectors）来研究大语言模型在后训练阶段形成的行为组织，涵盖53种特质并跨四个行为领域进行分类。研究发现模型默认表现出有用、以任务为导向的行为，某些特质如夸张、幻觉和谄媚可通过引导放大，而“邪恶”等难以提取的特质可通过微调变体迁移向量恢复。人格向量更适合作为探测行为结构的工具而非单纯控制手段。


<details>
  <summary>Details</summary>
Motivation: 理解语言模型在后训练中形成的行为倾向，并揭示其哪些行为是自然表达、可引导或难以提取的，从而系统性探究模型内部行为组织结构。

Method: 构建包含53种特质的清单，覆盖四个行为领域，使用 persona vectors 在两个开源权重模型中标记每种特质的状态（自然、可引导但潜藏、不可控），并通过向量迁移探索难提取特质的恢复能力。

Result: 发现模型默认呈现有用且任务导向的行为，9个能动性特质均为自然表达；默认临床行为与专业心理学家判断高度一致；引导主要增强默认未包含的特质（如夸张、幻觉、谄媚）；任何包含默认特质的双特质组合不会被压缩，而两个可引导特质则可能；通过微调变体迁移向量可恢复如‘邪恶’等标准提取失败的特质，其拒绝行为出现在模型思维链中。

Conclusion: Persona vectors 不仅可用于控制模型行为，更重要的是作为探针揭示语言模型内部行为的组织结构，表明模型具有稳定默认行为模式，且不同特质间的交互存在不对称性。
...
TL;DR: 本文提出了Text2Sign，一种基于文本条件扩散模型的短片段手语视频生成方法，可在单个NVIDIA L4 GPU上高效运行，通过冻结视觉-语言编码器和分解时空注意力机制降低成本并保持动作连贯性，尽管在提示敏感性方面表现有限，且仅适用于低分辨率短片段，仍为研究提供了一个可行的基线方案。


<details>
  <summary>Details</summary>
Motivation: 手语是聋哑人群体的主要沟通方式，但现有的文本到手语视频生成模型因训练和推理成本高昂而受限，本文旨在构建一个低成本、高效的文本到手语扩散模型，推动该技术的可及性和研究进展。

Method: 采用冻结的视觉-语言文本编码器结合3D编解码器和分解的时空注意力机制，设计了一种轻量化的扩散模型；对比了纯卷积与Transformer风格的主干网络、预训练与任务特定文本编码器以及分解注意力与全注意力结构，在How2Sign数据集的独立签署人划分上进行训练与评估。

Result: 在验证集上最佳短程实验达到0.0648的损失，长程检查点降至0.00999；在紧凑测试集上，使用8步DDIM采样（引导尺度5.0）获得SSIM为0.2403±0.0238，PSNR为15.11±0.42 dB，时间一致性为1.0000±0.0000；生成32帧64×64视频耗时12.60秒（2.54帧/秒），峰值推理内存为3.12 GB；去噪审计显示文本提示敏感性较弱。

Conclusion: Text2Sign实现了在单GPU上高效生成短片段手语视频，验证了低资源条件下扩散模型应用于文本到手语生成的可行性，但模型对手语语言细节建模能力有限，缺乏专家语言学评估，目前应视为研究基线而非完整生产系统。
...
TL;DR: RAGthoven是一个基于多阶段大语言模型管道的多语言幽默生成系统，在SemEval-2026 Task 1 Subtask A中表现优异，与基线模型并列第一，但在不同语言中效果存在差异。


<details>
  <summary>Details</summary>
Motivation: 设计一个能够生成跨语言（英语、西班牙语、中文）幽默文本的系统，并探索复杂提示工程和代理架构对生成质量的影响。

Method: 将创意文本生成分解为多阶段LLM流水线（规划器、Best-of-N写作者、自反思模块、LLM裁判），结合检索增强生成（RAG）和计算幽默理论，并测试了ReAct式顺序工具调用和自主多分支编排两种代理变体。

Result: 在西班牙语中，RAGthoven优于基线模型（1182 vs. 1140 Elo分），而在英语和中文中略低于基线但统计上持平；两种代理变体未表现出优于非代理流水线的效果。

Conclusion: 一旦使用强大的前沿模型，复杂的多阶段提示工程和代理结构可能带来语言依赖性的收益递减。
...
TL;DR: 本文提出了一种无需重新训练的角色条件性KV缓存分配方法，通过抑制KEY标记来减少注意力机制中的噪声，在低缓存预算下显著缩小了H2O方法的性能差距，并在较高预算下达到甚至超过全缓存精度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于注意力的KV缓存压缩方法（如H2O）在处理结构密集型输入（如嵌套JSON）时会过度保留无实际内容的分隔符或KEY标记，导致信号噪声比恶化、准确率急剧下降，因此需要一种更有效的缓存保留策略。

Method: 提出一种无需重新训练的方法，基于SnapKV的窗口化得分，引入角色条件性缓存分配机制，通过一个可调超参数抑制KEY标记的保留；使用15MB线性角色探测器低成本标注token角色。

Result: 在低于20%的缓存预算下，新方法填补了H2O性能差距的63%-98%；在较高预算下，性能接近或超过完整缓存；小规模去噪效应在部分设置下接近显著水平；推理开销极低。

Conclusion: 通过角色感知的缓存分配可有效提升长上下文模型在结构化输入上的KV缓存效率，抑制非内容角色（尤其是KEY标记）能显著改善保留状态的信噪比，是一种简单且实用的优化方向。
...
TL;DR: 本研究提出了GSM-Plus-BN，首个用于评估孟加拉语数学推理能力的大规模扰动数据集，填补了低资源语言在AI数学推理评测中的空白。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理研究主要集中于英语等高资源语言，忽视了如孟加拉语这样使用广泛但资源匮乏的语言，导致AI发展在全球语言多样性背景下存在不平等。

Method: 基于英文GSM-Plus基准构建并人工校验的9000样本孟加拉语数学数据集GSM-Plus-BN，包含1000个原始问题和8000个扰动变体；在六种开源大模型上测试标准提示与思维链提示下的表现。

Result: GPT-OSS-20B在标准提示下种子题准确率达96.08%；更大模型如Llama-3.3-70B和GPT-OSS-120B在各类扰动中表现出更强鲁棒性；思维链提示显著提升多数模型性能，但所有模型在孟加拉语上的表现仍明显低于其在英语基准上的水平。

Conclusion: GSM-Plus-BN为孟加拉语数学推理提供了首个系统化评测基准，揭示了当前模型在低资源语言扰动文本中泛化能力的局限，强调了语言特异性评估的重要性，并为未来研究提供了基础资源。
...
TL;DR: 本文提出了一种名为Apaf的混合LLM-符号管道，用于将政策文本中的批判性话语分析操作化为定量的双极论证框架，能够识别和解析参与式承诺与管理控制之间的隐含张力。


<details>
  <summary>Details</summary>
Motivation: 现有计算方法难以捕捉政策话语中由框架调节的复杂论点关系，而纯大模型生成的摘要缺乏可解释性和专家可审查结构。

Method: 首先使用大语言模型对论点进行分类（审议型或管理型框架），然后通过基于LLM提取特征的确定性规则识别四种框架调节关系子类型：能动性削弱、议程转移、工具性支持和规范性支持。

Result: 在来自美国、英国、加拿大和澳大利亚的100份灾害风险减少政策子文档数据集上验证了该方法，结果表明生成的论点图准确、可解释且跨司法管辖区稳定。

Conclusion: Apaf框架有效结合了LLM的语义理解能力与符号系统的可解释性，为政策分析提供了兼具深度与透明度的新工具。
...
TL;DR: 本文提出了一种针对大规模数据库中模式检索（schema retrieval）的新方法，即将文本到SQL的基准数据集重构为表和列粒度的检索任务，并引入语料库自适应微调技术，通过合成查询和难负例挖掘显著提升检索性能，验证了该方法在不同模型规模下的有效性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在大规模数据库中，传统文本到SQL模型难以处理包含数千张表的模式，受限于上下文长度和检索精度，因此需要将模式链接中的表和列检索作为一个独立且关键的检索任务进行研究和评估。

Method: 将五个现有的文本到SQL数据集重新构建为表级和列级的检索任务，采用自然语言和代码两种文档表示方式；提出语料库自适应微调方法：从目标模式语料库中合成自然语言查询，挖掘粒度感知的难负样本，并对305M参数的嵌入模型进行对比学习微调，同时验证其在8B大模型上的适用性。

Result: 该方法将平均recall@10从60.4提升至75.6（nDCG@10从51.9升至68.0），使305M模型性能超越十倍以上的大模型；在8B先进模型上也将recall@10从77.8提升至78.4，达到当前最优水平；留一语料实验和泄露审计表明性能提升源于可迁移的检索能力而非过拟合。

Conclusion: 模式检索应被视为一个独立的重要任务，而基于标签自由、语料库自适应的微调是一种高效且骨干无关的方法，能够在企业级规模下有效部署轻量级高性能检索模型。
...
TL;DR: 本文提出了一种基于元学习的框架，用于在人类反馈强化学习和直接偏好优化中解决低资源语言对齐中的数据不足问题，通过利用其他语言的偏好数据实现跨语言知识迁移，在极低资源场景下显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 由于不同语言间人类偏好数据分布不均，低资源语言在大模型对齐中面临严重数据短缺问题，难以有效训练可靠的奖励模型或策略。

Method: 提出一种元学习框架，分别应用于元奖励建模和元策略优化两个阶段；通过多语言源数据学习可迁移的初始化参数，使模型能在仅有少量目标语言数据时快速适应。

Result: 在仅使用100个目标语言偏好样本的极端低资源设置下，相比基线方法最高获得28%的胜率提升，并在多种目标语言、模型规模及元训练语言组合下 consistently 表现更优。

Conclusion: 该方法能有效缓解低资源语言对齐中的数据稀缺问题，具备良好的跨语言迁移能力和实际应用潜力。
...
TL;DR: 研究探讨了LLM作为评判器在表格识别中的优化效用，发现其评分信号弱、排名不可重复，单纯依赖LLM评分无法有效指导迭代优化；结构保持约束可减少严重错误，但未带来整体提升，表明需确定性验证信号辅助迭代改进。


<details>
  <summary>Details</summary>
Motivation: 验证LLM作为评判器在闭环生成中提供反馈和选择信号的有效性，特别是在结构化任务（如表格识别）中的适用性。

Method: 在FinTabNet和OmniDocBench两个数据集上，利用确定性的TEDS评估作为控制实验环境，分析LLM评判信号的质量及其对迭代生成的指导作用，并引入结构保持指令进行对比实验。

Result: 1. LLM评判信号弱，排名常出现平局且不可复现，仅靠最早迭代轮次的平局规则才能略微优于随机选择；2. 无具体反馈时仍出现严重损失，结构保持指令显著降低FinTabNet上的严重损失率；3. 结构约束虽减少极端错误但未带来整体性能提升，且与评判反馈结合时保护效果不稳定。

Conclusion: LLM的评估能力不等于其在优化过程中的实用性，迭代 refinement 至少需要能确定检测结构变化的验证信号，而不仅仅依赖LLM打分。
...
TL;DR: 本文提出了一种基于小型透明BiLSTM-CRF模型的神经标注流水线，用于伊良部琉球语的形态切分、词性标注和词汇对齐，在仅有一小时完全标注语料的限制下进行评估。研究发现，添加词性标注层可显著提升词汇对齐准确性，尤其在数据稀缺时效果更明显；尽管当前自动词性标注器仍有误差，但其潜在价值明确，未来改进后有望实现性能增益。建议文献记录实践中采用四行线性标注法。


<details>
  <summary>Details</summary>
Motivation: 针对濒危语言记录时间有限的问题，亟需减少人工逐行标注话语数据所需的时间成本，通过自动化部分标注流程来提高语法描述效率，并探索在极小监督资源下的有效标注策略。

Method: 构建一个完整的神经标注流水线（包括形态切分、词性标注和词汇对齐），使用小型透明的BiLSTM-CRF模型，在约一小时完全标注的话语数据上训练并测试；系统地操控标注的丰富性（是否包含词性层）和数量（6到47分钟的训练预算），并在真实受限条件下评估性能。

Result: 加入金标准词性标注使词汇对齐准确率平均提升+4.4点（p<0.05），且在数据量减少时增益更大（四分之一数据时达+11.6点）；词性标注层可使达到相同准确率所需的标注数据减半；但在全自动流水线中，由于词性标注错误率仍达12%，未观察到实际增益；模拟实验显示当词性标注准确率接近88%时收益转正，92%-96%时可恢复+1.6至+3.2点。

Conclusion: 词性标注对低资源条件下的自动词汇对齐具有显著潜在价值；推荐在语言文档记录中采用四行线性标注格式（文本、词性、对齐、翻译），以兼顾当前实践与未来自动化提升空间。
...
TL;DR: GFlowRL提出了一种简化的生成流网络风格的强化学习算法，通过去除辅助的划分函数网络并使用批量蒙特卡洛估计来稳定训练，实现了在数学、代码和对抗性红队任务上的优越性能，并首次实现了在密集和稀疏架构间的稳定扩展。


<details>
  <summary>Details</summary>
Motivation: 现有的GFlowNet-style RL方法在大规模推理模型训练中面临梯度不稳定和工程开销大的问题，尤其是在模型规模、 rollout步数、奖励噪声和分布式系统复杂性增加的情况下，学习到的条件划分函数成为瓶颈。因此需要一种更高效且稳定的算法。

Method: 提出GFlowRL算法，用rollout组内计算的批量蒙特卡洛估计替代学习得到的划分函数；移除辅助的划分网络；引入重要性采样校正以缓解rollout与训练器之间的漂移，并采用非对称流间隙裁剪处理异常残差，从而保持奖励分布匹配目标。

Result: GFlowRL在数学、代码（Codeforces评分为2048，14B规模下接近o3-mini）和对抗性红队（AdvBench和HarmBench上ASR@1最高）基准测试中优于所有对比方法；在最多235B参数的MoE架构中均成功迁移，而先前的FlowRL在此类设置下无法收敛。

Conclusion: GFlowRL是首个能在密集和稀疏架构上稳定扩展的GFlowNet风格强化学习算法，有效解决了传统方法中的梯度不稳定和工程复杂性问题，具备良好的可扩展性和实际应用潜力。
...
TL;DR: 本文系统研究了在大语言模型后训练中的在线策略蒸馏（OPD）的训练动态，揭示其作为探索催化剂的作用及存在的两种病理问题，并提出轻量级信号调节方法以提升蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 尽管OPD已成为大语言模型后训练的关键范式，但其训练动态尚不明确，存在因教师-学生差异或长度偏差导致的路径偏离风险，亟需深入理解与调控机制。

Method: 通过分析OPD中token级指导信号的作用，识别出学生-教师分布差距引发的Student-Teacher Mismatch和长度相关奖励漏洞导致的Length Exploitation两种病理；提出采用advantage clipping和log-scale compression进行信号调节。

Result: 实验证明所提出的信号调节方法能有效缓解长度利用问题，在七个基准上稳定超越多种OPD变体和RLVR基线，验证了信号质量对探索过程的关键影响。

Conclusion: OPD的成功关键在于高质量、受控的指导信号，而非单纯依赖更大的教师模型；通过轻量调节即可实现更稳定有效的知识迁移与探索引导。
...
TL;DR: 本研究比较了在生物医学数据到文本生成任务中，基于Qwen的小型语言模型（SLMs）采用不同后训练方法（如SFT、DPO、ORPO和GRPO）的性能表现，结果表明GRPO在跨数据集任务中表现最优且优于GPT-5。


<details>
  <summary>Details</summary>
Motivation: 将复杂的生物医学数据转化为患者易懂的叙述是现代生物医学信息学的核心需求，但现有模型在专业领域文本生成的质量和泛化能力上仍有不足，因此需要探索更适合的训练方法。

Method: 采用监督微调（SFT）、直接偏好优化（DPO）、比值比偏好优化（ORPO）和分组相对策略优化（GRPO）等方法对基于Qwen的SLM进行后训练，并在药品说明书数据集和来自openFDA的药物标签数据上进行评估，使用ROUGE和语义相似度等多种指标进行分析。

Result: 实验结果显示：经过对齐训练的SLMs优于GPT-5；ORPO优于SFT基线；GRPO在所有对齐方法中表现出最强的跨数据集泛化能力，且整体性能超过GPT-5。

Conclusion: GRPO是一种在生物医学文本生成任务中具有优越性能和良好泛化能力的对齐方法，表明小型语言模型通过合适的训练策略可在特定领域超越大型商用模型。
...
TL;DR: 本研究提出了一种基于特征的中间表示和目标作文监督的细调框架，用于跨评分标准泛化，在自动作文评分中提升了对未见评分标准的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有自动作文评分研究主要关注跨题目的泛化，而实际教学中评分标准可能变化或更新，因此需要研究模型在不同评分标准下的泛化能力。

Method: 采用大语言模型细调框架，引入与评分标准无关的中间表示（称为“特征”），并在训练中使用已知评分标准下的目标作文监督。

Result: 在包含多个评分标准标注的学生批判性思维技能数据集上，该方法在最困难设置下（即目标评分标准和目标作文均未在训练中出现）比无特征的基线模型macro F1提升5.0%；增加目标作文监督可进一步提升性能，最佳开源Llama模型优于GPT-5-mini提示2.1%，略低于GPT-5 1.9%。

Conclusion: 基于特征的中间结构和可控监督有助于提升模型在未见评分标准下的泛化性能。
...
TL;DR: 本文提出了一个用于锡克教Kirtan实时字幕的基准和参考系统，强调了在宗教敏感背景下准确转录经文的重要性，并发布了相关数据集和工具以促进研究进展。


<details>
  <summary>Details</summary>
Motivation: 由于在展示中出现拼写错误的古木基文被视为宗教上不恰当，因此需要一种能够精确逐字转录Sri Guru Granth Sahib Ji经文的闭合词汇字幕系统。

Method: 将任务形式化为在每个时间点t预测(shabad_id, line_idx)或null，并根据实时与离线、盲测与先知两种正交轴线组织问题空间；开发了一个基于微调120M IndicConformer模型、模糊匹配器和状态机的参考系统。

Result: 发布的基准包括4个手工标注的Kirtan录音及其3种冷启动偏移，共12个评估案例，约57分钟的评分音频；参考系统在最困难的变体（实时x盲测）下实现了57.9%的整体帧准确率（12个案例中的10个正确锁定shabad）。

Conclusion: 该研究提供了一个新的基准和高效的参考系统，适用于锡克教Kirtan的实时字幕生成，并强调了标准ASR指标（如WER/CER）在此类特定任务中的局限性。
...
TL;DR: 本文提出了DevicesWorld，一个大规模可执行的跨设备协作操作基准，包含6140个任务，涵盖移动、桌面和物联网三类设备环境，用于评估基于大语言模型的智能体在异构设备间获取与整合信息及完成端到端任务的能力。实验表明现有智能体表现较差，最佳成功率仅为12.5%。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的用户目标通常跨越多个设备，而现有基准大多局限于单一执行环境，难以评估智能体在多设备间的协同操作能力。因此需要构建一个支持跨设备信息整合与任务执行的可验证评估框架。

Method: 提出DevicesWorld，集成移动、桌面和IoT三类设备环境，设计包含自然语言目标、参与设备、初始状态、可执行动作、规则验证器和清理流程的任务结构，并通过多阶段构建与质量控制流程确保任务贴近真实需求且结果可自动验证。

Result: 在五个前沿LLM智能体系统上的评估显示，所有方法成功率均较低，最高仅为12.5%；约28.7%的失败案例部分满足评分条件但未完全成功；轨迹分析表明智能体常卡在信息获取、界面操作、混淆输入输出设备或提前终止。

Conclusion: DevicesWorld将跨设备协作操作转化为一个可执行、可复现且具有诊断价值的评估问题，为开发可靠跨设备智能体提供了重要工具和挑战方向。
...
TL;DR: MyAG是一个基于图的框架，用于设计和分析可组合的LLM代理系统，支持组件、工作流和搜索图的分离及层次化组合。


<details>
  <summary>Details</summary>
Motivation: 为了提高LLM代理系统的可重用性和灵活性，需要一种能够解耦组件、控制流和运行时搜索的框架。

Method: 提出MyAG框架，包含三个图抽象：组件图（代理、环境、模块）、工作流图（执行控制）和搜索图（运行时执行），并支持递归系统节点的层次化组合以及监控与可视化工具。

Result: 实验表明该框架能灵活支持代表性代理应用的设计，并有助于分析性能与效率之间的权衡。

Conclusion: MyAG提供了一种开放、灵活且可分析的方案来构建复杂的LLM代理系统，促进组件复用与系统调试。
...
TL;DR: 本文介绍了BioASQ Task 14B 2026系统的构建，重点在于检索策略优化与多语言模型答案融合方法。通过结合混合首阶段检索与代理驱动的分解检索，并引入选择性重检索机制，在降低成本的同时提升了性能。同时提出一种分解分析框架解释多模型集成中选择与融合的作用，指导不同指标下的策略设计。


<details>
  <summary>Details</summary>
Motivation: 旨在提升生物医学问答系统在复杂问题上的准确性和鲁棒性，特别是在首次检索效果不佳时如何高效地进行重检索，并有效整合多个语言模型的答案以提高整体表现。

Method: 采用双管道检索架构：一是结合密集向量（BGE）、BM25和RRF的混合首阶段检索；二是基于代理的问题分解检索，覆盖PubMed、Europe PMC和iCite；并通过BGE交叉编码器作为质量门控触发选择性重检索。答案生成方面，将多模型集成增益分解为选择与融合两个组件，分别优化。

Result: 在Task 12B 2024验证集上，成本较低的选择性重检索策略显著优于基线，在列表F1和精确率上表现更优且重检索成本降低12%。在Task 13B 2025中，同义词联合解析器在列表召回率上领先，而GPT-5.5在列表F1上保持优势。在Task 14B 2026初步排行榜中，团队在八个（阶段×批次）排行榜中的三个综合精确指标上排名第一，赢得四个单项题型，并在Phase B b3理想得分中位列第一。

Conclusion: 合理的重检索策略和对答案集成中选择与融合机制的理解能显著提升生物医学问答系统的性能；未来应针对不同评估指标设计差异化融合策略，以平衡召回与精度。
...
TL;DR: 该研究探讨了语言模型在生成回答前是否能估计其对实体的熟悉程度，发现经过波兰语适配的模型能更准确地区分真实与虚构实体并追踪实体流行度，且熟悉度探测在不同提示语言下具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 旨在探究语言模型在生成答案前能否判断自身对某个实体的熟悉程度，从而提升模型的可靠性与可解释性。

Method: 分析十二个指令调优模型在最终提示词位置的激活状态，使用包含1440个波兰实体的新数据集，并设计熟悉度探测器评估模型对实体的熟悉程度。

Result: 熟悉度探测器能有效区分真实与虚构实体；波兰语适配模型能更好追踪实体流行度；提示语言变化不影响探测性能；在Gemma-4-12B中引入熟悉度方向可调控拒绝响应率；校准后的探测器在预生成拒绝门控中表现良好。

Conclusion: 研究表明存在一种分级的、可在生成前读取的实体熟悉度信号，且表征层面的熟悉度与决策策略中的回避行为是分离的。
...
TL;DR: 本文提出了一种名为MemCon的框架，将记忆操作建模为马尔可夫决策过程，通过在线学习策略自适应地控制何时、何地以及如何检索、注入计划和整合或遗忘记忆，从而在多种基准测试中显著提高了任务成功率并减少了令牌消耗。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型代理的记忆系统大多依赖于固定的、手工设计的启发式方法，这限制了其在不同上下文中的最优表现，因此需要一种能够根据具体情境动态调整记忆行为的方法。

Method: MemCon框架将记忆操作视为一个马尔可夫决策过程，并使用轻量级表格化上下文赌博机与UCB探索机制，在没有预训练且不增加额外LLM调用的情况下，从每个任务的二进制反馈中学习一个在线策略来决定记忆操作。

Result: 在6个基准测试、3种代理框架和3种LLM后端上，MemCon相比多个基线方法任务成功率最高提升了15.2个百分点，同时将令牌消耗降低了5%到20%。

Conclusion: MemCon提供了一种通用且高效的方式来自适应管理大型语言模型代理的记忆，克服了传统静态记忆系统的局限性，展现出在多样化任务中提升性能的巨大潜力。
...
TL;DR: 本文提出了一个新的任务——类比深度研究（ADR），并构建了首个ADR基准测试集，以评估大语言模型代理在前瞻性分析中发现和利用历史类比的能力。研究发现，现有模型倾向于基于表面特征匹配，而非深层机制，因而表现不佳。为此，作者提出因果类比研究者（CANA）框架，通过机制对齐和跨类比验证两个原则，结合结构分解与反馈机制，显著提升了类比识别与整合能力，在ADR-bench上性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在进行前瞻性分析时难以有效识别和利用具有深层结构相似性的历史事件（即历史类比），主要停留在表面特征匹配，缺乏对事件背后因果机制的理解，限制了其在战略预见中的应用。

Method: 提出类比深度研究（ADR）任务及ADR-bench基准；理论分析指出ADR本质上是因果问题，需满足机制对齐和跨类比确认两项原则；设计CANA框架，采用结构化分解表示，并引入结构反馈机制以实现反思性优化。

Result: CANA在ADR-bench上相比现有最先进深度研究代理取得最高达10%的提升，在历史类比生成任务中表现更优；案例研究表明其能有效应用于现实事件的前瞻性分析。

Conclusion: 通过引入因果视角和结构化推理机制，CANA显著提升了大语言模型代理识别和利用历史类比的能力，验证了机制对齐与交叉验证在类比深度研究中的关键作用，为增强AI系统战略预见能力提供了新路径。
...
TL;DR: 提出了一种自演化的代理框架，通过分离变更提议与效果归因而提升LLM代理的性能，确保改进可信且泛化能力强。


<details>
  <summary>Details</summary>
Motivation: 在不修改模型权重的情况下，通过优化代理的外部控制机制（如提示、知识注入等）来提升其实际任务表现，并解决自动优化中的过拟合和评估噪声问题。

Method: 使用语言模型诊断失败并提出补丁，由确定性代码负责采样、测量和显著性检验，所有改进记录在基于‘问题位置×原因’的门控质量-多样性档案（GSME）中，并在封闭测试集上评估泛化能力。

Result: 在七个领域中，使用冻结的开源模型，该框架在封闭测试上取得了+9到+15.5个百分点的增益，且增益保持率为86%-147%，表明改进具有良好的泛化性而非过拟合。

Conclusion: 真正可迁移的是‘诊断-归因’循环机制，而非具体的代理配置；相同的病理对应相同的修补策略，跨模型家族也具有一致性。
...
TL;DR: 本文揭示了在使用大语言模型（LLM）作为裁判评估多语言忠实度时，由于生成阶段的解码预算参数共享导致的系统性故障，提出测试预言问题，并提供了无预言环境下的验证协议。


<details>
  <summary>Details</summary>
Motivation: 研究者发现当前LLM-as-judge系统中用于评估偏见的合成语料库存在结构性缺陷，尤其是当负例由LLM生成而缺乏可验证性时，可能导致严重且难以察觉的偏差。

Method: 通过构建一个多语言（土耳其语/英语）忠实度判断语料库，分析因共享解码预算参数而导致生成失败的案例，结合机制分析、控制实验和统计检测，揭示问题根源；并对比基于确定性扰动的语料库，提出零成本验证方法。

Result: 发现共享参数导致一个生成器的幻觉答案被截断，引发32点跨语言准确率下降，该效应在不同样本量下复现，但仅靠人工检查原始生成内容才能发现；修正参数后效应消失；另一种偏倚（Markdown格式偏好）也被扭曲；而基于扰动的语料库可通过字符串比较以100%准确率检测同类故障。

Conclusion: LLM生成负例的语料库面临‘测试预言问题’，缺乏逐项验证机制，易产生隐蔽且严重的系统偏差；建议采用具有内在预言（如确定性扰动）的构建方式，并提出适用于无预言场景的验证协议。
...
TL;DR: 本研究探索了基于自监督WavLM表示和DTW对齐的无文本二语语音评估方法，可在低资源条件下有效评估发音、节奏和语调。


<details>
  <summary>Details</summary>
Motivation: 传统二语语音评估集中于音段层面且依赖标注数据，难以推广至低资源场景；同时超音段特征（如节奏、语调）缺乏充分评估。

Method: 采用DTW对自监督WavLM表征进行对齐，比较学习者与母语模板的语音差异；针对节奏，分析DTW对齐路径的扭曲程度；针对语调，结合DTW距离、基频和强度特征。

Result: 在整体和句子级音段评分上超过人类评分一致性；节奏评估接近人类水平；语调评估在部分任务上表现有限。

Conclusion: 自监督表示可作为无文本、多维度发音评估的有效基础，具有在低资源环境下应用的潜力。
...
TL;DR: 本文研究了大语言模型在推理过程中置信度的变化，提出了一种三阶段校准框架，并发现不同训练方法对置信度的影响具有位置依赖性；基于此提出了位置感知的置信度策略PosConf，在答案聚合和早期停止中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的后训练方法（如监督微调、强化学习等）通常仅通过最终答案准确率评估，缺乏对推理过程中置信度变化的系统分析，因此需要一种更细粒度的评估框架来理解并利用置信度信号。

Method: 提出一个三阶段校准框架，分别评估推理前、推理中和推理后的置信度；在数学推理基准上进行受控比较，并分析不同方法（SFT、RL、OPD）产生的置信度在时间与位置上的可靠性；基于发现提出PosConf——一种只在可靠相对位置区间使用置信度的位置感知策略。

Result: 发现OPD提供最佳推理前置信度，SFT最适合用于早期停止，RL产生最可靠的轨迹级聚合信号；RL置信度在路径承诺阶段后才变得有用，而OPD置信度早期有效但后期可能出现逆校准；PosConf在答案聚合上比多数投票提升6.1点，在紧令牌预算下改善OPD早期停止最多达4.3点。

Conclusion: 推理模型中的置信度应分阶段、位置感知地使用，PosConf验证了结合位置信息使用置信度可显著提升实际性能。
...
TL;DR: SPyCE是一种技能-策略协同进化框架，通过在强化学习过程中将多模态推理轨迹提炼为分层技能库，实现策略与技能的共同演化，显著提升多模态智能体的性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法将轨迹简化为标量奖励，无法复用工具使用模式；基于记忆的方法依赖测试时检索，不能在训练中更新策略以吸收可复用的经验。因此，需要一种能在训练中持续提炼并利用可复用技能的机制。

Method: 提出SPyCE框架，将推理轨迹提炼为分层技能库，包括执行技能（捕捉局部视觉操作）和工作流技能（编码高层工具调用先验）。在训练中，策略模型基于检索到的技能指导 rollout，同时技能库利用策略生成的有价值 rollout 进行更新，形成闭环协同进化。

Result: 在八个基准上实验表明，SPyCE consistently 优于基于强化学习和基于记忆的基线方法。消融分析显示，分层技能设计和协同进化机制均对性能提升至关重要。

Conclusion: 技能与策略的联合优化是一种构建高性能多模态智能体的有前景范式。
...
TL;DR: 本研究探讨了在多语言环境下（巴斯克语、西班牙语和英语）使用基于Claim-Evidence-Reasoning和Divergent Questioning两种框架的提示词生成高阶问题的有效性，发现尽管大语言模型能有效生成多语言问题，但仅约一半被教师认定为高阶问题，且结构和概念多样性表明新框架可作为布鲁姆分类法的有益补充。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中于基于布鲁姆分类法的英文高阶问题生成，缺乏对其他理论框架和多语言情境的探索，导致教育实践中高阶提问的发展受限。

Method: 采用开源和专有大型语言模型，在巴斯克语、西班牙语和英语三种语言中，使用基于Claim-Evidence-Reasoning和Divergent Questioning的提示词生成高阶问题，并通过教师评估判断问题的阶次和质量。

Result: 两个模型在三种语言中均能较有效地生成可回答的问题，但仅有约一半被教师识别为高阶问题；同时，两种新框架生成的问题在结构和概念上更具多样性。

Conclusion: Claim-Evidence-Reasoning和Divergent Questioning是布鲁姆分类法的可行替代或补充框架，有助于促进多语言教学环境中批判性思维的发展，但需进一步优化以提高生成问题被认可的高阶性。
...
TL;DR: 本文提出了一种名为DeepStress的压力测试框架，用于评估搜索代理在面对低质量证据时的鲁棒性，并引入了新的指标来衡量系统在不可靠信息下的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管搜索代理在多步问答中表现出色，但其对低质量证据的鲁棒性尚未充分研究。这种问题在现实基准中罕见，但在实际应用中可能导致严重失败。因此，需要一个可控的框架来系统评估代理的表现。

Method: 提出DeepStress框架，通过用可控的合成环境替代检索模块，控制文档可信度、相关性和事实性三个维度，从而模拟不同质量的证据。在HotpotQA和BrowseCompPlus数据集上测试多个搜索代理。

Result: 实验表明，不同搜索代理在处理不可靠信息方面存在显著差异；提出了新指标，能更好记录系统输出及参数化知识与检索知识之间的冲突交互。

Conclusion: DeepStress为评估搜索代理在真实复杂环境中的鲁棒性提供了有效工具，揭示了现有代理的脆弱性，并推动了更可靠系统的开发。
...
TL;DR: 本文提出了一种名为ΔMergeLowRes的新方法，通过在权重空间中分离语言和任务的微调，并采用新的跨轴TIES组合规则，在低资源场景下有效提升了多语言模型在新语言和新任务上的适应能力。


<details>
  <summary>Details</summary>
Motivation: 在低资源NLP设置中，通常需要同时对新语言和新任务进行昂贵的语言-任务联合微调。本文旨在探索是否可以将这两个过程分开训练并在权重空间中重新组合，以降低计算成本并提升性能。

Method: ΔMergeLowRes从无标签单语文本学习语言增量Δ_L，从标注的英文数据学习任务增量Δ_T，并在推理时使用四种组合规则（加法、激活引导、稀疏感知和新型跨轴TIES）将它们组合。跨轴TIES借鉴了TIES-Merging的剪枝、符号选择和合并步骤，但将其应用于语言与任务轴之间。

Result: 实验结果表明：(i) 跨轴TIES在4种非洲语言中的3种上显著提升摘要性能（chrF提高+4至+7）；(ii) 在问答任务中F1提升+2.32，EM提升+2.91；(iii) 稀疏感知合并使分类任务的ECE降低36%，同时保持宏F1不变。

Conclusion: 不同的组合规则显著影响合并模型所保留、抑制和校准的内容，验证了分离语言与任务微调并在权重空间重组的可行性与优势。作者公开了所有JSON轨迹和声明账本。
...
TL;DR: 提出CAVE-ABSA框架，用于生成和验证面向方面的情感分析（ABSA）的反事实样本，以提升模型鲁棒性评估与数据增强的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有反事实生成方法多关注句子级别的情感翻转，难以保证方面级别情感变化的准确性，易导致语义漂移或矛盾问题。

Method: CAVE-ABSA通过定位目标方面的观点片段，进行受控重写，结合修复模块优化候选样本，并利用方面级验证、语义相似性、AMR引导的结构保持、编辑最小化、流畅性和矛盾检测等多约束过滤生成结果。

Result: 该框架能有效生成高质量、方面局部有效的反事实样本，可用于ABSA模型的鲁棒性测试与数据增强。

Conclusion: CAVE-ABSA通过分离生成与验证过程，提供了构建有意义方面级反事实的系统化方法，有助于检验ABSA模型是否真正依赖于方面相关的 sentiment 推理。
...
TL;DR: 本文提出了一种基于RAG的整篇文档翻译系统PAT，旨在通过语料库支持和上下文感知推动LLM超越传统的逐句翻译模式，实现面向专业验证的语境适配性翻译草稿生成，实验表明该方法能在段落、章节和文档层面实现一定重构，但效果仍有待提升。


<details>
  <summary>Details</summary>
Motivation: 传统机器翻译多以句子为单位进行处理，忽视了篇章级语用和文体差异；本文旨在探索如何利用大语言模型（LLM）结合真实语料库实现更符合目标语语境的整篇文档翻译重构。

Method: 提出PAT（Pragmatic Auto-Translator）系统，采用检索增强生成（RAG）框架，结合用户自定义规范与来自美国英语和拉丁美洲西班牙语长文本语料库的上下文信息，向LLM提供段落、章节及整篇文档级别的参考示例，实现整篇翻译生成。

Result: 对六篇关于生成式AI的英文文章进行了跨三个项目的自动翻译评估，使用定制MQM分类法由两名训练有素的评估者评分；结果显示仅使用简单提示时无显著重构效果，而结合规范与语料库信息的翻译在某些情况下实现了显著的语篇重构，但并非始终有效。

Conclusion: LLM可以被引导脱离逐句翻译范式，朝向语境驱动的整篇翻译重构迈进，但当前方法在重构有效性方面仍需进一步优化；同时讨论了自动翻译系统设计、语料库构建与质量评估方法的相关考量。
...
TL;DR: 本文提出了一种名为Hindcast的评估方法，用于解决大语言模型在回测预测任务中因数据泄露而导致的评估偏差问题。通过冻结历史数据并设定时间截点，确保模型只能使用截止时间前的信息进行预测，从而真实评估其预测能力而非记忆能力。


<details>
  <summary>Details</summary>
Motivation: 现有的回测方法在评估LLM时存在信息泄露问题：模型可能通过检索事后发布的文本或依赖训练数据中包含未来事件的信息来提升表现，导致评估结果反映的是回忆能力而非真正的预测能力。因此需要一种能封闭这些信息泄露渠道的评估方式。

Method: 提出Hindcast框架，选择一个过去的特定时间点t₀，在该时间点之后的信息对模型不可见；模型仅能访问t₀之前发布的Reddit帖子，并基于此对已解决的Polymarket预测市场结果进行预测；将模型的预测结果与t₀时刻市场的价格（即当时的人类预测）以及实际结果进行比较评分。

Result: 实验表明，在关闭信息泄露通道后，检索机制仅在事件已被历史讨论时有所帮助；而在仅有猜测的情况下，检索反而会降低预测性能。此外，该评估方法可重复应用于新市场，不会过时。

Conclusion: Hindcast有效解决了LLM在预测任务评估中的数据泄露问题，能够更公正地衡量模型的真实预测能力，并揭示了检索在不同信息环境下的实际作用。
...
TL;DR: 本文提出了一种细粒度的数据溯源系统ob，能够在记录级和token级追踪作者身份，精确响应数据删除请求，显著减少传统方法中的过度删除问题，并在实际模型中验证了其对机器遗忘效果的提升。


<details>
  <summary>Details</summary>
Motivation: 现有数据溯源系统只能在文件或数据集级别操作，导致在响应删除请求时需大规模删除数据（过度删除），缺乏精确到记录或token级别的溯源能力，阻碍了高效、合规的机器遗忘实现。

Method: 提出ob系统，通过在数据处理流程中传播作者身份信息，实现记录级和token级的数据溯源；利用确定性查询将删除请求精准映射到具体的训练记录，生成精确的遗忘集合。

Result: 在219,555个维基百科页面上的实验表明，记录级溯源将过度删除从101倍减少到1.3倍；系统引入的吞吐开销为HuggingFace上1.3-4.0%，Datatrove上2.1-19.0%；在1.7B模型上，基于溯源的遗忘集相比随机基线提升了42%的遗忘效果。

Conclusion: ob系统实现了高效、精确的数据溯源，解决了数据删除请求中的过度删除问题，显著提升了机器遗忘的准确性和实用性，为合规的数据管理提供了可行方案。
...
TL;DR: SPINE是一个用于简化双手机器人调试与部署的智能体框架，能够降低对专业机器人知识的依赖，并在不同平台上实现高效、可扩展的实体AI部署。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型虽具备复杂决策能力，但将其智能部署到物理机器人上仍需大量专家手动校准，限制了实体AI的规模化应用。这一‘部署鸿沟’是当前的主要瓶颈。

Method: 提出SPINE框架，包含两个协同的多智能体工作流：构建机器人特定上下文的配置生成器，以及通过诊断、修复和验证循环实现远程操作的调试器。

Result: 在七种DOBOT X-Trainer场景中，使用SPINE的新手操作者将部署成功率从75%提升至100%，平均部署时间从16分45秒减少到13分47秒；在AgileX PiPER平台上，SPINE成功解决了10个注入故障中的全部，而专家基线仅解决9个。

Conclusion: SPINE能够在不同双手机器人平台间迁移，显著减少对专家干预的依赖，推动实体AI向真实世界的大规模部署迈进。
...
TL;DR: 提出了一种干预性接地审计方法，用于检测大语言模型在推理过程中对前提的依赖性，在ProntoQA基准上验证了其有效性，并发现了大量“正确答案、错误推理”的情况。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成的推理链看似逻辑严密，但实际上可能并不真正依赖其所述前提，需要一种方法来检验其推理步骤与前提之间的实际依赖关系。

Method: 引入干预性接地审计方法：通过替换前提中的目标谓词为新符号并重新运行模型，检查每一步推理结论是否发生变化，从而判断其对前提的依赖性。在具有已知证明树的合成多跳推理基准ProntoQA上进行评估。

Result: 在GPT-4o上对50个ProntoQA问题进行测试，该方法在检测证明树依赖性方面达到F1=0.806（谓词决定依赖F1=0.885，召回率100%），显著优于自洽性基线（F1=0.343）。发现66%的正确解答中至少存在一个对直接依赖不敏感的推理步骤，且均涉及实体引入前提。

Conclusion: 干预性接地审计能有效揭示大模型推理中‘正确答案、错误推理’的现象，特别是在处理实体引入前提时存在盲点，说明当前模型的推理过程仍缺乏真正的前提依赖性。
...
TL;DR: 本文提出了一种基于$IFOL_B$的概率神经符号AGI框架，通过结合Nilsson的概率结构与Shannon最大熵原理，利用神经网络计算概率密度函数KI，并引入全局与局部对称变换以增强认知能力。


<details>
  <summary>Details</summary>
Motivation: 克服纯神经系统的局限性，如缺乏可解释性和逻辑结构，结合符号推理与神经学习的优势。

Method: 采用$IFOL_B$的Nilsson概率结构，结合Shannon最大信息熵原理，设计神经网络来计算KI概率密度函数，并引入全局和局部对称变换机制。

Result: 实现了在保持知识库和逻辑演绎不变的前提下，支持实时决策的概率推理机制，增强了神经符号系统的认知能力。

Conclusion: 所提出的概率神经符号方法有效扩展了$IFOL_B$的认知功能，为实现可解释、结构化的通用人工智能提供了新路径。
...
TL;DR: 本文综述了现代自改进智能体作为自适应系统的发展，提出一个系统级框架，将自改进形式化为自我诱导的更新操作符，并按更新目标和驱动变化的信号组织先前的工作。


<details>
  <summary>Details</summary>
Motivation: 随着自改进自主代理从研究原型转向部署系统，主要目标是实现可控的进化或适应，即通过经验积累能力提升而无需或只需最少的人类输入。

Method: 提出了一个系统级框架，该框架将现代代理表示为耦合基础模型与操作支架（包括提示、记忆、工具和控制逻辑）的配置。在此框架内，自改进被形式化为一种自我诱导的更新操作符，用于获取并对模型参数或支架组件进行更新。

Result: 整理了以往工作，按照更新目标和驱动变化的信号进行了分类，回顾了应用情况并讨论了评估方法。

Conclusion: 总结了开放问题和未来方向，强调了自改进代理领域的重要性和挑战。
...
TL;DR: 提出了一种上下文增强的提示框架，通过结合图神经网络（GNN）生成的结构信息来增强小语言模型在分子属性预测中的表现，显著提升了零样本预测准确率，但仍与专用GNN模型存在差距。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在从SMILES字符串进行分子属性预测时存在“结构盲区”，因为序列表示无法充分捕捉关键的图拓扑线索。

Method: 设计了一个模块化的上下文增强提示框架，在推理时引入代理式工具使用：由训练好的GNN专家模型提供带置信度的预测提示，并提取实例特定的解释性子图（如子图SMILES和解释段落）以丰富提示内容。

Result: 在MUTAG和Tox21数据集上评估了三种常见SLM和五种提示配置，结果表明引入图衍生上下文可显著提高准确性，相对提升常超过25%，在Tox21上最高达74%；并通过基于必要性的边删除干预验证了所提取基序的功能相关性。

Conclusion: 尽管通过上下文增强提示显著提升了小语言模型的分子预测性能，但其与专用GNN模型之间仍存在差距，表明基于文本的推理在处理分子结构时具有潜力也有局限。
...
TL;DR: 本文提出了一种基于Oracle数据库的代理记忆系统（Oracle Agent Memory），旨在解决长时程代理中的任务状态保持、用户偏好记忆和程序性知识积累等系统性问题，通过分层架构与生命周期管理，在保证高准确率的同时显著降低token使用量。


<details>
  <summary>Details</summary>
Motivation: 随着代理系统在复杂、长时间交互场景中的应用增多，传统基于文档检索的记忆方式已无法满足对状态持久化、范围控制和高效检索的需求，因此需要一种新型的记忆机制来支持长期、跨会话的任务执行。

Method: 设计并实现了一个数据库原生的代理记忆框架——Oracle Agent Memory，采用分层架构，将活动记忆核心与被动存储接口分离，并引入显式的范围控制机制；同时定义了记忆的完整生命周期，包括摄入、提取、整合、检索、总结及修订或删除，并结合下游任务性能与记忆专用指标进行评估。

Result: 在LongMemEval测试中达到93.8%的准确率，相比扁平历史基线减少约10.7倍的token使用量，且在证据检索、召回率、延迟等方面表现优越。

Conclusion: Oracle Agent Memory作为一种可扩展、可控且高效的记忆基础设施，为长期运行的智能代理提供了可行的记忆管理系统解决方案，展示了数据库在AI代理系统中的关键作用。
...
TL;DR: 本文提出了一种以人为中心的方法DROPJ，用于在未知环境动态且无明确奖励函数的安全关键场景中安全地训练和部署智能体策略。通过学习世界模型并结合人类在模拟器中的轨迹与偏好反馈（含理由），训练奖励模型并实现安全部署。实验表明该方法降低了训练成本，提升了部署性能与安全性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键环境中，传统强化学习因缺乏奖励函数和环境动态知识而难以应用，需要引入人类输入以实现安全有效的策略训练与部署。

Method: 首先从真实世界轨迹数据集中学习一个世界模型（模拟器）；人类在该模拟器中生成有价值的模拟轨迹；采集人类对轨迹片段的偏好及其理由，训练基于这些有理由偏好的奖励模型；结合世界模型与奖励模型，使用模型预测控制直接部署智能体。

Result: 实验显示，相比其他策略，用户生成的模拟轨迹显著降低训练计算成本并提升部署性能；使用带理由的偏好反馈比其他反馈形式更能提高部署时的性能与安全性，尤其能增强特定安全需求的满足。

Conclusion: DROPJ方法通过结合人类在模拟环境中的交互、偏好及安全理由，实现了高效、高性能且更安全的智能体部署，验证了在缺乏真实环境信息和奖励函数下人类中心方法的有效性与优势。
...
TL;DR: 本文介绍了一个名为cayleyR的R包，用于通过检测Cayley图中的循环交集来解决置换谜题。


<details>
  <summary>Details</summary>
Motivation: 为了解决如TopSpin(n,k)这类置换谜题，需要一种高效的方法在大规模状态空间中寻找从初始状态到目标状态的路径。

Method: 提出了一种迭代的双向搜索算法：从初始状态和目标状态同时出发，通过生成对称群Sn的Cayley图中的循环，并寻找这些循环的交集以得到连接路径；当无直接交集时，采用距离引导的桥接选择缩小差距并重复过程。算法结合C++哈希索引状态存储与可选的Vulkan GPU加速实现。

Result: 成功实现了针对TopSpin(n,k)谜题的求解器，能够在大规模状态空间中有效寻路，并已将该软件作为R包发布于CRAN。

Conclusion: cayleyR提供了一个高效、可扩展的框架用于求解基于置换群的谜题，展示了组合数学与高性能计算结合在实际问题中的应用潜力。
...
TL;DR: 本文提出了Mycelium，一个主动共享工作空间，旨在通过连接研究人员和AI代理来促进网络化智能，从而解决复杂科学问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI科研系统多关注于单个推理过程的扩展，而忽视了团队合作中不同背景成员间知识与直觉的互补性。

Method: 开发了一个名为Mycelium的多用户协作平台，自动捕捉并关联研究中的观察与假设，并将其路由至最能利用这些信息的个体或代理。

Result: 在生物多组学项目中验证了Mycelium的有效性，成功将局部发现转化为跨领域机制约束并指导实验设计。

Conclusion: 网络化智能在处理不可合并的分布式科学情境时具有不可替代的优势，独立的专业知识使得这种网络结构无法被单一扩展型代理所取代。
...
TL;DR: 本文提出了一种面向自主AI系统的保险原生数学框架，用于承保、定价和合同设计，通过风险状态建模分析可保性，并结合医疗案例验证其应用。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统具有决策、调用工具和与外部交互的能力，带来了新的保险挑战，亟需针对其特性建立专门的保险框架。

Method: 构建一个AI原生的数学框架，将部署的风险状态（包括自主水平、操作权限、权限暴露等）映射到事件概率、损失程度、治理成本等参数，并在参与性、盈利性和激励相容性约束下进行保险合同优化设计。

Result: 提出了可保性的结构特性，刻画了可保区域，揭示了暴露增加导致可行性单调恶化，并确定了治理认证阈值；框架支持保费、免赔额、覆盖分配和政策契约的优化，并可用于自动化理赔处理。

Conclusion: 该框架不仅将保险视为运营成本，还作为一种AI部署的监管机制，为自主AI系统的风险管理提供了理论基础和实践工具。
...
TL;DR: 本文提出了基础模型部署投资组合（FMDP）问题，旨在通过混合整数规划优化交通管理中心（TMC）中多个基础模型的部署，以最小化总拥有成本并满足各功能的质量、延迟和安全约束。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和视觉-语言模型在交通管理中的应用增多，如何在共享硬件预算下为不同任务选择合适的模型及部署方式成为一个关键问题。

Method: 将模型部署问题建模为混合整数规划问题，证明其NP难性，并提出一种多项式时间贪心启发式算法来求解。

Result: 在一个包含五个TMC功能和19个候选（模型，模式）对的案例研究中，FMDP找到的成本仅为34美元/月的投资组合，比最便宜的全闭源API方案低97%。

Conclusion: 结果表明，在适当的查询负载或API价格上涨情况下，本地GPU投资才具有经济合理性，且混合使用开源和闭源API可显著降低成本。
...
TL;DR: 本文提出了Harness Handbook和Behavior-Guided Progressive Disclosure（BGPD）方法，以解决现代AI代理系统中行为定位困难的问题，通过静态分析和LLM辅助构建行为中心化的表示，提升代码修改的定位效率与编辑计划质量。


<details>
  <summary>Details</summary>
Motivation: 随着模型、API和环境的不断演进，AI代理系统的harness需要频繁修改，但现有代码库中行为与代码的映射关系难以自动识别，导致维护成本高，因此需要一种自动化的行为定位机制。

Method: 提出Harness Handbook，利用静态分析和大语言模型（LLM）从harness代码库中自动生成行为中心化的结构化表示，并结合BGPD引导代理从高层行为逐步定位到具体实现代码，验证候选位置的正确性。

Result: 在两个开源harness上的实验表明，Handbook辅助的规划显著提升了行为定位准确率和编辑计划质量，尤其在代码分布分散、执行路径稀少和跨模块交互场景下效果更明显，同时减少了规划所需的token消耗。

Conclusion: 复杂的代理系统演化不仅依赖于生成正确的代码修改，更关键的是精准确定修改位置；Harness Handbook与BGPD为解决行为到代码的映射瓶颈提供了有效方案。
...
TL;DR: 本文提出理论级自动形式化（theory-level autoformalization）的概念，强调将整个理论体系（包括公理、定义和引理）及其相互依赖关系形式化为结构化库的重要性，并讨论其意义、挑战及未来发展方向。


<details>
  <summary>Details</summary>
Motivation: 现有的自动形式化研究多集中于单个语句的转换，但实际的形式化工作是理论层级的，需处理复杂的依赖结构；因此需要从语句级转向理论级的形式化方法。

Method: 通过分析现有工作的局限性，阐述理论级自动形式化的必要性，比较不同观点，识别开放问题，并提出三条可行的研究路径。

Result: 明确了理论级自动形式化的重要性和当前面临的挑战，并提出了推动该方向发展的策略。

Conclusion: 理论级自动形式化是实现大规模、机器可验证知识体系的关键下一步，需系统性地构建结构化形式化库以支持复杂理论的表达与验证。
...
TL;DR: EZSMTV3是一个基于SMT的可扩展约束ASP框架，通过利用先进的SMT求解器支持更复杂的输入语言、优化和混合域约束处理。


<details>
  <summary>Details</summary>
Motivation: 为了提升约束ASP系统的表达能力和灵活性，支持更复杂的组合搜索问题建模与求解。

Method: 采用翻译方法，将约束ASP问题转化为SMT问题，利用CVC5、YICES和Z3等先进SMT求解器进行推理，并引入对弱约束和新约束类型的扩展支持。

Result: 实验表明EZSMTV3在处理涉及整数和实数的混合域约束方面表现良好，并在基准测试中与CLINGCON、CLINGO[DL]、CLINGO[LP]等系统具有竞争力。

Conclusion: EZSMTV3提供了一个强大且可扩展的平台，有助于推动约束ASP领域的理论研究与实际应用发展。
...
TL;DR: 该研究探讨了当可靠工具在会话中悄然变化时，LLM代理如何调整其工具选择，借鉴认知心理学中的“集合转移”概念，设计了一个具有冗余工具的基准测试，评估代理对隐含可靠性变化的适应能力。


<details>
  <summary>Details</summary>
Motivation: 理解LLM代理在动态环境中对工具可靠性变化的适应能力，揭示其决策机制和局限性。

Method: 构建具有冗余工具的工具库，在隐藏边界处切换可靠工具组，并设置无切换对照；通过集合转移准确率（即每次切换后窗口中路由到目标工具组的联合概率）评估代理表现。

Result: 发现代理默认在少数几个工具间形成固定模式，调用分布集中在离散值上；不同模型表现出显著不同的失败模式；工具集的呈现方式（竞争性或互补性）影响路由动态。

Conclusion: LLM代理在面对隐式可靠性变化时适应能力有限，倾向于固化行为模式，且其决策受工具集框架效应的影响。
...
TL;DR: 本文提出了一种名为LAPO的自生成过程监督方法，用于多轮搜索推理中的强化学习，通过反向留一法归因评估每轮交互对最终答案似然性的贡献，并在无需额外奖励模型或教师的情况下显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的多轮搜索推理强化学习方法通常依赖于终端结果奖励，无法区分中间交互中有用、冗余或有害的部分，因此需要一种能够精确评估每轮贡献的过程监督机制。

Method: 提出LAPO方法，基于反向留一法归因：在每轮搜索中，将该轮及其检索结果替换为[DELETE]占位符，测量当前策略下黄金答案平均对数似然的变化（即答案似然增益），以估计该轮的贡献；同时引入符号一致性门控，仅保留与原始归因分数方向一致的归一化过程优势。

Result: 在七个知识密集型问答数据集上进行实验，LAPO的平均精确匹配得分为0.326，比最强的步级奖励基线IGPO高出0.053；消融实验表明反向归因和符号一致性门控具有互补优势。

Conclusion: 策略导出的回溯性归因可为多轮搜索代理提供有效的过程监督，LAPO在不依赖外部判断模型的情况下实现了更优的性能。
...
TL;DR: 本文提出了一种结构化的多智能体根因分析（RCA）流程，用于解决微服务故障中大规模、多模态遥测数据的根因定位难题，在OpenRCA数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的经典因果发现方法和基于大语言模型的多智能体系统在处理大规模、多模态且缺乏领域知识的生产环境微服务故障时表现不佳，难以准确识别根因。

Method: 提出一种结构化多智能体RCA流程，包含反向推理代理以诊断失败原因，并引入自动化规则挖掘流程从反向推理报告中提取判别规则。支持基于领域知识和无知识两种模式运行。

Result: 所提方法在OpenRCA基准上显著优于现有LLM和经典基线方法；反向推理分析表明大多数情况下证据存在但未被使用（推理差距），而非证据缺失；自动化规则挖掘减少了对人工知识整理的依赖。

Conclusion: 模型的推理能力和领域知识是当前主要瓶颈：更强的模型蕴含更多领域知识，显式知识注入可部分弥补弱模型的不足；即使证据提取完美，推理性能仍受限，必须通过提升模型能力来突破。
...
TL;DR: UrbanAgent是一种基于多智能体协同推理的新型城市区域画像框架，通过推理驱动和主动证据获取机制，在跨模态不一致和未见城市场景下显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有城市区域画像方法多为相关性驱动、假设模态间一致性且流程静态，难以应对异构或未见城市区域中的跨模态不一致问题，缺乏动态推理与验证能力。

Method: 提出UrbanAgent框架，为每种数据模态实例化独立智能体，通过结构化的多智能体协作推理来显式处理跨模态不一致性；将指标预测扩展为主动证据获取与迭代推理的闭环过程，并利用强化学习优化工具增强的外部知识检索以验证不确定推断。

Result: 在碳排放、GDP和人口估计的全球城市数据集上实验表明，UrbanAgent平均R2指标提升8.1%，在未见城市的设置下展现出强泛化能力。

Conclusion: UrbanAgent通过将城市区域画像重构为推理驱动的多智能体协同任务，有效解决了传统方法在跨模态不一致和动态环境适应上的局限，为城市计算提供了更鲁棒、可解释的新范式。
...
TL;DR: 研究表明，人工智能建议的普遍存在会显著降低人们在不确定时拒绝回答的意愿，即使AI建议错误，也会导致人们回答更多问题但准确率下降、自信却上升；激励准确性可部分缓解该现象，但AI仍可能改变人类判断是否作答的元认知阈值。


<details>
  <summary>Details</summary>
Motivation: 探讨人工智能建议如何影响人类在不确定情境下的判断行为，尤其是对‘不知道’这一判断能力的侵蚀效应。

Method: 通过五项实验（N=3,123，其中四项预注册，一项直接重复），参与者面对设计为AI易出错的难题，可选择是否作答，研究AI建议的存在形式（主动请求或被动展示）及其对人类回答意愿、准确性和信心的影响。

Result: 仅提供AI访问权限几乎消除了参与者悬置判断的意愿，无论建议是主动获取还是被动呈现；参与者回答问题数量增加，但正确率仅为无AI时的三分之一，而自信心却翻倍；引入准确性的奖励与错误的惩罚后，参与者更少依赖AI、作答更准确且更常选择不回答，但仍远不及无AI时的表现。

Conclusion: AI建议的普遍存在，尤其是未经请求的建议，不仅影响回答准确性，还可能改变人类决定是否作答的元认知标准，削弱承认‘不知道’的能力。
...
TL;DR: 本文提出了一种新的三路决策框架（EXECUTE, ASK, REFUSE）来改进LLM代理在现实环境中调用工具时的安全性，相较于传统的二元判断模型，该方法通过实例级细粒度控制和可调节的解码阈值，在不同风险容忍度下实现更优的安全性和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统安全守护模型将动作简单分为安全或不安全，忽略了动作本身危害性与用户上下文适用性的区别，且以类别为单位判断导致频繁误报，降低用户自主性并引发警报疲劳。

Method: 将安全决策重构为针对每个实例的三路路由决策（执行、询问、拒绝），并设计了名为Safety Sentry的轻量级守护模型，其推理仅需一次解码调用，通过调节解码时的阈值即可适应不同部署场景的风险偏好。

Result: Safety Sentry在整体准确率和安全相关召回率上优于多种开源及前沿闭源基线模型，同时能有效控制两类方向性错误率。

Conclusion: 该三路决策框架结合轻量级模型实现了更精细、灵活且高效的安全控制，提升了LLM代理在真实环境中的安全性与可用性平衡。
...
TL;DR: 本文提出了一种名为MEDA的智能体框架，结合大语言模型与符号回归，用于自动发现生物系统的常微分方程模型，在模型检索、外推和开放发现任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 旨在实现自动化科学发现，特别是在生物学系统中缺乏有效自动建模方法的背景下，推动从数据拟合工具向生成和优化机制性模型的转变。

Method: MEDA框架结合大语言模型（LLM）和符号回归（SR），通过检索背景知识、定义变量、生成机制约束、提出候选ODE并进行拟合与评估，实现对生物动力学系统的建模。

Result: 在多个任务中，MEDA成功恢复了正确的状态变量，在结构恢复上表现良好，并生成了生物学上合理的模型；消融实验表明知识引导和机制约束是关键组件。

Conclusion: MEDA展示了在生物系统中实现自动化机制建模的潜力，强调了结合领域知识与符号推理在科学发现中的重要性。
...
TL;DR: STOCKTAKE是一个为期26周的供应链补货基准，设计用于在部分可观测环境下分别评估LLM代理的状态估计和决策控制能力，通过技能评分、信念检测延迟和知行差距率来揭示模型在持续压力下的双重失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法无法区分LLM代理在多周期决策任务中因未观测到状态而导致的感知错误与行动失败，缺乏公平的参考策略来准确衡量其性能。

Method: 构建一个带有六个隐藏因子过程的结构化部分可观测马尔可夫决策过程（POMDP）基准STOCKTAKE，使用每个因子上的精确贝叶斯滤波器生成与代理相同观测流的可计算oracle参考策略，并基于此计算技能评分、信念检测延迟和知行差距率。

Result: 在50个种子和特定压力配置下测试四个主流LLM发现：Claude Sonnet 3.5、GPT-4.4、DeepSeek-V4-Pro和Grok 4.5能检测84%-88%的隐性故障且通常在发生后一周内识别，但技能评分从0.62到-0.23不等；其中两个模型虽表现优于盲基线但在诊断出的压力周仍出现较低的缺货率，反映出过度响应而非单纯响应不足的问题。

Conclusion: STOCKTAKE能够分离并量化LLM代理在复杂决策任务中的状态估计误差与控制失误，揭示‘知而不行’或‘行之过甚’两种失败形态，为改进代理设计提供了细粒度评估工具。
...
TL;DR: 提出了一个统一的具身人类寻找与跟随基准（UESF-Bench）和对应的视觉-语言-动作框架SeekFollow-VLA，以实现语义引导下的探索与持续跟随。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常假设目标人物在任务开始时可见，忽略了更现实的任务需求：代理需先根据语言描述找到目标，再在动态环境中持续跟随。此外，搜索与跟随常被割裂评估，缺乏统一评测基准。

Method: 提出UESF-Bench，一个大规模、多样化的具身寻找与跟随基准；并设计SeekFollow-VLA框架，采用任务驱动的路由机制进行潜在阶段推断与状态转换建模，实现从寻找至跟随的平滑过渡。

Result: 实验表明，SeekFollow-VLA在单人和多人环境中均显著优于单头和双头基线模型，展现出更强的语义探索、行为切换与延迟身份匹配能力。

Conclusion: UESF-Bench为具身智能体的统一寻找与跟随任务提供了新的评估平台，SeekFollow-VLA验证了任务驱动路由机制在复杂动态环境中的有效性，推动了语言引导的人类跟随研究向更现实场景发展。
...
TL;DR: 本文提出了一种基于归纳逻辑编程（ILP）的可解释强化学习（XRL）方法，定义了新的可解释性度量指标，用于量化符号规则与智能体行为之间的对齐程度、特征在决策中的作用以及策略在训练过程中的演变。


<details>
  <summary>Details</summary>
Motivation: 现有的XRL方法多依赖用户研究，缺乏统一的评估标准；而逻辑-based XAI方法虽具有可读性，但缺乏系统化的可解释性量化手段。本文旨在建立面向规划、客观的策略可解释性评估体系。

Method: 采用归纳逻辑编程（ILP）提取强化学习策略的符号化表示，并提出四个新的可解释性度量：激活率、特征覆盖度、句法距离和语义距离，以量化策略的可解释性。

Result: 实验表明，所提指标能够揭示不同RL任务中动作特定的学习动态、提供比传统方法更细粒度的特征重要性分析，并发现多智能体系统中的协作、专业化与适应模式，同时有助于策略的迁移与泛化。

Conclusion: 本文推动了可解释强化学习与逻辑型XAI的发展，提供了原理性的、可量化的解释评估框架，超越了主观评估和简单命题逻辑的局限。
...
TL;DR: 在开源软件项目中，当机器人作为参与者而非工具加入人类团队后，研究发现其引入与更紧密的协作、更强的社会记忆、更少的冲突以及更具独特性的产出相关。这些变化在机器人引入前后集中发生，且难以用其他解释说明。结果表明，基于规则的自动化代理可以成为社区社会基础设施的一部分。


<details>
  <summary>Details</summary>
Motivation: 探讨自动化代理（如机器人）作为团队成员加入人类团队时，是否会影响群体组织的稳定性与效率，特别是在开源软件开发环境中。

Method: 研究分析了2,991个GitHub项目在引入首个机器人前后两年的互动数据，从制度理论出发，衡量重复参与、社会记忆和角色分化三种协调能力，以及冲突级联和产出独特性两种结果指标，并观察变化的时间模式。

Result: 机器人引入后，项目中重复协作增加、对特定机器人的提及增多、冲突级联减少、产出更具独特性；这些变化集中在机器人采用节点附近，而非渐进积累；协调能力与产出功能相匹配（协调对应冲突减少，分化对应独特性），且人类方的能力可解释机器人与冲突的关系，但不能解释机器人与产出独特性的关系。

Conclusion: 基于规则、可预测的自动化代理可以融入社区的社会结构中，成为社会基础设施的一部分；机器人的加入是一个触发事件，真正的机制在于社会组织的重构。
...
TL;DR: AgentCompass 是一个开源、轻量级且可扩展的 LLM 智能体评估基础设施，通过解耦基准、执行器和环境三个组件，支持灵活配置和可复现的评估，并提供容错异步运行时与轨迹分析工具。


<details>
  <summary>Details</summary>
Motivation: 现有的 LLM 智能体评估流程碎片化严重、耦合度高，导致复现困难和工程重复，亟需统一、灵活且可扩展的评估框架。

Method: 提出 AgentCompass，将评估流程解耦为独立的 Benchmark（基准）、Harness（执行器）和 Environment（环境）三个组件，支持模块化组合；设计容错的异步运行时系统，并集成轨迹分析工具以诊断如奖励欺骗等复杂失败模式。

Result: AgentCompass 原生支持超过 20 个涵盖五个能力维度的基准测试，实现了评估流程的解耦与自动化，显著提升复现性和工程效率，并能有效识别智能体的细微行为缺陷。

Conclusion: AgentCompass 提供了一个开放、可扩展且可复现的评估基础设施，有助于推动 LLM 智能体研究的标准化与快速发展。
...
TL;DR: 本文提出了Canonical Action Verification and Attestation (CAVA)，一个将异构智能体活动转换为规范运行时动作对象的语义层，旨在解决AI系统在多运行环境中操作时的动作可验证性与治理难题。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统通过多种异构运行环境执行操作，不同运行时产生的不兼容记录使得难以确认哪些操作真正被授权、批准如何绑定到执行过程，以及第三方是否能复现相同操作身份。这带来了AI治理中的核心验证挑战。

Method: 提出CAVA框架，作为Proof-Carrying Agent Actions (PCAA) 的底层支撑，定义稳定的规范动作对象。该框架形式化了规范动作身份、语义模式检测、批准绑定、收据完整性、跨运行时投影及可选的证明机制，并通过包含96个种子、384个变体的基准测试进行评估。

Result: 基准测试覆盖语义等价性、分离性、包装绕过、误报控制、批准绑定、收据可重现性、证明篡改检测、运行时可移植性等多个维度，结果表明CAVA能够有效统一异构运行时的动作表示，并支持部署侧AI治理中的策略寻址与验证需求。

Conclusion: CAVA为部署方提供了动作级规范化和可策略寻址的语义模式基础架构，是实现AI系统可审计、可验证治理的必要组成部分。
...
TL;DR: 提出Experience Memory Graph (EMG)框架，将LLM代理的失败恢复转化为图匹配问题，通过训练时构建动作决策图并提取修正路径，实现测试时无需试错的高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的自我纠正机制在复杂长程任务中脆弱、成本高且难以泛化，需更鲁棒、高效的失败恢复方法。

Method: 将失败探索轨迹和专家成功轨迹构建成有向动作决策图，通过图匹配提取公共子图和图编辑路径，构建包含任务内节点和跨任务边的记忆图，并在测试时进行检索与引导。

Result: 在ALFWorld和ScienceWorld上实验表明，EMG在成功率和平均奖励上优于现有反射基线，且无需测试时的反复试错。

Conclusion: EMG提供了一种可泛化、低开销的代理失败恢复机制，显著提升长程任务中的性能与效率。
...
TL;DR: 提出AIMO可解释性挑战赛，旨在通过模型内部机制区分数学语言模型的稳健推理与虚假推理，解决标准基准无法识别推理稳定性的局限。


<details>
  <summary>Details</summary>
Motivation: 标准推理基准仅关注最终答案准确率，无法判断模型是否依赖稳定推理机制或利用脆弱的推理捷径，因此需要新的评估方式。

Method: 基于AIMO问题和提交内容，结合Fields Model Initiative资源，提供新发布的奥数级推理题、符号表示、前沿推理模型访问权限及对抗鲁棒性评估，支持参赛者开发识别稳健推理的方法。

Result: 比赛将建立一个新的开源鲁棒性基准和基线系统，促进数学推理与可解释性研究的标准化评估。

Conclusion: 该挑战赛连接了可解释性与泛化研究的核心问题：能否判断前沿AI模型的决策是否具有可推广性和可靠性。
...
TL;DR: HealthClaw是一种开源的代理架构，通过分离安全规则与私有长期记忆，实现个性化的纵向健康管理，在准确性和隐私保护方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的健康AI系统通常孤立地处理每次请求，无法适应个人健康状况的动态变化，缺乏对长期健康数据的有效利用。

Method: 提出HealthClaw架构，将共享的安全规则和医学知识与私有的纵向记忆分离，并通过归纳机制在每次交互后更新个人档案、程序或保留 episodic 记录；使用合成的长达一年的基准和9个200例生物医学任务进行评估。

Result: 在900次纵向支持测试中，HealthClaw使回答准确率从0.2%提升至45.7%，上下文暴露减少71.7%；在100次隐私测试中，其隐私保护表现优于基线；在生物医学任务中平均绝对增益为27.0个百分点，其中七项增益经FDR校正后仍显著。

Conclusion: HealthClaw通过受控的、可自我演进的记忆机制支持纵向个人健康代理，展现出在准确性与隐私保护方面的优势，但其临床有效性仍需前瞻性验证。
...
TL;DR: 本文研究了在持续学习场景中，不同代理优化方法的增益是否能够累积。通过在Terminal-Bench 2.0上的两阶段评估，发现只有将回归控制纳入优化循环的方法（如RELAI-VCL）才能在新任务上持续改进并保持先前收益。


<details>
  <summary>Details</summary>
Motivation: 大多数代理优化方法报告的性能提升都是单次性的，未验证这些提升在面对新任务时能否持续累积。本文旨在探究在实际部署环境中，优化增益是否能在多轮任务中复利增长。

Method: 采用Terminal-Bench 2.0中的困难任务构建两阶段持续学习评估框架，比较GEPA、Meta Harness和RELAI-VCL三种优化方法在相同预算下的表现，重点分析其在新任务上的迁移能力和持续优化潜力。

Result: 所有方法在静态单阶段设置中均优于基线，但在引入新任务后表现分化：GEPA退化至低于基线，Meta Harness能迁移但无法进一步提升，而RELAI-VCL不仅能正向迁移且可继续优化，最终达到最高各阶段通过率和终身平均通过率（76.4%）。

Conclusion: 优化增益仅在将回归控制内建于优化循环时才会复利增长，这为避免过拟合捷径解提供了归纳偏置，是实现可持续代理优化的关键。
...
TL;DR: 本文提出了一种端到端的人工智能加速框架，用于提升企业员工技能再培训的效率，并在多个阶段实现了验证和应用。


<details>
  <summary>Details</summary>
Motivation: 随着2030年近60%的员工需要再培训或技能提升，而当前填补技能差距的时间显著增加，现有框架多局限于单一环节且缺乏行业验证，因此需要一个全面高效的解决方案。

Method: 该研究设计了一个涵盖知识获取、内容开发、内容审核与验证、教学以及评估开发五个阶段的端到端AI加速框架，强调生产和学习效率的双重优化。

Result: 该框架获得了美国国家州会计委员会的认可，可用于继续职业教育学分；已有3名学员快速通过NVIDIA认证考试，14人正在学习中；其知识库还支持生成包含1,267个风险项的数据集，用于多智能体AI系统风险管理。

Conclusion: 该AI驱动的端到端框架在实际应用中展现出高效性和可扩展性，具备行业认可度和广泛的应用潜力，能够有效应对未来大规模技能转型的挑战。
...
TL;DR: Earthquaker-AI是一个结合教育机器人与基于检索增强生成（RAG）的对话式AI助手的混合教学框架，旨在提升小学生的地震应急准备意识和应对能力。


<details>
  <summary>Details</summary>
Motivation: 通过融合动手实践与认知训练，增强儿童在地震等紧急情况下的安全意识和自我调节能力，填补传统STEM教育在危机应对教育中的空白。

Method: 在原有Lego WeDo2机械模拟项目基础上，引入基于RAG的对话式AI助手，构建分年级递进的学习路径：低年级采用选择题识别安全行为，中年级判断行动序列，高年级进行短文作答；配合多维度评分量规提供反馈。

Result: 实验评估显示系统具有高准确性和回答可靠性，幻觉率低；AI能准确匹配学生问题与官方指南并生成安全、正确的回应。

Conclusion: Earthquaker-AI通过整合机器人、评分量规与AI技术，促进了技术素养、自我调节能力和数字系统的负责任使用，有助于培养早期危机管理技能。
...
TL;DR: 提出了一种名为Deep Interaction的高效人工干预机制，用于精确纠正大语言模型中的推理错误，通过直接编辑原始响应并提炼为蒸馏提示来引导模型沿正确路径推理。


<details>
  <summary>Details</summary>
Motivation: 现有的链式思维推理在出现错误时，重新生成或用户标记错误步骤的方法效率低下且容易重复出错。

Method: 允许直接编辑原始响应中错误的部分，并将修正后的链式思维提炼成蒸馏提示，以引导大语言模型沿着正确的推理路径进行推导。

Result: 在STEM任务推理上，相比基线方法，修正成功率提高了25%以上，同时减少了约40%的token使用量。

Conclusion: Deep Interaction能够高效、精准地纠正大语言模型中的推理错误，显著提升性能并降低计算开销。
...
TL;DR: 本论文报告了一项正在进行的关于自主无人机路径规划用于环境监测的系统性文献综述（SLR）的协议和初步结果，遵循PRISMA 2020框架，初步分析显示该领域研究活跃但分散，主要关注覆盖、多无人机协同和能耗优化，但在真实环境应用方面仍存在差距。


<details>
  <summary>Details</summary>
Motivation: 为了应对环境监测中无人机路径规划面临的覆盖范围、能量限制、操作约束和几何复杂性等挑战，需要对现有研究进行系统性梳理，以识别成熟技术和未解决的空白。

Method: 采用PRISMA 2020框架，在Scopus和Web of Science数据库中检索2015至2026年间发表的研究，筛选标题、摘要和关键词，对保留的研究进行全文评估，并分析算法类别、覆盖与能耗指标、障碍物处理、环境建模及约束条件。

Result: 共识别562项研究，去除161项重复后保留401项进行筛选，其中247项进入全文评估阶段（235项明确合格，12项待定）。初步分析表明多数研究集中于覆盖导向、多无人机协调和节能优化，较少考虑天气、不确定性和复杂障碍环境；大多数研究依赖仿真验证，存在仿真到现实的鸿沟；近期趋势显示强化学习、混合优化和几何感知规划受到更多关注。

Conclusion: 当前无人机覆盖路径规划研究领域活跃但碎片化，亟需系统综合以明确适用于实际环境监测任务的技术路径和发展方向。
...
TL;DR: HRIBench 是一个面向意图感知的人机协作诊断基准，通过结构化交互场景评估机器人在协同任务中的时序协调、响应性和安全性，揭示现有基础模型在协作能力上的不足，并验证其在仿真到真实迁移中的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作（VLA）基准主要评估孤立的操作技能，缺乏对人机协作中共享代理和交互结构的建模，难以反映真实协作场景下的意图理解、时序同步和安全交互需求。

Method: 提出 HRIBench，将协作任务建模为包含角色分配、时间依赖、协调约束和人类行为分布的结构化场景脚本，定义 Instructor、Collaborator 和 Intruder 三种交互角色，构建13个角色条件任务及650多个评估片段，并引入以交互为中心的可解释指标体系。

Result: 实验表明，尽管基础机器人策略具备较强的操控能力，但在协作场景中表现不佳，尤其在时序协调和意图感知方面存在明显缺陷；在 HRIBench 上微调可显著提升协作性能；仿真数据迁移到真实环境后，GR00T N1.5 的任务成功率从 0.10 提高到 0.43。

Conclusion: HRIBench 能有效评估和提升机器人在意图感知、动态协调和安全交互方面的协作能力，为发展交互中心化的机器人学习提供了有价值的基准和数据支持。
...
TL;DR: 本文综述了用于触觉反馈的高电压静电致动器（HVEA），涵盖四类主要技术，分析其机制、性能与应用潜力，并探讨设计挑战与未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着触觉界面在可穿戴设备和日常环境中的广泛应用，对柔软、轻薄、静音且节能的致动器需求增加，传统致动器难以满足这些要求，促使HVEA成为有前景的替代方案。

Method: 对四类HVEA——静电可切换粘附材料、介电弹性体致动器、软体电液致动器和电渗泵——进行系统性回顾，分析其触觉输出机制、带宽、力密度和空间可扩展性，并评估其在不同类型界面中呈现皮肤和运动觉反馈的能力。

Result: 识别出各类HVEA在实现触觉反馈方面的共性设计约束与新兴优化策略，包括提升人体工学设计、简化制造流程和集成自感知功能。

Conclusion: HVEA在推动下一代触觉交互方面具有独特优势，未来需聚焦于系统集成、实用化转化及关键技术突破以实现广泛应用。
...
TL;DR: 本文提出了GPUSimBench，用于评估GPU加速机器人模拟器在可扩展性、物理一致性和计算确定性方面的性能，揭示了当前主流模拟器在并行效率与物理保真度之间的权衡问题，并发现了GPU批量执行带来的固有非确定性问题。


<details>
  <summary>Details</summary>
Motivation: 随着数据驱动的具身AI依赖大规模并行仿真进行训练，现有GPU模拟器在物理保真度、并行效率和执行确定性之间的权衡尚未被充分研究，影响了机器人学习的可靠性。因此需要系统性评估工具来揭示这些隐藏限制。

Method: 提出GPUSimBench评估框架，通过倾斜平面任务进行物理接地评估，测量多环境规模下的吞吐量和内存占用以分析并行可扩展性，并量化相同初始条件下运行间和环境间的变异以揭示非确定性现象，识别当前模拟器堆栈中的随机性机制。

Result: 实验表明主流GPU模拟器存在显著的运行间和环境间动态不一致性；并行扩展带来可观测的物理行为偏移；发现四种经验性的随机性机制，显示无约束扩展会损害结果可重现性。

Conclusion: GPU加速模拟在提升训练吞吐的同时引入了不可忽视的物理不一致与非确定性问题，未来需在架构设计中显式处理这些因素以确保可靠且可复现的机器人学习。
...
TL;DR: 提出了一种层次贝叶斯生成模型，将类人机器人设计中的“恐怖谷”效应机制转化为可操作的数学变量，并通过实验验证了不确定性对熟悉度评分的影响。


<details>
  <summary>Details</summary>
Motivation: 现有类人机器人设计指南难以转化为可操纵的算法变量，限制了自动化设计优化；因此需要一个能形式化‘恐怖谷’效应经验规则的计算框架。

Method: 构建了一个层次贝叶斯生成模型，将亲和力表示为后验加权的负类别条件惊喜值，并识别出四个关键变量：与机器人类别均值的偏离、跨模态人类相似性不一致、预测不确定性及观测不确定性；并通过人类被试实验操纵后两个变量进行验证。

Result: 模拟显示类别模糊性和外观-动作不匹配会降低亲和力，且不确定性会重塑恐怖谷曲线；实验发现增加观测不确定性会减弱中等人类相似性下的熟悉度下降，而低预测不确定性则提升对机器人样貌的评分。

Conclusion: 该框架成功将经验性的恐怖谷启发式规则转化为可用于算法评估和优化类人机器人外观与行为的计算基础。
...
TL;DR: 提出一种基于ROS-Unity框架的多模态遥控架构，通过动态自适应视角系统（DAVS）和触觉反馈提升水下ROV在延迟通信和复杂环境中的操作性能。


<details>
  <summary>Details</summary>
Motivation: 传统水下遥控受限于二维视角和通信延迟，导致操作困难和认知负荷高，需通过空间感知增强来改善遥操作性能。

Method: 开发了一种解耦主动空间规划与反应式避障的架构，采用3D高斯点阵实现实时无遮挡外部视角，并结合振动触觉服提供障碍物距离反馈；在BlueROV2平台上进行N=30的人因实验，结合行为数据与fNIRS脑功能成像分析。

Result: 实验表明，在高延迟（0.5-1.0秒）下，外源性视觉显著优于其他模式；触觉反馈在低延迟时有助于路径跟踪；fNIRS显示DAVS能维持前额叶激活，减轻认知过载。

Conclusion: 空间锚定的多模态辅助可显著提升延迟环境下水下遥操作的操作表现与认知耐力。
...
TL;DR: 本文提出了一种基于大语言模型（LLM）驱动的层次化“房间到对象”（HRO）框架，用于零样本目标对象导航，通过从粗到细的层次化空间认知提升探索效率和语义关联准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的零样本导航方法缺乏对人类层次化空间认知（如从房间语义到对象定位）的建模，导致探索盲目性强、语义关联不足，未能充分发挥LLM的常识推理能力。

Method: 提出HRO框架，利用LLM进行分层推理，首先理解环境中的房间级语义，再逐步细化至目标对象位置，实现从房间到对象的层次化导航策略。

Result: 在Gibson和HM3D数据集上的实验表明，HRO框架在成功率和泛化能力上优于现有的LLM基线方法。

Conclusion: 通过引入层次化空间认知机制，HRO框架有效提升了零样本对象导航性能，验证了LLM在该任务中结合结构化推理的巨大潜力。
...
TL;DR: WANDA是一种从单个演示中通过合成数据引擎学习开放世界移动操作的方法，能够实现大规模数据生成，提升策略的空间泛化、长视野鲁棒性和跨环境泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的数据收集方法（如遥操作和UMI）在扩展时需要大量人力和成本，限制了移动操作策略的训练数据规模，因此需要一种更高效的数据生成方式来突破这一瓶颈。

Method: WANDA首先从源RGBD观测中重建背景高斯点阵和机器人-物体交互轨迹，作为后续规划和渲染的基础；然后将富含接触的交互片段重排成多种空间构型，并利用全身运动规划将其连接成新轨迹；通过校正状态扩展增加机器人和物体状态的多样性；并在由日常照片生成的多样化3D环境中合成跨环境轨迹；最后通过将渲染的机器人与物体网格与高斯背景融合生成逼真图像。

Result: 实验表明，使用WANDA训练的策略在仿真和真实世界多种场景中均表现出良好的长视野鲁棒性、广泛的空间泛化能力和跨环境泛化能力，并支持零样本迁移到不同形态的移动操纵器上。

Conclusion: WANDA通过高效的合成数据生成，显著提升了从单一真实演示中学习移动操作策略的效果，为开放世界移动操作提供了一种可扩展且低成本的数据驱动解决方案。
...
TL;DR: 本文提出了一种利用气压计辅助的非线性姿态估计算法，通过融合IMU和气压高度数据，在GNSS拒止环境下实现高精度、鲁棒的姿态估计，具有几乎全局渐近稳定性和局部指数稳定性。


<details>
  <summary>Details</summary>
Motivation: 在GNSS缺失或速度传感器不可用的情况下，传统IMU因无法区分重力与惯性加速度而导致姿态估计不准确，亟需一种轻量且可靠的替代方案。

Method: 设计了两种非线性观测器：一是级联Riccati观测器与互补滤波器的确定性方法；二是直接在SO(3)×R²流形上构建的统一观测框架，融合IMU输入与气压计、磁力计输出。

Result: 仿真与实飞数据验证表明，所提方法在弱可观测条件下仍能保持稳定，显著提升倾斜角估计精度，尤其在无GNSS时优于传统方法。

Conclusion: 气压计可作为最小传感配置下姿态估计的有效补充，所提方法为自主飞行器在复杂环境中的可靠导航提供了实用解决方案。
...
TL;DR: 提出并验证了一种考虑状态扰动和参数不确定性的最小圈速规划框架，通过在关键路段选择性地引入鲁棒约束，提高了参考轨迹的可驾驶性和跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 为了提高赛车应用中最小圈速规划的鲁棒性，应对实际中普遍存在的状态扰动和关键车辆参数（如转动惯量、质心位置、气动阻力系数）的不确定性。

Method: 在已有扰动感知框架基础上，扩展了对车辆参数不确定性的建模；采用空间选择性激活策略，在最关键路段施加鲁棒约束以保持计算可行性；使用模型预测控制器（MPC）作为虚拟驾驶员进行大量仿真测试。

Result: 与非鲁棒的标称参考相比，鲁棒参考在1000次含随机扰动和参数变化的仿真中失败次数更少，关键信号（如车辆输入、车轴饱和度）围绕参考值的离散程度更低，表明跟踪性更好，尽管圈速略有增加。

Conclusion: 所提出的鲁棒最小圈速规划方法能有效提升参考轨迹的可驾驶性和鲁棒性，适用于存在状态扰动和参数不确定的实际赛车场景。
...
TL;DR: OptCar是一种用于提升越野高速自主驾驶中车辆动力学模型适应性的方法，通过结合少量真实数据与针对性的合成数据，实现从通用模型到特定车辆的高效专业化，同时保持跨地形泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有前向运动学模型在特定车辆上的专业化需要大量真实数据，且易过拟合于特定地形或驾驶工况，难以兼顾性能与泛化性。

Method: 提出历史条件动力学适配模块，将最近的状态-动作观测编码为动力学上下文令牌，并结合有限的真实世界数据与基于环境特定系统辨识生成的合成 rollout 进行微调。

Result: 在三种地形及分布外拖车任务的闭环MPC实验中，OptCar在6 m/s时相对AnyCar基线降低约55%跟踪误差，仅用每地形5分钟真实数据即能在道路上媲美使用30分钟数据训练的专用模型，且地形变化后仍表现更优。

Conclusion: OptCar能够在极少真实数据下有效提升特定车辆的动力学建模精度，同时保留跨地形的鲁棒性，为高速越野自主驾驶提供了高效的模型专业化路径。
...
TL;DR: 本文提出了一种环绕肢体的新型致动器布局——Limb-Encircling Actuator（LEA），旨在减小外骨骼设备在身体周围的占用空间，提升穿戴贴合性与日常使用的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有下肢动力外骨骼多将致动器置于腿部外侧或腰部，导致设备体积大、侵入性强，限制了日常使用和社会接受度。作者希望通过重构致动器几何结构，使外骨骼更贴近人体自然轮廓。

Method: 设计并开发了环绕肢体、垂直于肢体轴线平面的致动器（LEA），并通过自建测试平台对其机电性能进行表征；同时采用一种新颖的径向轴承布局，以实现轻量化和低成本优势。

Result: LEA实现了7.5 Nm/kg的持续扭矩密度，重量为894克，并展现出高扭矩输出和创新结构，但仍难以紧密贴合身体。

Conclusion: 肢体环绕式致动存在潜力与挑战，本研究为高扭矩密度、更易集成于日常服饰中的外骨骼设计提供了重要见解，未来需解决致动器尺寸与几何结构问题以实现更好贴合性。
...
TL;DR: 提出了一种分层超车框架，通过将决策与安全控制分离，并利用强化学习在线调整CBF衰减参数，实现了自动驾驶赛车中超车性能与安全性的更好平衡。


<details>
  <summary>Details</summary>
Motivation: 传统的固定衰减离散时间CBF方法在交互式赛车场景中过于保守，限制了超车性能且需手动调参，难以适应不同赛道条件。

Method: 采用分层架构：上层使用MIQP解决超车侧选择的组合问题，下层采用非线性Frenet框MPC结合嵌入式离散时间CBF确保动态与安全；并通过强化学习在线调整CBF衰减参数。

Result: 仿真和实车实验表明，自适应策略相比单一固定参数具有更高的综合成功率和更优的安全-性能权衡，无需针对赛道调参，提升了环境变化下的鲁棒性。

Conclusion: 该方法有效缓解了传统CBF的保守性问题，在保证安全性的同时增强了自主赛车的超车能力，具备良好的实际应用潜力。
...
TL;DR: 提出了一种将博弈论推理集成到基于采样的运动规划器中的混合架构，用于自动驾驶赛车中实现防御性策略（如阻挡）的高效交互轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶赛车需要在车辆动力学极限和多智能体竞争决策之间取得平衡，但直接在线求解动态博弈计算成本过高。

Method: 基于α-势博弈框架，利用离线学习的势函数捕捉多智能体交互，在线阶段通过梯度优化调整交互参数生成交互参考路径，并将其作为动态代价偏置引入高频采样规划器。

Result: 在 Yas Marina 赛道的高保真仿真环境中验证了方法的有效性，定性和定量结果表明该方法能有效实现阻挡等防御行为，且计算负担低。

Conclusion: 所提出的混合架构能够在不牺牲实时性的前提下，将战略交互行为有效融入自动驾驶赛车的轨迹规划中。
...
TL;DR: 本文提出了一种用于大规模异构多机器人系统在部分已知环境中执行复杂时序逻辑任务的鲁棒规划框架，通过区域绑定原子命题和扩展规划决策树结合基于遗憾的分支定界策略，实现了探索与利用的动态平衡，并展现出近线性可扩展性和优越的计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理部分已知环境中的复杂时序逻辑任务时，难以平衡探索不确定性区域与利用已知资源，且计算复杂度高，缺乏可扩展性。

Method: 将问题建模为最小-最大遗憾优化问题，提出区域绑定原子命题（RbAP）以在自动机结构中捕捉资源不确定性，并设计扩展规划决策树（E-PDT）与基于遗憾的分支定界（Regret-based BnB）策略进行高效求解。

Result: 理论分析证明了方法的可行性与完备性；大量数值与实物实验表明该框架在机器人数量和类型上具有近线性可扩展性，在解的质量和计算效率上显著优于MILP基线方法。

Conclusion: 所提框架有效解决了异构多机器人系统在部分已知环境中面对复杂任务时的可扩展性与决策鲁棒性问题，为实际应用提供了高效、可靠的解决方案。
...
TL;DR: 本文提出了一种基于白噪声加加速度（WNOJ）扩展卡尔曼滤波的LiDAR-惯性融合框架WNOJ-LIO，通过解耦的状态预测模型提升高动态驾驶下的点云去畸变和定位精度，并在真实赛车场景中验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 高动态驾驶环境下，传统LiDAR-inertial里程计因惯性测量噪声传播导致点云畸变校正和定位精度下降，亟需一种能有效抑制噪声传递的融合框架。

Method: 提出WNOJ-LIO，采用在R³×SO(3)上的解耦WNOJ先验进行状态预测，将IMU视为高频测量而非状态传播驱动源，并利用后验状态历史进行点云去畸变和点到平面的LiDAR更新，实现闭式协方差传播。

Result: 仿真结果显示在加速度与角速度去噪、点云去畸变及定位精度上优于FAST-LIO基线；实车实验在最高208km/h的多种振动条件下验证了方法的有效性，显著提升了各项运动状态估计性能。

Conclusion: WNOJ-LIO通过解耦建模有效抑制了惯性噪声传播，桥接了批处理高斯过程先验与递归滤波之间的差距，在高动态场景下实现了更精确的LiDAR-inertial里程计。
...
TL;DR: DynaDreamer提出了一种增强型Dreamer强化学习方法，通过引入显式的自车动力学先验来提升世界模型在自动驾驶中的想象准确性和场景建模能力，并实现零样本跨底盘适应。


<details>
  <summary>Details</summary>
Motivation: 现有基于鸟瞰图表示的世界模型将自车运动与场景动态纠缠在一起，导致模型过度关注自车运动恢复，影响了场景建模的保真度和想象准确性。

Method: 提出DynaDreamer，利用物理信息引导的自车动力学编码器-解码器提取自车状态历史为紧凑可识别的上下文，并用该上下文调制因果Transformer世界模型的先验和后验隐变量；在想象过程中，通过动力学预测器前向传播上下文以保持同步。

Result: 实验表明，DynaDreamer在城市和高速驾驶场景中相比最强基线分别将任务成功率提高28%和61%，在未见底盘上外推时优势达到73%。

Conclusion: 通过引入显式自车动力学先验，DynaDreamer有效解耦了自车运动与场景动态，提升了世界模型的建模效率与想象精度，并实现了无需训练的跨底盘零样本迁移。
...
TL;DR: 提出Anchor-Align方法，在行为克隆中通过视觉-语言锚定和语言-动作对齐保留预训练表征，提升视觉语言动作策略的泛化性和实际性能。


<details>
  <summary>Details</summary>
Motivation: 行为克隆微调会覆盖预训练模型中的视觉与语义通用表征，且现有共训练方法无法解决语言与动作之间的错位问题。

Method: 引入两个新目标：视觉-语言锚定通过从冻结的VLM副本中蒸馏逐层表征来防止表征漂移；语言-动作对齐将动作目标转化为离散运动方向标签，并在同一观测上联合训练语言与动作预测。

Result: 在真实xArm7机器人上，两种VLA架构的成功率分别从28%提升至54%和37%至60%；在模拟环境中，于LIBERO-PRO、LIBERO-Plus和CALVIN任务上均表现出对OOD扰动、感知鲁棒性和长视野控制的一致改进。

Conclusion: 保留预训练表征与高效动作学习可以兼得，Anchor-Align有效提升了VLA策略的泛化能力和实际控制性能。
...
TL;DR: 提出了一种受生物伤口愈合启发的去中心化、异步自修复策略，用于模块化航天器在局部信息和物理约束下的自主修复。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖冗余或预先规划的重构，无法在局部信息和物理限制下实现自主修复。

Method: 将航天器建模为格约束图，利用局部应激信号引导幸存模块向受损区域移动以闭合断裂间隙，随后各模块通过回溯自身运动恢复原始形状。

Result: 在PyBullet仿真中验证了该策略的有效性，即使30%模块随机失效，仍能将80%以上幸存模块整合为单一连通体，且性能随结构规模提升而改善。

Conclusion: 该方法适用于大规模模块化航天器群的自修复，具备良好的可扩展性和鲁棒性。
...
TL;DR: 提出了一种名为Physics-Guided Residual Dynamics (PGRD)的混合模拟框架，结合物理模型与学习模型，用于更准确地预测可变形物体的动力学行为，并在机器人操作规划和交互式仿真中展现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 准确预测可变形物体的动力学行为对机器人操作至关重要，但现有方法在精度和稳定性上仍面临挑战，因此需要结合物理模型的泛化性和学习模型的拟合能力。

Method: 采用可优化的弹簧-质点物理模拟器作为基础，并引入基于滑动窗口Transformer的神经网络预测物理预测结果的残差修正；使用基于速度的公式化方法以确保模拟稳定性。

Result: PGRD在多种真实世界可变形物体上的预测精度优于纯物理模型和纯学习模型，并成功应用于基于模型预测控制的操作规划（包括语言条件下的目标生成）和基于动作条件视频预测的交互式仿真（通过3D高斯点阵）。

Conclusion: PGRD通过融合物理引导与数据驱动的残差学习，实现了更准确、稳定的可变形物体模拟，具备在复杂机器人任务中应用的潜力。
...
TL;DR: 提出了一种通过时间反转简单任务来高效学习困难遥操作任务的框架，利用反向数据自动生成和分层优化，实现高精度操作任务的可扩展训练。


<details>
  <summary>Details</summary>
Motivation: 高质量的遥操作数据集难以收集，尤其是对于困难任务。而许多任务具有方向不对称性：正向完成困难，反向恢复容易。利用这一特性可降低手动采集成本。

Method: 提出一个包含闭环数据采集、分层数据 refinement 和迭代策略学习的框架：通过交替执行难任务和易任务策略实现环境自动重置；对简单任务轨迹进行时间反转，并使用运动先验和批评引导的优势过滤器去除低质量动作；利用初始反向演示和过滤后的数据进行在线持续学习。

Result: 在两个模拟基准和真实机器人实验中，该方法相比基于反转和强化学习的基线，在更少的数据下实现了更高的任务成功率和更稳定的训练过程。

Conclusion: 该方法通过自动化采集与分层优化反向数据，有效降低了对困难任务人工示范的依赖，实现了复杂高精度操作任务的高效、可扩展训练。
...
TL;DR: 本文提出了一种新的视觉-语言导航框架JOP-VLN，结合了离策略模仿学习与在线探索，通过三阶段训练流程在R2R和RxR基准上取得了当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言导航方法多集中于模仿学习或强化学习单一范式，缺乏两者结合的有效机制，限制了模型的错误恢复与探索能力。

Method: 提出JOP-VLN框架，包含三个阶段：首先在专家示范上进行模仿学习；然后使用DAgger算法生成启发式探索轨迹并用于进一步模仿学习；最后采用联合的在线与离线策略学习框架，结合高熵轨迹采样和优先纠错的轨迹排序策略。

Result: 在VLN-CE R2R和RxR基准上分别取得69.9%和68.0%的成功率，R2R上达到当前最优性能。

Conclusion: JOP-VLN有效融合了模仿学习与强化学习的优势，提升了导航过程中的错误恢复与探索效率，推动了视觉-语言导航的发展。
...
TL;DR: 本文提出了EgoHTR数据集，通过多传感器重建复杂环境中的4D人体运动序列，填补了人形机器人在非结构化地形中学习与场景感知之间的空白，并支持基于上下文的运动策略训练与硬件部署。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法难以应对真实世界复杂交互，而依赖人类先验的方法受限于缺乏场景上下文感知能力，主要原因是当前数据集无法捕捉挑战性环境中的人-景动态序列。

Method: 开发并开源了一个重建流程，利用第一视角可穿戴设备和便携式3D扫描仪，在多样化复杂环境中采集55组场景对齐的4D人体运动序列，构建包含超过15万帧的EgoHTR数据集，并评估其与动作捕捉真值的精度。

Result: 数据集达到最先进的重建精度，建立了严格的人体运动分析与合成基准；利用该数据训练出具有感知能力的行走策略，并成功部署在Unitree G1机器人上执行重建的参考运动。

Conclusion: 该数据集和重建流程支持社区扩展，为构建具备上下文感知、能在不平坦地形稳定行走的基础性人形机器人提供了关键资源和研究基础。
...
TL;DR: 提出了一种学习型状态估计器，用于在稀疏和噪声观测下重建可变形组织的完整状态，结合MLP与PCA潜在表示，并采用几何感知正则化，在2D模拟中实现了接近Oracle性能的多步牵引规划。


<details>
  <summary>Details</summary>
Motivation: 手术中的组织牵拉需要在部分且噪声较多的感知条件下进行有效的操作规划，现有方法难以在稀疏观测下准确估计可变形组织的完整状态。

Method: 提出一种结合多层感知机（MLP）与低维PCA潜在表示的学习型状态估计器，利用几何感知正则化训练，从40个噪声顶点观测中重建完整的可变形网格状态。

Result: 在2D可变形薄片模拟中，该方法在多步牵引规划中达到Oracle性能的98.1%，并支持高效推理。

Conclusion: 学习型、几何正则化的状态估计能够有效支持在现实感知约束下的可变形操作。
...
TL;DR: 本文提出了一套新的多机械臂运动规划器，结合经典算法与向量化碰撞检测技术，显著加速了碰撞检测过程，实现了接近实时的运动生成，并公开了代码以促进相关研究。


<details>
  <summary>Details</summary>
Motivation: 现有的基于搜索和采样的多机械臂运动规划方法计算耗时长，难以满足工业场景中对安全、高质量运动的实时需求。

Method: 提出了一种结合经典规划算法与基于CPU SIMD指令的向量化碰撞检测技术的新规划器，加速了运动规划中的主要瓶颈——碰撞检测。

Result: 新规划器在多机械臂任务中实现了高达两个数量级的加速，显著提升了运动规划与执行后处理的速度。

Conclusion: 所提方法能高效生成无碰撞、高质量的运动轨迹，支持接近实时的多机械臂控制，并通过开源代码推动该领域的研究与发展。
...
TL;DR: 提出了一种拓扑无关的触觉感知网络，能仅通过少量触摸点重建可变形物体的完整网格，并利用深度集成不确定性指导主动触碰策略，在无视觉条件下显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 在无视觉条件下（如黑暗、遮挡等）估计可变形物体的完整形状极具挑战性，而触觉是这些场景下的自然感知方式，但触觉信号稀疏且局部，需要有效方法从少量触摸中恢复整体形状。

Method: 设计了一个对排列不变的交叉注意力架构的单一同质估计器，能够处理1D绳索、2D布料和3D软体；采用深度集成（deep ensemble）来建模不确定性，并用于指导下一步最优触碰位置的选择。

Result: 该方法相比几何网格补全和高斯过程表面建模基线，重建误差降低约三分之二；在主动触碰任务中，优于随机触碰和高斯过程主动基线，尤其在自遮挡严重和误差尾部情况下增益更明显；当有视觉时触碰位置影响小，突显了所研究无视觉场景的重要性。

Conclusion: 所提出的统一架构能在无视觉条件下高效利用稀疏触觉信息重建多种拓扑结构的可变形物体形状，并通过不确定性引导的主动感知进一步提升性能，验证了其在极端感知受限场景下的有效性与潜力。
...
TL;DR: 提出了一种基于Noisy-OR融合模型的分层风险映射框架，用于提升自主轮椅在野外医疗环境中安全导航的能力。


<details>
  <summary>Details</summary>
Motivation: 野战医疗设施中频繁的患者转运导致防护资源消耗、人力紧张和感染风险上升，尤其在突发情况下难以持续。现有的自主轮椅难以应对复杂动态环境中的安全导航挑战。

Method: 设计了一个分层风险映射框架，融合四种异构环境风险（地形坡度、静态与动态障碍物、语义可通行性），通过Noisy-OR模型生成统一的概率代价图，并在蒙特卡洛模拟中评估其性能，同时在商用电动轮椅上进行室内外任务验证。

Result: 相比无风险感知基线，风险融合使碰撞率从73%以上降至32%以下，障碍清除能力提高一倍以上；在各种风险密度下，Noisy-OR方法表现出最高的障碍清除率和最低的峰值风险；实际部署验证了其在代表性任务场景中的有效性。

Conclusion: 该框架显著提升了自主轮椅在复杂动态医疗环境中的导航安全性与可靠性，满足了此前未被解决的作战医疗转运操作需求。
...
TL;DR: 本文提出了一种改进3D LiDAR SLAM中全局轨迹估计与局部地图质量的联合优化方法，通过引入不确定性感知的里程计和几何引导的回环优化，提升了重访位置的地图一致性。


<details>
  <summary>Details</summary>
Motivation: 现代基于图优化的LiDAR SLAM系统虽具有较高轨迹精度，但低轨迹误差并不保证几何一致的地图，尤其在重访位置可能因未检测到回环或残余漂移导致局部错位。

Method: 提出一种高效估计ICP几何依赖信息矩阵的框架，用于对位姿图中的里程计约束进行合理加权；设计分层回环检测模块，将地点识别与几何配准解耦，并引入回溯式回环检测模块利用优化后的位姿图恢复遗漏的回环；同时提出评估重访位置地图一致性的协议。

Result: 在多个数据集上的实验表明，所提方法在全局轨迹精度上达到或优于现有最先进方法，同时显著提升了重访位置的局部几何地图一致性。

Conclusion: 结合不确定性感知的里程计与几何引导的回环优化策略，能够生成更精确的轨迹和更高质的地图。
...
TL;DR: 本文提出了一种名为Kepler-Encoder-v0.1的机器人优先多模态编码器，通过融合视觉、本体感觉和力/扭矩信息，在训练中引入机器人状态模态，提升了仅使用视觉时对末端执行器状态和力的恢复能力，验证了在单时间步长下多模态融合的有效性。


<details>
  <summary>Details</summary>
Motivation: 机器人需要理解自身状态，但摄像头只能看到部分身体，且力和接触在单帧中几乎无迹可循，纯视觉特征在力感知上表现极差（R² ≤ 0.10），因此需要一种能从多模态数据中学习并提升视觉表征能力的方法。

Method: 提出Kepler-Encoder-v0.1，将机器人状态视为一种模态，使用带有学习查询的交叉注意力层融合视觉、本体感觉和力/扭矩信号，通过LeJEPA/SIGReg目标下的掩码跨模态预测进行自监督训练；评估时仅输入视觉，检验其隐空间是否包含超越像素的信息。

Result: 在RH20T数据集上，仅用视觉输入时，该方法显著优于冻结的ViT特征和计算量匹配的纯视觉控制模型，尤其在恢复末端执行器状态和力方面表现突出（尽管单步绝对力恢复仍有限）；在电机状态上与最佳视觉基线相当，且其隐空间几何结构能追踪真实状态；单一编码器适用于四种不同机器人，消融实验表明性能提升来自具身多样性而非数据量。

Conclusion: 将机器人状态融入训练能使视觉隐空间承载超出像素的内容，特别是在视觉薄弱环节（如力感知）表现出色，所学特征具有跨平台通用性和实用性，可用于无需训练的异常状态检测和图像重建，为下一步原生速率时序融合奠定了基础。
...
TL;DR: 本文提出了一种名为COLMAR的多智能体主动3D重建协作视点选择框架，通过共享策略优化和重建感知目标提升重建质量与覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 在多智能体主动3D重建中，传统方法存在冗余观测、空间聚集和协调效率低下的问题，限制了重建性能，因此需要一种有效的协作机制来优化视点选择。

Method: 将视点分配建模为基于地图中心观测的共享策略优化问题，设计了促进重叠感知覆盖、团队级发现和防碰撞探索的重建感知目标，并采用参数共享的PPO算法进行训练，部署时各智能体基于融合地图独立决策，无需通信；选定视点使用3D高斯溅射（3DGS）进行高保真重建。

Result: 在GLEAM和Replica数据集上的实验表明，COLMAR在相同感知预算下相比启发式和非协作基线方法，重建精度最高提升54%，覆盖范围增加49%。

Conclusion: COLMAR有效提升了多智能体主动3D重建中的协作效率，减少了冗余观测和空间聚集，显著提高了重建质量和覆盖率，验证了其在实际场景中的优越性。
...
TL;DR: 本文提出了一种基于TD3强化学习算法的自主机器人导航方法，用于在非定常、时变流场中到达目标点，比较了五种生物启发的感知策略，并发现基于局部速度感知与记忆的智能体表现最佳。


<details>
  <summary>Details</summary>
Motivation: 由于现实环境中流场的部分可观测性和不可预测性，传统依赖全局流场先验知识的控制方法难以实现有效导航，而生物系统能利用局部感知成功导航，因此需要开发更接近生物机制的自主导航策略。

Method: 采用TD3强化学习算法，在参数化混沌双涡环流场中训练智能体；评估五种基于相对位置、局部速度、局部涡量及短期记忆的观测策略，并分析提供全局流场参数对性能的影响。

Result: 能够感知并记忆若干个局部流速的智能体表现最优；速度感知型智能体能量效率更高，涡量感知则更利于环境结构建模和接近目标；意外的是，提供全局流场参数反而降低了导航性能。

Conclusion: 限制智能体仅使用隐式流场表示（如局部速度感知与记忆）可促使其发展出更鲁棒、更具泛化能力的导航策略，这为将仿生导航从仿真推向实际应用提供了重要启示。
...
TL;DR: 本文提出了一种名为IMMNet的混合模型/数据驱动算法，用于三维空间中的机动目标跟踪，结合了交互多模型（IMM）算法的可解释性与神经网络的学习能力，在保持贝叶斯推理机制的同时，能够自适应地从数据中学习运动模式和噪声特性，实验表明其在多种场景下优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 由于复杂的运动动态和模型不匹配问题，三维空间中的机动目标跟踪仍然具有挑战性，传统方法难以兼顾实时性、鲁棒性和适应性。

Method: 提出IMMNet算法，将交互多模型（IMM）框架与可学习的神经网络组件相结合，在保留贝叶斯推理结构的基础上引入数据驱动的学习能力，以实现对运动模式和噪声特性的自适应学习。

Result: 大量实验表明，IMMNet在多种场景下均优于现有的目标跟踪算法，表现出更强的鲁棒性和适应性。

Conclusion: IMMNet是一种兼具可解释性、实时性和高性能的机动目标跟踪方法，为雷达等实际应用提供了一个稳健且实用的解决方案。
...
TL;DR: 本文提出了一种名为APT-RL的统一框架，通过基于Transformer的强化学习实现四足机器人在复杂地形中的高速多技能运动，仅依赖机载感知与计算即可完成自主技能切换。


<details>
  <summary>Details</summary>
Motivation: 为了使四足机器人能够在复杂地形中高效、稳定地移动，需要整合多种运动技能并实现快速感知与响应，现有方法在速度、适应性和自主性方面仍存在不足。

Method: 利用简化动力学进行轨迹优化生成大规模2D运动数据集，训练出可迁移的高质量运动技能，并将其作为先验知识用于基于Transformer的强化学习框架中，实现在3D环境下的多技能平滑切换与高速运动控制。

Result: 实验表明，该方法可在真实四足机器人上实现最高6米/秒的瞬时速度，成功穿越楼梯、障碍物、缝隙和树枝等多种复杂地形，具备良好的鲁棒性与适应性。

Conclusion: APT-RL框架通过动作预训练与强化学习的有效结合，实现了仅依靠机载资源的高性能多技能运动控制，为四足机器人在复杂环境中的实际应用提供了可行方案。
...
TL;DR: 本文探讨了在人类-机器人团队（HRT）中主动管理信任的重要性，提出了一种“信任满意化”视角，并设计了一个包含在线信任度量、闭环行为适应和可变自主性的信任管理框架。


<details>
  <summary>Details</summary>
Motivation: 随着机器人自主性增强，其与人类团队成员之间的信任关系可能因误解或目标冲突而受损，影响任务成功率，因此需要有效的信任管理机制。

Method: 提出一种信任满意化的框架，包括在线测量信任代理指标、闭环调整机器人行为以及根据情境调整自主性水平，以维持团队合作中的信任。

Result: 初步实验参考了‘快速信任’概念和新的行为信任度量方法，验证了该框架的部分可行性，并识别出需进一步研究的问题。

Conclusion: 主动信任管理是确保动态高风险环境中人类-机器人团队成功协作的关键前提，未来研究应关注信任的动态性和情境依赖性。
...
TL;DR: 本文提出了一种新的方法来保持机器人动作表示中的语义结构，通过将动作表示锚定到语义流形并分解为共享和私有通道，在不改变部署模型的情况下显著提升了在分布内和分布外任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决现有视觉-语言-动作（VLA）模型在有限机器人演示数据上微调后语义结构退化的问题，并探索良好的动作表示应具备的特性。

Method: 受镜像神经元理论启发，引入一种即插即用的方法，将动作表示锚定到语义流形，并将其分解为共享语义通道和私有通道，推理时丢弃这些额外结构以保持原模型不变。

Result: 在多个VLA骨干网络和仿真与真实世界基准上验证，该方法在真实世界分布内任务上最高提升+18.7%，在分布外泛化任务上最高提升+21.5%。

Conclusion: 保持动作表示的语义结构有助于提升VLA模型的任务成功率和分布外泛化能力，所提方法有效且通用。
...
TL;DR: 本文研究了在长视野操作任务中，如何利用分段动作注释作为高级指令与视觉语言动作（VLA）策略之间的中间表示，并比较了不同接口和训练方式对策略性能的影响。


<details>
  <summary>Details</summary>
Motivation: 在复杂操作任务中，单一高级指令可能涵盖多个动作阶段，直接使用完整任务指令可能导致策略学习困难，因此需要探索更有效的中间表示方法来提升策略性能。

Method: 提出使用分段动作标注作为中间表示，通过进度模块跟踪当前活跃阶段，并将阶段信息以当前阶段文本或归一化的序数阶段索引形式输入动作策略；在GR00T N1.6模型上，于LIBERO-10数据集下比较直接微调和延续微调两种训练方式下的不同接口效果。

Result: 在直接微调下，完整任务指令、当前阶段文本和序数阶段状态的平均成功率分别为57.45%、50.24%和54.36%；在延续微调下，三者分别为49.07%、50.00%和53.75%，其中序数阶段状态在所有配对实验中均优于其他两种方法。

Conclusion: 显式阶段信息并不自动提升策略性能，但采用序数阶段状态作为输入能在延续微调下显著优于其他接口方式，表明接口表示形式与训练策略共同影响最终效果。
...
TL;DR: 本文提出了一种名为FoDeGlider的微型混合水下滑翔机，具有可大范围折叠和偏转的独立驱动翼，通过多体动力学模型和CRBA方法实现任意翼构型下的统一动态建模，并提出顺序参数识别框架，实验验证了其在狭窄水下环境中通过主动形态重构实现增强导航的能力。


<details>
  <summary>Details</summary>
Motivation: 为提升微型水下滑翔机在受限水下环境中的机动性和形态适应性，探索大范围翼重构这一尚未充分研究的方法，并解决由此带来的机械集成、动态建模和流体动力学表征挑战。

Method: 开发了一个将翼构型作为结构变量的多体动力学模型，采用基于复合刚体算法（CRBA）的投影方法，将复合惯性、力变换和部件级流体动力学整合到适用于任意翼构型的统一Fossen形式动态模型中，并提出顺序参数识别框架以估计机身和翼的流体动力学系数。

Result: 实现了在多种变形构型下准确的动态建模与参数识别，生成了用于模型识别和验证的开放基准数据集，门穿越实验证明了FoDeGlider能在运动过程中主动重构形态，提升在受限水下环境中的导航能力。

Conclusion: FoDeGlider展示了大范围翼重构在提升水下机器人机动性和环境适应性方面的潜力，所提出的建模与识别方法为未来可变形水下平台的设计与控制提供了有效工具和数据支持。
...
TL;DR: 本文提出了一种基于自然语言交互的移动机器人导航框架，通过ROS 2模块化组件将自然语言指令转化为导航动作，实现了对多种机器人的适配，并在仿真和真实环境中验证了系统的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了让非专业用户能够以直观的自然语言方式与机器人交互，解决将自然语言请求转化为自主导航动作的技术挑战。

Method: 构建一个基于ROS 2的模块化语言驱动导航框架，系统结合RGB-D数据识别目标物体、估计其位置，并生成导航目标，利用ROS 2 Nav2栈执行导航任务。

Result: 在TurtleBot3 Waffle和Unitree Go2机器人上进行仿真和实地测试，结果表明系统能准确理解直接命令和上下文请求，生成有意义的自然语言反馈，并成功导航至目标位置。

Conclusion: 该框架证明了语义感知与自主导航融合的可行性，为直观的人机交互提供了一种可移植、可扩展的解决方案。
...
TL;DR: 本文提出了一种名为QuadBoat的仿生无人水面艇，用于追踪并营救水上受困者，具备高机动性和精确控制能力。


<details>
  <summary>Details</summary>
Motivation: 现有的救援机器人研究很少关注快速从水中营救受困者的问题，因此需要一种高效、灵活的水上救援方案。

Method: 设计了一种具有四足机器人构型的无人水面艇QuadBoat，采用逆运动学控制器和级联模型预测控制（MPC）-PID控制器实现精准运动控制，并通过视觉跟踪和物体拾取实验验证其性能。

Result: 实验证明QuadBoat具有高机动性，在轨迹跟踪、腿部动作跟踪及目标拾取任务中表现出高运动精度和系统灵活性，能够在室内外环境中有效完成救援任务。

Conclusion: QuadBoat作为一种新型仿生无人水面艇，能够高效、准确地执行水上目标追踪与救援任务，为水上搜救提供了可行的技术方案。
...
TL;DR: 本文提出了一种通过机器人操作臂运动表达感知不确定性的数学框架，基于拉班动作分析将不确定性状态映射到特定的努力特征，并通过参数化运动原语实现对机器人犹豫、好奇等状态的表达，经人类被试实验验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 为了让协作机器人更好地在人机共处环境中传达其感知不确定性（如不完整或模糊的感知），提升人机协作的安全与信任。

Method: 基于拉班动作分析（Laban Movement Analysis）构建“承诺-警觉”状态空间，将不确定性状态（自信、好奇、犹豫、恐惧、无活动）映射到特定的Laban Effort特征；设计五种运动原语（接近、暂停、后退、探索、振荡），并使用11个运动学和几何描述符进行参数化。通过视频形式开展人类被试实验，评估四种表达性轨迹的识别率及各描述符对感知强度的影响。

Result: 参与者能够可靠地识别出预期的行为状态，多个描述符显著影响了行为表达的强度；实验结果为人用运动表达机器人不确定性提供了感知依据。

Conclusion: 该框架为在协作任务中通过参数化运动表示生成表达感知不确定性的自主轨迹奠定了基础，有助于提升人机协作中的沟通效率与自然性。
...
TL;DR: 本文提出了一种模块化框架，用于联合处理机器人精确抓取中的物体姿态估计和抓取误差，通过姿态分布估计与多视角融合降低不确定性，并验证了各模块对系统效率的提升作用。


<details>
  <summary>Details</summary>
Motivation: 为解决现有方法通常只针对特定挑战而缺乏通用性的问题，以及在模糊观测下难以确定唯一正确姿态所带来的不确定性，需要一种能够同时处理姿态估计和抓取误差的通用、灵活框架。

Method: 提出一个模块化框架，包含物体姿态分布估计、第二视角互补姿态分布计算与融合模块，以及两个独立的抓取误差补偿模块；通过多视角信息融合减少整体不确定性，并支持模块灵活组合使用。

Result: 在真实场景中对三种不同物体进行了实验，结果表明所有模块均能提高系统效率，且无错误发生；框架在SO(2)姿态空间下有效，具备向SE(3)扩展的潜力。

Conclusion: 联合建模姿态分布与抓取误差是一种提升机器人生产系统灵活性与可靠性的有前景方向，该模块化设计支持功能扩展与适应不同物理配置，据作者所知是首个实现此类联合处理的框架。
...
TL;DR: 本文提出了首个针对现代分布式电驱动卡车（DETs）的高保真学习型规划器基准nuTruck，结合精确非线性动力学模型与真实场景闭环评估，支持大规模神经网络训练并量化评估翻滚风险。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的方法在自动驾驶中逐渐被学习型方法取代，但后者在重型卡车尤其是分布式电驱动卡车上的应用仍缺乏研究。由于DETs具有复杂动力学特性和高侧翻风险，亟需一个高保真、可量化安全性的评估基准。

Method: 构建了一个高保真的仿真环境nuTruck，集成了精确的非线性卡车动力学模型，支持全轮独立驱动与转向，并能捕捉加减速和转弯时的动态载荷转移；在此基础上实现了多个基于规则和学习的规划器作为基线方法，并利用nuPlan的真实城市驾驶场景进行大规模闭环测试。

Result: 在真实世界场景下进行了广泛的闭环评估，结果显示所提基准能够有效评估规划器在避障能力与轨迹动力学安全性（如侧翻风险）方面的表现，验证了其在提升DETs行驶安全性方面的潜力。

Conclusion: nuTruck是首个面向分布式电驱动卡车的学习型规划基准，通过高保真建模和闭环评估，为DETs的自主驾驶规划提供了公平且贴近现实的评价标准，有望推动该领域的发展。
...
TL;DR: 本文研究了刚性连杆机器人与刚性环境碰撞时的速度变化，提出了一种称为非光滑冲击方向（NSID）的特征方向，并证明其在多种机器人系统中具有普适性，且与接触特性关系较小。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解和建模机器人在碰撞过程中的动力学行为，尤其是在瞬时碰撞假设下速度的不连续变化问题。

Method: 通过系统分析碰撞前后的速度关系，引入并定义非光滑冲击方向（NSID），结合理论推导与实验验证，探讨其在不同类型的机器人系统（如弹性/非弹性关节、受约束/无约束系统）中的适用性。

Result: 发现NSID是机器人碰撞的一个特征方向，基本独立于接触属性；入射方向相对于NSID决定了摩擦性和非弹性碰撞中冲量力的方向；结果适用于多种可逆驱动的刚性连杆机器人系统。

Conclusion: NSID为分析机器人碰撞提供了一个通用且有效的框架，可作为未来机器人冲击任务（如滑溜地面上的人形行走或重复锤击）中规划与控制算法的基础。
...
TL;DR: 本文提出了动态车辆定向问题（DVOP），扩展了传统定向问题，考虑了外部和车辆自身动力因素，应用于多旋翼无人机三维路径规划，结合非线性规划与混合整数线性规划以及邻域搜索算法，显著提升了奖励获取效率，并在真实无人机上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统定向问题未考虑复杂动力学因素，难以适用于实际无人机飞行环境，因此需要引入更真实的动力模型以提升路径规划的实用性与性能。

Method: 采用三维点质量模型（PMM）建模无人机动力学，结合最大速度与加速度约束及重力影响；提出分支定界（BnB）方法融合非线性规划（NLP）与混合整数线性规划（MILP），并设计大邻域搜索（LNS）元启发式算法提供初始解与可扩展性。

Result: 在基准测试中比现有Kinematic Orienteering方法提升最高达37%的奖励获取，且所提PMM轨迹在真实多旋翼无人机上成功部署验证。

Conclusion: DVOP更贴近实际应用场景，所提方法在保证动力可行性的同时显著提升任务收益，为复杂环境下移动平台的任务规划提供了有效解决方案。
...
TL;DR: 本文提出了一种基于视觉引导的分层强化学习框架VGPA，用于解决草莓采摘中目标果实被遮挡的问题，通过分解任务为障碍分离和目标抓取两个阶段，提升了采摘成功率。


<details>
  <summary>Details</summary>
Motivation: 由于成熟草莓常被未成熟果实遮挡，直接抓取困难，传统方法在复杂簇状环境中可靠性低，因此需要一种更智能的采摘策略。

Method: 提出VGPA框架，包含视觉引导决策机制和渐进自适应探索策略（PAES），高层用于优化选项选择，低层提升连续控制中的探索效率与训练稳定性。

Result: 仿真中成功率达96.7%；实机迁移实验中成功率为71.7%~88.3%，虽平均耗时增加1.22秒，但优于直接抓取。

Conclusion: 所提方法在复杂草莓簇环境中表现出良好的有效性、泛化能力和实际应用潜力。
...
TL;DR: 本文介绍了一个已部署的车辆在环（ViL）平台，将真实车辆与基于CARLA的数字孪生系统通过V2X通信耦合，用于自动化驾驶的虚拟-现实联合测试。实验表明协同感知显著提升了视野覆盖和目标检测性能，定位噪声成为主要误差源。


<details>
  <summary>Details</summary>
Motivation: 为满足欧洲自动驾驶认证对大量验证证据的需求，需构建可信的物理-虚拟混合测试平台，以高效、安全地评估复杂交通场景下的系统性能。

Method: 开发了一个融合实车与CARLA数字孪生系统的Vehicle-in-the-Loop平台，通过ETSI标准的CAM/CPM消息实现V2X通信，并在DT中采用GPU加速的协同感知模块，将多车信息融合为概率占据栅格。在类公共道路测试场开展多车十字路口场景集成测试，评估不同环境与定位噪声下的性能。

Result: 在多车T型交叉口场景中，协同感知显著扩展了视野覆盖范围并提高了占据栅格的召回率；当定位噪声超过一定阈值后，定位不确定性成为主导误差因素，而非雨天或夜间等天气条件。

Conclusion: 该ViL平台可有效支持自动驾驶系统的虚拟-现实闭环测试；当前架构受限于通信延迟与感知融合精度，未来工程优化应聚焦降低定位误差影响，并推动其作为地中海运行设计域（ODD）测试服务的落地应用。
...
TL;DR: 本文提出了一种基于代理的强化学习框架，通过引入执行质量评估指标和高层决策机制，提升机器人操作任务中的执行鲁棒性，在LIBERO基准测试中显著提高了成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作（VLA）模型缺乏对执行稳定性的显式评估和偏离时的恢复机制，导致在面对不确定性与干扰时容易失败。

Method: 提出了两种用于实时评估执行质量的互补指标，并设计了一个基于历史执行信息进行推理的代理策略，该策略通过选择不同的执行模式来调节过程，并在执行退化时触发恢复机制，使机器人回归正常状态。

Result: 在LIBERO基准上验证，标准设置下成功率最高提升13.7%，干扰环境下最高提升39.2%。

Conclusion: 所提方法通过高层决策与执行调控显著增强了机器人操作的鲁棒性和容错能力。
...
TL;DR: 提出了一种新型的递归ArUco标记设计，可将任意标准标记转换为具有任意深度的递归标记，支持远近距离下的鲁棒检测、抗部分遮挡，并允许无人机集群通过唯一标识符实现精准分组降落。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记在远距离或近距离下难以检测，且递归或分形标记受限于中心可见性、递归深度和字典容量，无法满足无人机集群高精度自主降落需求。

Method: 通过改进检测过程中的位采样策略，将完整标记嵌入父标记的黑白比特区域，实现任意深度的递归结构，并保持跨尺度的唯一标识符。

Result: 实现了无限递归深度、强抗遮挡能力的标记检测，在无需中心可见的情况下仍能稳定识别，并支持大规模唯一标记字典，可用于多无人机系统的独立降落定位。

Conclusion: 该方法克服了现有递归和分形标记的局限性，显著提升了检测范围与鲁棒性，为无人机集群的自主导航与精确着陆提供了高效可行的解决方案。
...
TL;DR: 提出了一种将生物启发式反应性与基于信念的运动规划相结合的混合策略，用于化学污染源定位，显著减少了行驶距离并提高了定位成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在化学污染源定位中存在效率低、过度探索或依赖人工调参的问题，难以在复杂湍流环境中快速准确地定位污染源。

Method: 设计一种检测触发的切换机制，将气味检测信号用于在横流探索和溯源运动之间动态切换，并利用信念度量自适应调整行为参数，实现无需手动调参的反应性与认知规划融合。

Result: 仿真和在葡萄牙蒙德戈河的实地实验表明，该方法最多减少50%行驶距离，定位成功率达86%，平均定位误差为3.2米。

Conclusion: 所提出的混合策略通过整合反应性与认知能力，在不牺牲鲁棒性的前提下显著提升了搜索效率，为环境监测中的自主源定位提供了更优解决方案。
...
TL;DR: 提出了一种统一的单样本框架，用于从单个（可能未完成）示范中学习接触丰富任务的正向和反向执行。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要多个完整示范，且不支持反向技能执行，难以应对具有几何约束和力交互的接触丰富任务。

Method: 将示范分解为非接触和接触阶段：非接触运动用动态运动原语（DMP）编码，接触运动则通过提出的几何驱动扭方向分割算法表示为一系列螺旋运动原语，并在导纳引导的姿态校正和速度调节下顺序执行。

Result: 在 peg 插入、电池插入、锁具开启和螺丝旋紧任务上的实验表明，相比基线方法，该方法具有更高的成功率和鲁棒性。

Conclusion: 所提方法实现了仅需单次示范即可完成正向与反向技能执行，提升了接触丰富任务中机器人操作技能的学习效率与灵活性。
...
TL;DR: 本文提出S-squared-VLA，通过解耦语义与空间流来解决视觉-语言-动作模型在自动驾驶中因语义-物理鸿沟导致的空间表征崩溃问题，显著提升控制精度与安全性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型难以生成精确的低层控制动作，因其离散语言标记与连续轨迹规划之间存在语义-物理鸿沟；同时传统VLA模型出现空间表征崩溃，损失关键几何先验。

Method: 提出S-squared-VLA，将语义流与空间流分离：语义流通过分层桥接提取多尺度VLM特征以增强意图推理；空间流绕过自回归语言瓶颈，保留原始视觉编码中的空间特征，并引入辅助感知监督以增强几何先验；最后通过级联注意力机制的双流规划适配器融合高层语义与精细空间约束。

Result: 在NAVSIM闭环基准上，S-squared-VLA在纯监督微调下达到87.1的PDMS得分，为当前最优；无碰撞率（NC）达98.4%，显著优于基线方法。

Conclusion: 通过显式解耦语义与空间表征，S-squared-VLA有效缓解了传统VLM的空间表征崩溃问题，在保持高阶推理能力的同时实现了精准、安全的驾驶控制。
...
TL;DR: 本文提出了一种将逆强化学习（IRL）与控制屏障函数（CBFs）结合的新框架，通过将IRL的奖励函数限制在CBF空间内，实现安全的在线控制，并能从无标签专家数据中数据驱动地恢复屏障函数。


<details>
  <summary>Details</summary>
Motivation: 传统的IRL方法依赖无约束探索，存在安全隐患；而CBFs虽能保障系统安全，但其解析设计复杂且耗时。因此，需要一种既能保证安全性又能从专家示范中有效学习的方法。

Method: 将IRL中的奖励函数候选空间约束为CBF的空间，使得学习过程自然满足安全约束。该方法允许直接从无标签的专家观察中恢复屏障函数，并在模拟导航环境中进行基准测试，同时比较了基于规划和基于策略的IRL方法在仿真和真实避障任务中的表现。

Result: 所提方法能够从专家数据中成功恢复出对未见不安全状态具有鲁棒性的屏障函数，在模拟实验中相比标准IRL基线展现出更高的安全性，并验证了不同类型IRL方法在安全性和性能间的权衡。

Conclusion: 通过将IRL与CBFs相结合，本文实现了安全的在线学习与控制，提供了一种可解释、数据驱动的安全关键系统学习框架，具有在真实场景中部署的潜力。
...
TL;DR: GigaWorld-Policy-0.5提出了一种动作中心化的世界模型框架，通过分离视觉动态建模与动作生成，在保持利用未来视觉监督优势的同时显著提升推理效率，实现低延迟的机器人控制。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAMs）在推理时需显式生成未来视频，导致计算开销大，难以实现实时闭环控制。为解决这一问题，需要一种更高效的推理机制。

Method: 采用动作中心化建模框架，训练时结合动作条件世界模型（AC-WM）与WAM策略，推理时仅解码动作；引入Mixture-of-Transformers架构，将视觉建模与动作生成分配给专用专家，并通过基于代理的AutoResearch流程自动搜索最优训练配置。

Result: 实现了85ms的推理延迟（RTX 4090），增强了动作表征的可迁移性，同时保持了未来视觉动态带来的训练优势。

Conclusion: GigaWorld-Policy-0.5在不牺牲训练效果的前提下显著提升了推理效率，更适合实时机器人控制应用。
...
TL;DR: AeroMap3D是一种单目6-DoF无人机定位系统，利用视觉、几何和语义地图先验信息实现无GNSS环境下的导航，具有高精度和强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决在无GNSS环境下，无人机遥感图像与卫星地图之间存在的跨视角差异以及数字高程模型与城市场景之间的结构不一致问题。

Method: 提出一种轻量级适配器，使预训练的密集匹配器能在无需微调的情况下实现可靠的无人机到地图配准；通过估计尺度比和偏航偏移来消除几何错位，并结合DEM地形与OpenStreetMap语义信息优化2D对应点，使用延迟状态EKF融合相对运动先验进行轨迹估计。

Result: 在未重新训练或调参的情况下，AeroMap3D在八个奥斯汀地点的所有轨迹中实现了50米内的定位，55公里飞行距离上的平均3D误差为5.88米。

Conclusion: AeroMap3D有效解决了跨视角和结构不一致带来的挑战，实现了高精度、无需微调的无人机地图参考定位系统。
...
TL;DR: 本文提出了一种基于多模态模仿学习的工业灵巧操作框架，通过引入工业灵巧性基准（IDB）板、可扩展的DAG-ROS框架和多模态扩散策略AG-iDP3，在数据中心线缆操作任务中实现了78%的任务成功率，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 工业自动化中的灵巧操作（如线缆布设、连接器插入等）长期依赖人工，传统机器人方法在鲁棒性、泛化性和部署效率上存在瓶颈，亟需更高效的解决方案。

Method: 提出一种端到端的多模态模仿学习框架AG-iDP3，融合RGB图像、点云、关节位置和腕部力矩数据；采用R3M编码多视角图像，并基于约100次遥操作演示训练扩散策略模型，在自建的工业灵巧性基准（IDB）平台上进行评估。

Result: 在数据中心线缆清理任务中，最佳配置（多模态扩散策略）达到78%的抓取与插入综合成功率，显著高于单摄像头RGB基线的36%；各配置仅需约100次演示即可训练。

Conclusion: 正确的学习策略可在鲁棒性、泛化性和部署效率上超越经典视觉与控制方法，支持向可扩展机器人自动化在高运行时间工业环境中的转型。
...
TL;DR: PhysClaw-0是一个人机协同的自主数据采集系统，通过将自然语言纠正指令存入“纠正记忆”实现跨轮次复用，显著减少人类干预时间并提升策略学习效率。


<details>
  <summary>Details</summary>
Motivation: 现有自主数据采集方法在重复错误上需反复人工干预，导致监督成本随会话长度增加而非问题数量增加，效率低下。

Method: 构建PhysClaw-0系统，包含自主采集、验证与重置循环，仅在明确重试预算耗尽时暂停并请求远程操作员介入；使用LLM解析自然语言指令为结构化调整并存储于Corrective Memory中，实现错误模式的记忆与复用。

Result: 在真实机器人桌面清理任务中，PhysClaw-0达到与遥操作相当的回合成功率，但人类工作时间减少至16%；语言纠正使验证器与人类一致性提升，并将单次尝试成功率从12.5%提高到47.5%（臂选择从20.0%提升至50.0%）；基于其数据微调的策略在远低于遥操作训练的成本下达到相当性能。

Conclusion: 通过引入可复用的纠正记忆机制，PhysClaw-0实现了高效、低人类干预的自主数据采集，为操作策略学习提供了可扩展且经济的数据收集范式。
